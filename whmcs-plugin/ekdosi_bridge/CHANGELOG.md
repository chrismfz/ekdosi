...
## [0.53.0] — 2026-09-22
### Added
- **Read-only προειδοποίηση: ΑΦΜ τρίτου που υπάρχει και ως κανονικός πελάτης.** Στη σελίδα διαχείρισης
  τρίτων ενός πελάτη (`prefsClient`), αν το ΑΦΜ ενός δικαιούχου-τρίτου υπάρχει ήδη ως **κανονικός πελάτης
  WHMCS**, εμφανίζεται **badge** («⚠ υπάρχει ως πελάτης») στο κελί ΑΦΜ της επαφής, **badge** στη γραμμή της
  υπηρεσίας που είναι δρομολογημένη σε αυτόν τον τρίτο («⚠ ΑΦΜ = πελάτης»), και ένα summary `alert` πάνω από
  τη λίστα. **Μόνο ενημέρωση** — κανένα κουμπί/ενέργεια (move/transfer/delete)· ο χειριστής ελέγχει
  χειροκίνητα για διπλοεγγραφή ή αλλαγή δρομολόγησης. Το ΑΦΜ πελατών διαβάζεται από το client custom field
  (name-scored, όπως το ιστορικό ΑΦΜ), η αντιστοίχιση γίνεται σε canonical (digits) μορφή, και η όλη σάρωση
  fail-closed (δεν χαλάει ποτέ τη σελίδα).
...
- **Admin διαχείριση επαφών τρίτων (CRUD).** Στη σελίδα ενός πελάτη («Προτιμήσεις τρίτων» → Προβολή) ο
  χειριστής μπορεί πλέον να **προσθέσει** («+ Νέα επαφή»), να **επεξεργαστεί** και να **διαγράψει** επαφή
  δικαιούχου — πριν υπήρχε μόνο re-routing, οπότε ήταν αδύνατο να διορθώσεις μια λάθος/παλιά επαφή από το
  admin. Η διαγραφή επαφής κάνει cascade τις δρομολογήσεις της. (Reuse των userid-scoped store writes της
  client v2 σελίδας· η επωνυμία είναι υποχρεωτική.)
- **Σύνδεσμος + ID υπηρεσίας στη δρομολόγηση.** Κάθε ζωντανή υπηρεσία δείχνει πλέον το `#serviceid` και
  **link στο WHMCS** (clientshosting.php / clientsdomains.php), ώστε να βλέπεις αμέσως ποια είναι και αν ζει.
- **Ενότητα «Δρομολογήσεις σε νεκρές υπηρεσίες» ανά πελάτη.** Δρομολογήσεις των οποίων η υπηρεσία δεν
  υπάρχει πλέον (αόρατες στο `servicesForUser`) εμφανίζονται τώρα με **per-row διαγραφή**.
- **Αναλυτική σελίδα «Ορφανά» (`action=orphans`) με per-row διαγραφή.** Απαριθμεί τις ορφανές επαφές
  (διαγρ. πελάτης) και τις ορφανές δρομολογήσεις (διαγρ. πελάτης / νεκρή υπηρεσία) με λόγο, ώστε να **βλέπεις
  ΤΙ θα σβηστεί** πριν σβήσεις — επιλεκτικά ή όλα μαζί. Στη λίστα πελατών το τυφλό κουμπί «Καθαρισμός ορφανών»
  αντικαταστάθηκε από link «Προβολή/διαχείριση ορφανών» προς αυτή τη σελίδα (το bulk «Καθαρισμός όλων»
  παραμένει εκεί, πίσω από την ορατή λίστα).
...
- **Σελίδα «Προτιμήσεις τρίτων» — polish + UX.** Η λίστα πελατών (72 γραμμές) απέκτησε:
  **instant search** (επωνυμία / #id / domain, client-side), **ταξινόμηση κατά δραστηριότητα**
  (ενεργές δρομολογήσεις φθίν., μετά επαφές, μετά όνομα — οι «βαρείς» πάνω), **γραμμή συνόλων**,
  **counts ως badges** (muted στο 0), **link στο WHMCS προφίλ** του πελάτη, και μια **στήλη
  «Υπηρεσίες»** που δείχνει τα δρομολογημένα domains (έως 3 + «+N» με tooltip). Το badge δρομολογήσεων
  μετρά μόνο **ενεργές** (σύνολο − νεκρές), ώστε μια νεκρή γραμμή να μη μετριέται δύο φορές.
- **Ανίχνευση & καθαρισμός ορφανών.** Οι πίνακές μας δεν έχουν FKs προς το WHMCS, οπότε διαγραφή
  πελάτη/υπηρεσίας αφήνει «φαντάσματα». Η λίστα πλέον **επισημαίνει** τα ορφανά: γραμμές διαγραμμένου
  πελάτη (κίτρινο tint + badge «⚠ διαγραμμένος πελάτης», με fallback το όνομα της επαφής), και
  «⚠ N νεκρές» δρομολογήσεις σε υπηρεσία που δεν υπάρχει πλέον (η ανίχνευση είναι **existence-based**,
  όχι κενό domain)· φίλτρο **«Μόνο ορφανά»** + σύνοψη ανά κατηγορία. Νέο κουμπί **«Καθαρισμός ορφανών»**
  (POST-only + CSRF + confirm + activity log) που σβήνει *αυστηρά* μόνο routes/contacts που δείχνουν σε
  ανύπαρκτο **πελάτη ή υπηρεσία** — ακριβώς ό,τι δείχνει η λίστα, ποτέ ζωντανή δρομολόγηση.
  Νέα helpers: `ThirdPartyStore::routedServicesForUsers()` / `orphanSummary()` / `purgeOrphans()`.
