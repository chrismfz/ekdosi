...
1. **Push invoices to ekdosi.** From a WHMCS admin invoice page you send an
   ...
   **MARK** back into `tblinvoices.invoiced`.
...
2. **Activate**: *Admin → System Settings → Addon Modules → Ekdosi Bridge →
   ...
   - widens `tblinvoices.invoiced` to BIGINT (to hold 15-digit AADE MARKs);
   - creates its own tables `mod_ekdosi_contacts` + `mod_ekdosi_routing`.
...
- **Reset to unfiled** — sets `tblinvoices.invoiced = 0` (rare; only after you
  cancelled the AADE filing first).
...
- The bridge can run **alongside** the legacy `prepare_for_ekdosi` and
  `timologia` plugins during rollout. Activation warns if `prepare_for_ekdosi`
  is still active (both write `tblinvoices.invoiced`).
...
| Write-back fails / MARK not in WHMCS | `tblinvoices.invoiced` widened to BIGINT? (re-activate, or run the ALTER from §2). |
