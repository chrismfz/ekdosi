...
class InboundTicketRouterTest extends TestCase
{
    ...
    public function test_same_message_to_two_departments_opens_a_ticket_in_each(): void
    {
        $company = $this->company();
        ...
        // ONE email addressed To both departments → the same Message-ID lands in each
        // polled mailbox. Idempotency is per-DEPARTMENT, so each opens its own ticket
        // (a company-wide dedup would silently drop the second department's copy).
        $email = new ParsedInboundEmail(
        ...
        $this->assertNotSame($a->id, $b->id, 'each department gets its own ticket');
        $this->assertSame($deptA->id, $a->ticket_department_id);
        $this->assertSame($deptB->id, $b->ticket_department_id);

        // But a redelivery to the SAME department still dedups.
        $again = $this->router()->route($deptA, $email);
        $this->assertSame($a->id, $again->id, 'same-department redelivery is still idempotent');
        $this->assertSame(2, Ticket::withoutGlobalScope(CompanyScope::class)->count());
    }
