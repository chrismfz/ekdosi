object FrmCSConnect: TFrmCSConnect
  Left = 0
  Top = 0
  Caption = 'FrmCSConnect'
  ClientHeight = 660
  ClientWidth = 938
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Visible = True
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PanelMain: TJvPanel
    Left = 0
    Top = 65
    Width = 938
    Height = 314
    Transparent = True
    Align = alClient
    TabOrder = 0
    object GridCSInvoices: TcxGrid
      Left = 1
      Top = 1
      Width = 936
      Height = 312
      Align = alClient
      BorderStyle = cxcbsNone
      Font.Charset = GREEK_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      DragOpening = False
      LookAndFeel.Kind = lfStandard
      LookAndFeel.NativeStyle = True
      object ViewCSInvoice: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        Navigator.Buttons.First.Visible = False
        Navigator.Buttons.PriorPage.Hint = #928#961#959#951#947#959#973#956#949#957#951' '#963#949#955#943#948#945
        Navigator.Buttons.Prior.Enabled = False
        Navigator.Buttons.Prior.Visible = False
        Navigator.Buttons.Next.Enabled = False
        Navigator.Buttons.Next.Visible = False
        Navigator.Buttons.NextPage.Hint = #917#960#972#956#949#957#951' '#963#949#955#943#948#945
        Navigator.Buttons.Last.Visible = False
        Navigator.Buttons.Insert.Enabled = False
        Navigator.Buttons.Insert.Visible = False
        Navigator.Buttons.Append.Enabled = False
        Navigator.Buttons.Append.Visible = False
        Navigator.Buttons.Delete.Enabled = False
        Navigator.Buttons.Delete.Visible = False
        Navigator.Buttons.Edit.Enabled = False
        Navigator.Buttons.Edit.Visible = False
        Navigator.Buttons.Post.Visible = False
        Navigator.Buttons.Cancel.Enabled = False
        Navigator.Buttons.Cancel.Visible = False
        Navigator.Buttons.Refresh.Visible = False
        Navigator.Buttons.Filter.Hint = #934#953#955#964#961#940#961#953#963#956#945' '#948#949#948#959#956#941#957#969#957
        Navigator.Visible = True
        DataController.DataSource = DSInvoices
        DataController.Filter.AutoDataSetFilter = True
        DataController.Summary.DefaultGroupSummaryItems = <
          item
            Kind = skSum
          end>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        NewItemRow.InfoText = #917#953#963#945#947#969#947#942
        OptionsBehavior.NavigatorHints = True
        OptionsCustomize.ColumnHiding = True
        OptionsCustomize.ColumnsQuickCustomization = True
        OptionsCustomize.ColumnsQuickCustomizationReordering = qcrEnabled
        OptionsData.Appending = True
        OptionsSelection.CellSelect = False
        OptionsSelection.MultiSelect = True
        OptionsView.FocusRect = False
        OptionsView.NavigatorOffset = 10
        OptionsView.NoDataToDisplayInfoText = '<'#916#949#957' '#965#960#940#961#967#959#965#957' '#948#949#948#959#956#941#957#945'>'
        OptionsView.GridLines = glVertical
        OptionsView.GroupByBox = False
        OptionsView.HeaderEndEllipsis = True
        OptionsView.Indicator = True
        Styles.Background = StyleMain
        Styles.ContentEven = StyleEven
        Styles.ContentOdd = StyleOdd
        object ViewCSInvoiceid: TcxGridDBColumn
          DataBinding.FieldName = 'id'
        end
        object ViewCSInvoiceuserid: TcxGridDBColumn
          DataBinding.FieldName = 'userid'
        end
        object ViewCSInvoicegkriniaris: TcxGridDBColumn
          Caption = 'Invoice ASAP'
          DataBinding.FieldName = 'gkriniaris'
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.NullStyle = nssUnchecked
          Properties.ValueChecked = 'on'
          Properties.ValueGrayed = ''
          Width = 83
        end
        object ViewCSInvoiceHAS_3RD_INVOICE: TcxGridDBColumn
          Caption = #932#953#956'. '#964#961#943#964#969#957
          DataBinding.FieldName = 'HAS_3RD_INVOICE'
          PropertiesClassName = 'TcxCheckBoxProperties'
          Properties.Alignment = taRightJustify
          Properties.NullStyle = nssUnchecked
          Properties.ValueChecked = 1
          Properties.ValueUnchecked = 0
        end
        object ViewCSInvoicefirstname: TcxGridDBColumn
          DataBinding.FieldName = 'firstname'
          Width = 129
        end
        object ViewCSInvoicelastname: TcxGridDBColumn
          DataBinding.FieldName = 'lastname'
          Width = 97
        end
        object ViewCSInvoicecompanyname: TcxGridDBColumn
          DataBinding.FieldName = 'companyname'
          Width = 166
        end
        object ViewCSInvoicedatepaid: TcxGridDBColumn
          DataBinding.FieldName = 'datepaid'
        end
        object ViewCSInvoicetotal: TcxGridDBColumn
          DataBinding.FieldName = 'total'
        end
        object ViewCSInvoiceinvoiced: TcxGridDBColumn
          DataBinding.FieldName = 'invoiced'
        end
      end
      object GridCSInvoicessLevel1: TcxGridLevel
        GridView = ViewCSInvoice
      end
    end
  end
  object RollDetail: TJvRollOut
    Left = 0
    Top = 379
    Width = 938
    Height = 281
    Align = alBottom
    ButtonHeight = 23
    Caption = #923#949#960#964#959#956#941#961#953#949#962
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ImageOptions.IndexCollapsed = 11
    ImageOptions.IndexExpanded = 10
    ParentFont = False
    ShowFocus = False
    TabOrder = 1
    ToggleAnywhere = False
    FAWidth = 145
    FAHeight = 281
    FCWidth = 919
    FCHeight = 25
    object PageControl: TJvPageControl
      Left = 1
      Top = 24
      Width = 936
      Height = 256
      ActivePage = TabSheet1
      Align = alClient
      TabOrder = 0
      object TabSheet1: TTabSheet
        Caption = #915#949#957#953#954#940
        object Label1: TLabel
          Left = 32
          Top = 14
          Width = 73
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #908#957#959#956#945
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label2: TLabel
          Left = 32
          Top = 39
          Width = 73
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #917#960#974#957#965#956#959
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label3: TLabel
          Left = 16
          Top = 63
          Width = 89
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #917#960#969#957#965#956#943#945
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label4: TLabel
          Left = 328
          Top = 63
          Width = 48
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #913#934#924
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label5: TLabel
          Left = 328
          Top = 89
          Width = 48
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #916#927#933
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label6: TLabel
          Left = 17
          Top = 89
          Width = 89
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #916#953#949#952#965#957#963#951
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label7: TLabel
          Left = 17
          Top = 143
          Width = 89
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #928#972#955#951
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label8: TLabel
          Left = 263
          Top = 143
          Width = 75
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #932#945#967'. '#922#969#948#953#953#954#945#962
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label9: TLabel
          Left = 16
          Top = 195
          Width = 89
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #931#965#957#948#949#948#949#956#941#957#959
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentFont = False
          Transparent = True
        end
        object lblInvoiceThird: TJvLabel
          Left = 295
          Top = 21
          Width = 175
          Height = 19
          Caption = #928#945#961#945#963#964#945#964#953#954#940' '#963#949' '#964#961#943#964#959#965#962
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -16
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
          Visible = False
          HotTrackFont.Charset = DEFAULT_CHARSET
          HotTrackFont.Color = clWindowText
          HotTrackFont.Height = -16
          HotTrackFont.Name = 'Tahoma'
          HotTrackFont.Style = []
        end
        object cxDBTextEdit1: TcxDBTextEdit
          Left = 111
          Top = 11
          DataBinding.DataField = 'firstname'
          DataBinding.DataSource = DSInvoices
          TabOrder = 0
          Width = 146
        end
        object cxDBTextEdit2: TcxDBTextEdit
          Left = 111
          Top = 36
          DataBinding.DataField = 'lastname'
          DataBinding.DataSource = DSInvoices
          TabOrder = 1
          Width = 146
        end
        object cxDBTextEdit3: TcxDBTextEdit
          Left = 111
          Top = 59
          DataBinding.DataField = 'companyname'
          DataBinding.DataSource = DSInvoices
          TabOrder = 2
          Width = 210
        end
        object cxDBTextEdit4: TcxDBTextEdit
          Left = 382
          Top = 59
          DataBinding.DataField = 'vatno'
          DataBinding.DataSource = DSInvoices
          TabOrder = 3
          Width = 99
        end
        object cxDBTextEdit5: TcxDBTextEdit
          Left = 382
          Top = 86
          DataBinding.DataField = 'taxoffice'
          DataBinding.DataSource = DSInvoices
          TabOrder = 4
          Width = 115
        end
        object cxDBTextEdit6: TcxDBTextEdit
          Left = 111
          Top = 86
          DataBinding.DataField = 'address1'
          DataBinding.DataSource = DSInvoices
          TabOrder = 5
          Width = 211
        end
        object cxDBTextEdit7: TcxDBTextEdit
          Left = 111
          Top = 113
          DataBinding.DataField = 'address2'
          DataBinding.DataSource = DSInvoices
          TabOrder = 6
          Width = 210
        end
        object cxDBTextEdit8: TcxDBTextEdit
          Left = 111
          Top = 140
          DataBinding.DataField = 'city'
          DataBinding.DataSource = DSInvoices
          TabOrder = 7
          Width = 146
        end
        object cxDBTextEdit9: TcxDBTextEdit
          Left = 344
          Top = 140
          DataBinding.DataField = 'postcode'
          DataBinding.DataSource = DSInvoices
          TabOrder = 8
          Width = 73
        end
        object cxDBTextEdit10: TcxDBTextEdit
          Left = 111
          Top = 192
          DataBinding.DataField = 'NAME'
          DataBinding.DataSource = DSLocalCustomer
          TabOrder = 9
          Width = 236
        end
        object cmdSuppress: TButton
          Left = 504
          Top = 19
          Width = 129
          Height = 21
          Caption = 'Suppress'
          TabOrder = 10
          OnClick = cmdSuppressClick
        end
        object cmdInvoice: TButton
          Left = 504
          Top = 46
          Width = 129
          Height = 21
          Caption = #904#954#948#959#963#951' '#949#960#953#955#949#947#956#941#957#969#957
          TabOrder = 11
          OnClick = cmdInvoiceClick
        end
        object Button2: TButton
          Left = 503
          Top = 73
          Width = 130
          Height = 21
          Caption = 'Mark UnInvoiced'
          TabOrder = 12
          OnClick = Button2Click
        end
        object cmdCustomerAdd: TButton
          Left = 353
          Top = 192
          Width = 75
          Height = 21
          Caption = #916#951#956#953#959#965#961#947#943#945
          TabOrder = 13
          OnClick = cmdCustomerAddClick
        end
        object btnShowCust: TButton
          Left = 434
          Top = 192
          Width = 75
          Height = 21
          Caption = #928#961#959#946#959#955#942
          TabOrder = 14
          OnClick = btnShowCustClick
        end
        object Button4: TButton
          Left = 503
          Top = 132
          Width = 129
          Height = 21
          Hint = #904#954#948#959#963#951' '#972#955#969#957' '#947#953#945' '#964#959#957' '#963#965#947#954#949#954#961#953#956#941#957#959' '#960#949#955#940#964#951
          Caption = #904#954#948#959#963#951' '#972#955#969#957' '#945#957#945' '#960#949#955#940#964#951
          TabOrder = 15
          OnClick = Button4Click
        end
        object checkEmail: TcxCheckBox
          Left = 515
          Top = 100
          Caption = 'Email'
          ParentFont = False
          Style.Font.Charset = DEFAULT_CHARSET
          Style.Font.Color = clWindowText
          Style.Font.Height = -15
          Style.Font.Name = 'Tahoma'
          Style.Font.Style = []
          Style.IsFontAssigned = True
          TabOrder = 16
          Transparent = True
        end
        object Button1: TButton
          Left = 656
          Top = 19
          Width = 129
          Height = 21
          Caption = 'Auto Invoicing'
          TabOrder = 17
          OnClick = Button1Click
        end
      end
      object TabSheet2: TTabSheet
        Caption = #915#961#945#956#956#941#962' '#960#945#961'/'#954#959#973
        ImageIndex = 1
        object GridCSInvoiceLines: TcxGrid
          Left = 0
          Top = 0
          Width = 617
          Height = 228
          Align = alLeft
          BorderStyle = cxcbsNone
          TabOrder = 0
          DragOpening = False
          LookAndFeel.Kind = lfStandard
          LookAndFeel.NativeStyle = True
          object ViewInvoiceLines: TcxGridDBTableView
            Navigator.Buttons.CustomButtons = <>
            Navigator.Buttons.First.Visible = False
            Navigator.Buttons.PriorPage.Hint = #928#961#959#951#947#959#973#956#949#957#951' '#963#949#955#943#948#945
            Navigator.Buttons.Prior.Enabled = False
            Navigator.Buttons.Prior.Visible = False
            Navigator.Buttons.Next.Enabled = False
            Navigator.Buttons.Next.Visible = False
            Navigator.Buttons.NextPage.Hint = #917#960#972#956#949#957#951' '#963#949#955#943#948#945
            Navigator.Buttons.Last.Visible = False
            Navigator.Buttons.Insert.Enabled = False
            Navigator.Buttons.Insert.Visible = False
            Navigator.Buttons.Append.Enabled = False
            Navigator.Buttons.Append.Visible = False
            Navigator.Buttons.Delete.Enabled = False
            Navigator.Buttons.Delete.Visible = False
            Navigator.Buttons.Edit.Enabled = False
            Navigator.Buttons.Edit.Visible = False
            Navigator.Buttons.Post.Visible = False
            Navigator.Buttons.Cancel.Enabled = False
            Navigator.Buttons.Cancel.Visible = False
            Navigator.Buttons.Refresh.Visible = False
            Navigator.Buttons.Filter.Hint = #934#953#955#964#961#940#961#953#963#956#945' '#948#949#948#959#956#941#957#969#957
            Navigator.Visible = True
            DataController.DataModeController.SmartRefresh = True
            DataController.DataSource = DSInvLines
            DataController.Filter.AutoDataSetFilter = True
            DataController.Summary.DefaultGroupSummaryItems = <
              item
                Kind = skSum
              end>
            DataController.Summary.FooterSummaryItems = <>
            DataController.Summary.SummaryGroups = <>
            NewItemRow.InfoText = #917#953#963#945#947#969#947#942
            OptionsBehavior.NavigatorHints = True
            OptionsCustomize.ColumnHiding = True
            OptionsCustomize.ColumnsQuickCustomization = True
            OptionsCustomize.ColumnsQuickCustomizationReordering = qcrEnabled
            OptionsData.Appending = True
            OptionsSelection.CellSelect = False
            OptionsView.FocusRect = False
            OptionsView.NavigatorOffset = 10
            OptionsView.NoDataToDisplayInfoText = '<'#916#949#957' '#965#960#940#961#967#959#965#957' '#948#949#948#959#956#941#957#945'>'
            OptionsView.ScrollBars = ssVertical
            OptionsView.GridLines = glVertical
            OptionsView.GroupByBox = False
            OptionsView.HeaderEndEllipsis = True
            OptionsView.Indicator = True
            Styles.Background = StyleMain
            Styles.ContentEven = StyleEven
            Styles.ContentOdd = StyleOdd
            object ViewInvoiceLinestype: TcxGridDBColumn
              DataBinding.FieldName = 'type'
            end
            object ViewInvoiceLinesrelid: TcxGridDBColumn
              DataBinding.FieldName = 'relid'
              Visible = False
              Width = 45
            end
            object ViewInvoiceLinesdescription: TcxGridDBColumn
              DataBinding.FieldName = 'description'
              Width = 144
            end
            object ViewInvoiceLinesamount: TcxGridDBColumn
              DataBinding.FieldName = 'amount'
            end
            object ViewInvoiceLinesalt_company_name: TcxGridDBColumn
              Caption = 'Alt. Company Name'
              DataBinding.FieldName = 'alt_company_name'
              Width = 200
            end
          end
          object Level1InvoiceLines: TcxGridLevel
            GridView = ViewInvoiceLines
          end
        end
      end
      object TabSheet3: TTabSheet
        Caption = #928#945#961#945#963#964#945#964#953#954#940' '#963#949' '#964#961#943#964#959#965#962
        ImageIndex = 2
        object Label10: TLabel
          Left = 0
          Top = 15
          Width = 89
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #917#960#969#957#965#956#943#945
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label11: TLabel
          Left = 41
          Top = 42
          Width = 48
          Height = 14
          Cursor = crArrow
          Alignment = taRightJustify
          AutoSize = False
          Caption = #913#934#924
          Font.Charset = GREEK_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object cxDBTextEdit11: TcxDBTextEdit
          Left = 95
          Top = 11
          DataBinding.DataField = 'companyname'
          DataBinding.DataSource = DSThirdInvoice
          TabOrder = 0
          Width = 466
        end
        object cxDBTextEdit12: TcxDBTextEdit
          Left = 95
          Top = 38
          DataBinding.DataField = 'vatno'
          DataBinding.DataSource = DSThirdInvoice
          TabOrder = 1
          Width = 99
        end
      end
    end
  end
  object PanelTop: TJvPanel
    Left = 0
    Top = 0
    Width = 938
    Height = 65
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 2
    object JvPanel1: TJvPanel
      Left = 900
      Top = 0
      Width = 38
      Height = 65
      FlatBorder = True
      Align = alRight
      TabOrder = 0
      object JvToolBar1: TJvToolBar
        Left = 1
        Top = 1
        Width = 36
        Height = 33
        ButtonHeight = 30
        ButtonWidth = 32
        Caption = 'JvToolBar1'
        EdgeInner = esNone
        Images = ImageList1
        ParentShowHint = False
        ShowHint = False
        TabOrder = 0
        Transparent = True
        object ToolRefresh1: TToolButton
          Left = 0
          Top = 0
          Caption = 'ToolRefresh1'
          ImageIndex = 7
          OnClick = ToolRefresh1Click
        end
      end
    end
    object radioUnInvoiced: TJvRadioButton
      Left = 33
      Top = 9
      Width = 75
      Height = 17
      Alignment = taLeftJustify
      Caption = 'UnInvoiced'
      Checked = True
      TabOrder = 1
      TabStop = True
      OnClick = radioUnInvoicedClick
      LinkedControls = <>
    end
    object radioSuppressed: TJvRadioButton
      Left = 111
      Top = 9
      Width = 77
      Height = 17
      Alignment = taLeftJustify
      Caption = 'Suppressed'
      TabOrder = 2
      OnClick = radioSuppressedClick
      LinkedControls = <>
    end
    object JvRadioButton1: TJvRadioButton
      Left = 316
      Top = 9
      Width = 62
      Height = 17
      Alignment = taLeftJustify
      Caption = 'Invoiced'
      TabOrder = 3
      OnClick = JvRadioButton1Click
      LinkedControls = <>
    end
    object cmdSearchInvoice: TButton
      Left = 515
      Top = 7
      Width = 69
      Height = 21
      Caption = 'Search'
      Enabled = False
      TabOrder = 4
      OnClick = cmdSearchInvoiceClick
    end
    object cxCalcEdit1: TcxCalcEdit
      Left = 439
      Top = 7
      EditValue = 0.000000000000000000
      Enabled = False
      TabOrder = 5
      OnKeyDown = cxCalcEdit1KeyDown
      Width = 70
    end
    object JvRadioButton2: TJvRadioButton
      Left = 382
      Top = 9
      Width = 57
      Height = 17
      Alignment = taLeftJustify
      Caption = 'Custom'
      TabOrder = 6
      OnClick = JvRadioButton2Click
      LinkedControls = <>
    end
    object EditFilter: TcxTextEdit
      Left = 5
      Top = 35
      ParentFont = False
      Properties.OnChange = EditFilterPropertiesChange
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.IsFontAssigned = True
      TabOrder = 7
      Width = 517
    end
    object JvRadioButton3: TJvRadioButton
      Left = 203
      Top = 9
      Width = 98
      Height = 17
      Alignment = taLeftJustify
      Caption = 'Auto-Supressed'
      TabOrder = 8
      OnClick = JvRadioButton3Click
      LinkedControls = <>
    end
  end
  object DSInvoices: TDataSource
    AutoEdit = False
    DataSet = QueryInvoices
    Left = 296
    Top = 72
  end
  object StyleRepo: TcxStyleRepository
    PixelsPerInch = 96
    object StyleMain: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = clCream
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      TextColor = clGradientActiveCaption
    end
    object StyleEven: TcxStyle
      AssignedValues = [svColor]
      Color = clDefault
    end
    object StyleOdd: TcxStyle
      AssignedValues = [svColor]
      Color = clDefault
    end
    object StyleGroupBox: TcxStyle
    end
    object cxStyle1: TcxStyle
      AssignedValues = [svTextColor]
      TextColor = clRed
    end
  end
  object QueryLocalCustomer: TIBQuery
    Database = FrmMain.database
    Transaction = FrmMain.IBTransaction1
    AfterOpen = QueryLocalCustomerAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT CUSTOMER.* FROM CUSTOMER'
      'WHERE AFM = :AFM')
    Left = 256
    Top = 328
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'AFM'
        ParamType = ptUnknown
      end>
    object QueryLocalCustomerCUST_ID: TIntegerField
      FieldName = 'CUST_ID'
      Origin = '"CUSTOMER"."CUST_ID"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QueryLocalCustomerALT_CUSTID: TIntegerField
      FieldName = 'ALT_CUSTID'
      Origin = '"CUSTOMER"."ALT_CUSTID"'
    end
    object QueryLocalCustomerAFM: TIBStringField
      FieldName = 'AFM'
      Origin = '"CUSTOMER"."AFM"'
    end
    object QueryLocalCustomerNAME: TIBStringField
      FieldName = 'NAME'
      Origin = '"CUSTOMER"."NAME"'
      Required = True
      Size = 50
    end
    object QueryLocalCustomerADDRESS1: TIBStringField
      FieldName = 'ADDRESS1'
      Origin = '"CUSTOMER"."ADDRESS1"'
      Size = 50
    end
    object QueryLocalCustomerADDRESS2: TIBStringField
      FieldName = 'ADDRESS2'
      Origin = '"CUSTOMER"."ADDRESS2"'
      Size = 50
    end
    object QueryLocalCustomerCITY: TIBStringField
      FieldName = 'CITY'
      Origin = '"CUSTOMER"."CITY"'
      Size = 30
    end
    object QueryLocalCustomerPOSTCODE: TIBStringField
      FieldName = 'POSTCODE'
      Origin = '"CUSTOMER"."POSTCODE"'
      Size = 10
    end
    object QueryLocalCustomerPHONE1: TIBStringField
      FieldName = 'PHONE1'
      Origin = '"CUSTOMER"."PHONE1"'
      Size = 30
    end
    object QueryLocalCustomerPHONE2: TIBStringField
      FieldName = 'PHONE2'
      Origin = '"CUSTOMER"."PHONE2"'
      Size = 30
    end
    object QueryLocalCustomerFAX: TIBStringField
      FieldName = 'FAX'
      Origin = '"CUSTOMER"."FAX"'
      Size = 30
    end
    object QueryLocalCustomerOCCUPATION: TIBStringField
      FieldName = 'OCCUPATION'
      Origin = '"CUSTOMER"."OCCUPATION"'
      Size = 120
    end
    object QueryLocalCustomerTAXOFFICE: TIBStringField
      FieldName = 'TAXOFFICE'
      Origin = '"CUSTOMER"."TAXOFFICE"'
      Size = 30
    end
    object QueryLocalCustomerDETAILS: TWideMemoField
      FieldName = 'DETAILS'
      Origin = '"CUSTOMER"."DETAILS"'
      ProviderFlags = [pfInUpdate]
      BlobType = ftWideMemo
      Size = 8
    end
    object QueryLocalCustomerDISCOUNT: TIBBCDField
      FieldName = 'DISCOUNT'
      Origin = '"CUSTOMER"."DISCOUNT"'
      Precision = 18
      Size = 2
    end
    object QueryLocalCustomerSECONDARY_EMAIL: TIBStringField
      FieldName = 'SECONDARY_EMAIL'
      Origin = '"CUSTOMER"."SECONDARY_EMAIL"'
      Size = 120
    end
    object QueryLocalCustomerEMAIL: TIBStringField
      FieldName = 'EMAIL'
      Origin = '"CUSTOMER"."EMAIL"'
      Size = 120
    end
    object QueryLocalCustomerORDER: TIntegerField
      FieldName = 'ORDER'
      Origin = '"CUSTOMER"."ORDER"'
    end
    object QueryLocalCustomerCOUNTRY: TIBStringField
      FieldName = 'COUNTRY'
      Origin = '"CUSTOMER"."COUNTRY"'
      Size = 40
    end
    object QueryLocalCustomerPAYMETH_ID: TIntegerField
      FieldName = 'PAYMETH_ID'
      Origin = '"CUSTOMER"."PAYMETH_ID"'
    end
    object QueryLocalCustomerVAT_VIES: TIBStringField
      FieldName = 'VAT_VIES'
      Origin = '"CUSTOMER"."VAT_VIES"'
      Size = 30
    end
  end
  object DSLocalCustomer: TDataSource
    AutoEdit = False
    DataSet = QueryLocalCustomer
    Left = 288
    Top = 328
  end
  object DSInvLines: TDataSource
    AutoEdit = False
    DataSet = QueryInvLines
    Left = 256
    Top = 208
  end
  object ImageList1: TImageList
    Height = 24
    Width = 24
    Left = 368
    Top = 152
    Bitmap = {
      
      000000000000}
  end
  object sqlConnection: TUniConnection
    ProviderName = 'MySQL'
    Database = 'myipgr_clients'
    SpecificOptions.Strings = (
      'MySQL.Charset=UTF-8'
      'MySQL.UseUnicode=True')
    Username = 'myipgr_ekdosi'
    Server = 'sky.myip.gr'
    Connected = True
    LoginPrompt = False
    Left = 80
    Top = 144
    EncryptedPassword = '9DFF99FFC8FFB1FFB1FFABFFAAFFA9FF8FFFC8FF8AFFACFF'
  end
  object QueryInvoices: TUniQuery
    Connection = sqlConnection
    SQL.Strings = (
      'select mi.*,'
      '      b.companyname,'
      '      b.address1,'
      '      b.address2,'
      '      b.city,'
      '      b.state,'
      '      b.postcode,'
      '      b.country,'
      '      b.phonenumber,'
      '      b.email,'
      '      b.firstname,'
      '      b.lastname,'
      '      (select value from tblcustomfieldsvalues'
      
        '          where relid = b.id and fieldid = 12 limit 1) as toinvo' +
        'ice,'
      '        (select value from tblcustomfieldsvalues'
      
        '          where relid = b.id and fieldid = 15 limit 1) as occupa' +
        'tion,'
      '        (select value from tblcustomfieldsvalues'
      '          where relid = b.id and fieldid = 13 limit 1) as vatno,'
      '       (select value  from tblcustomfieldsvalues'
      
        '          where relid = b.id and fieldid = 14 limit 1) as taxoff' +
        'ice,'
      '        G.gkriniaris, '
      '      (select DISTINCT CAST(1 AS UNSIGNED)'
      
        #9#9#9'from tblinvoiceitems i left outer join  mod_timologia_service' +
        'types s   on i.type = s.invline_servicetype'
      
        #9#9#9'left outer join mod_timologia t on i.relid = t.serviceid and ' +
        's.contact_servicetype = t.service_type'
      
        #9#9#9'where i.invoiceid = mi.id AND t.contactid is NOT null) AS HAS' +
        '_3RD_INVOICE '
      
        '        from tblinvoices mi left outer join tblclients b on mi.u' +
        'serid = b.id '
      
        '         left outer join (select relid, value as gkriniaris  fro' +
        'm tblcustomfieldsvalues where fieldid = 338) G on relid = b.id'
      '      where mi.status ='#39'Paid'#39'  and mi.invoiced = 0 '
      '    order by mi.datepaid asc')
    AfterScroll = QueryInvoicesAfterScroll
    Left = 256
    Top = 72
    object QueryInvoicesid: TLongWordField
      AutoGenerateValue = arAutoInc
      FieldName = 'id'
    end
    object QueryInvoicesuserid: TIntegerField
      FieldName = 'userid'
      Required = True
    end
    object QueryInvoicesinvoicenum: TWideMemoField
      FieldName = 'invoicenum'
      Required = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesdate: TDateField
      FieldName = 'date'
    end
    object QueryInvoicesduedate: TDateField
      FieldName = 'duedate'
    end
    object QueryInvoicesdatepaid: TDateTimeField
      FieldName = 'datepaid'
      Required = True
      DisplayFormat = 'dd/mm/yyyy HH:mm'
    end
    object QueryInvoiceslast_capture_attempt: TDateTimeField
      FieldName = 'last_capture_attempt'
    end
    object QueryInvoicesdate_refunded: TDateTimeField
      FieldName = 'date_refunded'
    end
    object QueryInvoicesdate_cancelled: TDateTimeField
      FieldName = 'date_cancelled'
    end
    object QueryInvoicessubtotal: TFloatField
      FieldName = 'subtotal'
      Required = True
    end
    object QueryInvoicescredit: TFloatField
      FieldName = 'credit'
      Required = True
    end
    object QueryInvoicestax: TFloatField
      FieldName = 'tax'
      Required = True
    end
    object QueryInvoicestax2: TFloatField
      FieldName = 'tax2'
      Required = True
    end
    object QueryInvoicestotal: TFloatField
      FieldName = 'total'
      Required = True
      currency = True
    end
    object QueryInvoicestaxrate: TFloatField
      FieldName = 'taxrate'
      Required = True
    end
    object QueryInvoicestaxrate2: TFloatField
      FieldName = 'taxrate2'
      Required = True
    end
    object QueryInvoicesstatus: TWideMemoField
      FieldName = 'status'
      Required = True
      BlobType = ftWideMemo
    end
    object QueryInvoicespaymentmethod: TWideMemoField
      FieldName = 'paymentmethod'
      Required = True
      BlobType = ftWideMemo
    end
    object QueryInvoicespaymethodid: TLongWordField
      FieldName = 'paymethodid'
    end
    object QueryInvoicesnotes: TWideMemoField
      FieldName = 'notes'
      Required = True
      BlobType = ftWideMemo
    end
    object QueryInvoicescreated_at: TDateTimeField
      FieldName = 'created_at'
    end
    object QueryInvoicesupdated_at: TDateTimeField
      FieldName = 'updated_at'
    end
    object QueryInvoicesinvoiced: TSmallintField
      FieldName = 'invoiced'
    end
    object QueryInvoicescompanyname: TWideMemoField
      FieldName = 'companyname'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesaddress1: TWideMemoField
      FieldName = 'address1'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesaddress2: TWideMemoField
      FieldName = 'address2'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicescity: TWideMemoField
      FieldName = 'city'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesstate: TWideMemoField
      FieldName = 'state'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicespostcode: TWideMemoField
      FieldName = 'postcode'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicescountry: TWideMemoField
      FieldName = 'country'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesphonenumber: TWideMemoField
      FieldName = 'phonenumber'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesemail: TWideMemoField
      FieldName = 'email'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesfirstname: TWideMemoField
      FieldName = 'firstname'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoiceslastname: TWideMemoField
      FieldName = 'lastname'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicestoinvoice: TWideMemoField
      FieldName = 'toinvoice'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesoccupation: TWideMemoField
      FieldName = 'occupation'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesvatno: TWideMemoField
      FieldName = 'vatno'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicestaxoffice: TWideMemoField
      FieldName = 'taxoffice'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesgkriniaris: TWideMemoField
      FieldName = 'gkriniaris'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object QueryInvoicesHAS_3RD_INVOICE: TLongWordField
      FieldName = 'HAS_3RD_INVOICE'
      ReadOnly = True
    end
  end
  object MySQLUniProvider1: TMySQLUniProvider
    Left = 80
    Top = 192
  end
  object DatasetInvoices: TUniQuery
    Connection = sqlConnection
    SQL.Strings = (
      'select mi.*,'
      '      b.companyname,'
      '      b.address1,'
      '      b.address2,'
      '      b.city,'
      '      b.state,'
      '      b.postcode,'
      '      b.country,'
      '      b.phonenumber,'
      '      b.email,'
      '      b.firstname,'
      '      b.lastname,'
      '      (select value from tblcustomfieldsvalues'
      
        '          where relid = b.id and fieldid = 12 limit 1) as toinvo' +
        'ice,'
      '        (select value from tblcustomfieldsvalues'
      
        '          where relid = b.id and fieldid = 15 limit 1) as occupa' +
        'tion,'
      '        (select value from tblcustomfieldsvalues'
      '          where relid = b.id and fieldid = 13 limit 1) as vatno,'
      '       (select value  from tblcustomfieldsvalues'
      
        '          where relid = b.id and fieldid = 14 limit 1) as taxoff' +
        'ice,'
      '      (select DISTINCT 1 '
      
        'from tblinvoiceitems i left outer join  mod_timologia_servicetyp' +
        'es s   on i.type = s.invline_servicetype'
      
        'left outer join mod_timologia t on i.relid = t.serviceid and s.c' +
        'ontact_servicetype = t.service_type'
      
        'where i.invoiceid = mi.id AND t.contactid is NOT null) AS HAS_3R' +
        'D_INVOICE '
      
        '        from tblinvoices mi left outer join tblclients b on mi.u' +
        'serid = b.id'#9#9#9#9
      '      where mi.status ='#39'Paid'#39'  and mi.invoiced = 0'
      '    order by mi.date desc')
    Left = 216
    Top = 136
    object DatasetInvoicesid: TIntegerField
      FieldName = 'id'
    end
    object DatasetInvoicesuserid: TIntegerField
      FieldName = 'userid'
      Required = True
    end
    object DatasetInvoicesinvoicenum: TWideMemoField
      FieldName = 'invoicenum'
      Required = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicesdate: TDateField
      FieldName = 'date'
    end
    object DatasetInvoicesduedate: TDateField
      FieldName = 'duedate'
    end
    object DatasetInvoicesdatepaid: TDateTimeField
      FieldName = 'datepaid'
      Required = True
    end
    object DatasetInvoiceslast_capture_attempt: TDateTimeField
      FieldName = 'last_capture_attempt'
    end
    object DatasetInvoicesdate_refunded: TDateTimeField
      FieldName = 'date_refunded'
    end
    object DatasetInvoicesdate_cancelled: TDateTimeField
      FieldName = 'date_cancelled'
    end
    object DatasetInvoicessubtotal: TFloatField
      FieldName = 'subtotal'
      Required = True
    end
    object DatasetInvoicescredit: TFloatField
      FieldName = 'credit'
      Required = True
    end
    object DatasetInvoicestax: TFloatField
      FieldName = 'tax'
      Required = True
    end
    object DatasetInvoicestax2: TFloatField
      FieldName = 'tax2'
      Required = True
    end
    object DatasetInvoicestotal: TFloatField
      FieldName = 'total'
      Required = True
    end
    object DatasetInvoicestaxrate: TFloatField
      FieldName = 'taxrate'
      Required = True
    end
    object DatasetInvoicestaxrate2: TFloatField
      FieldName = 'taxrate2'
      Required = True
    end
    object DatasetInvoicesstatus: TWideMemoField
      FieldName = 'status'
      Required = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicespaymentmethod: TWideMemoField
      FieldName = 'paymentmethod'
      Required = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicespaymethodid: TLongWordField
      FieldName = 'paymethodid'
    end
    object DatasetInvoicesnotes: TWideMemoField
      FieldName = 'notes'
      Required = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicescreated_at: TDateTimeField
      FieldName = 'created_at'
    end
    object DatasetInvoicesupdated_at: TDateTimeField
      FieldName = 'updated_at'
    end
    object DatasetInvoicesinvoiced: TSmallintField
      FieldName = 'invoiced'
    end
    object DatasetInvoicescompanyname: TWideMemoField
      FieldName = 'companyname'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicesaddress1: TWideMemoField
      FieldName = 'address1'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicesaddress2: TWideMemoField
      FieldName = 'address2'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicescity: TWideMemoField
      FieldName = 'city'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicesstate: TWideMemoField
      FieldName = 'state'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicespostcode: TWideMemoField
      FieldName = 'postcode'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicescountry: TWideMemoField
      FieldName = 'country'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicesphonenumber: TWideMemoField
      FieldName = 'phonenumber'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicesemail: TWideMemoField
      FieldName = 'email'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicesfirstname: TWideMemoField
      FieldName = 'firstname'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoiceslastname: TWideMemoField
      FieldName = 'lastname'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicestoinvoice: TWideMemoField
      FieldName = 'toinvoice'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicesoccupation: TWideMemoField
      FieldName = 'occupation'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicesvatno: TWideMemoField
      FieldName = 'vatno'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicestaxoffice: TWideMemoField
      FieldName = 'taxoffice'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object DatasetInvoicesHAS_3RD_INVOICE: TLargeintField
      FieldName = 'HAS_3RD_INVOICE'
      ReadOnly = True
    end
  end
  object QueryInvLines: TUniQuery
    Connection = sqlConnection
    SQL.Strings = (
      'SELECT I.*,'
      
        '     (SELECT C.company_name FROM `mod_timologia` AS T LEFT OUTER' +
        ' JOIN `mod_timologia_contacts` AS C '
      #9#9#9'ON T.contactid = C.id'
      
        '         WHERE serviceid = I.relid and service_type = I.type) AS' +
        ' alt_company_name'
      'FROM `tblinvoiceitems` AS I'
      'WHERE invoiceid = :id')
    MasterSource = DSInvoices
    Left = 192
    Top = 208
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'id'
        Value = nil
      end>
    object QueryInvLinesid: TIntegerField
      AutoGenerateValue = arAutoInc
      FieldName = 'id'
    end
    object QueryInvLinesinvoiceid: TLongWordField
      FieldName = 'invoiceid'
      Required = True
    end
    object QueryInvLinesuserid: TIntegerField
      FieldName = 'userid'
      Required = True
    end
    object QueryInvLinestype: TWideStringField
      FieldName = 'type'
      Required = True
      Size = 30
    end
    object QueryInvLinesrelid: TIntegerField
      FieldName = 'relid'
      Required = True
    end
    object QueryInvLinesdescription: TWideMemoField
      FieldName = 'description'
      Required = True
      BlobType = ftWideMemo
    end
    object QueryInvLinesamount: TFloatField
      FieldName = 'amount'
      Required = True
    end
    object QueryInvLinestaxed: TIntegerField
      FieldName = 'taxed'
      Required = True
    end
    object QueryInvLinesduedate: TDateField
      FieldName = 'duedate'
    end
    object QueryInvLinespaymentmethod: TWideMemoField
      FieldName = 'paymentmethod'
      Required = True
      BlobType = ftWideMemo
    end
    object QueryInvLinesnotes: TWideMemoField
      FieldName = 'notes'
      Required = True
      BlobType = ftWideMemo
    end
    object QueryInvLinesalt_company_name: TWideStringField
      FieldName = 'alt_company_name'
      ReadOnly = True
      Size = 128
    end
  end
  object QueryThirdInvoice: TUniQuery
    Connection = sqlConnection
    SQL.Strings = (
      'select  t.contactid,'
      '      c.id,'
      '      c.userid,'
      '      c.company_name as companyname,'
      '      c.address1,'
      '      c.address2,'
      '      c.postal_code as postcode,'
      '      c.city,'
      '      c.telephone as phonenumber,'
      '      c.country,'
      '      c.email,'
      '      c.tax_office as taxoffice,'
      '      c.description as occupation,'
      '      c.gr_vatno as vatno,'
      '      c.company_name as lastname, '#39#39' as firstname, '
      '      t.isReceipt,'
      '      count(*)'
      
        'from `tblinvoiceitems` i left outer join  mod_timologia_servicet' +
        'ypes s on i.type = s.invline_servicetype'
      
        ' right outer join (`mod_timologia` t left outer join `mod_timolo' +
        'gia_contacts` as c on t.contactid = c.id)'
      
        #9#9#9#9#9'on i.relid = t.serviceid and t.service_type = s.contact_ser' +
        'vicetype'
      '                    '
      ' where i.invoiceid = :invoiceid'
      ' group by t.contactid'
      ' order by count(*) desc')
    Left = 360
    Top = 376
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'invoiceid'
        Value = nil
      end>
    object QueryThirdInvoicecontactid: TIntegerField
      FieldName = 'contactid'
      Required = True
    end
    object QueryThirdInvoiceid: TIntegerField
      AutoGenerateValue = arAutoInc
      FieldName = 'id'
      ReadOnly = True
    end
    object QueryThirdInvoiceuserid: TIntegerField
      FieldName = 'userid'
      ReadOnly = True
    end
    object QueryThirdInvoicecompanyname: TWideStringField
      FieldName = 'companyname'
      ReadOnly = True
      Size = 128
    end
    object QueryThirdInvoiceaddress1: TWideStringField
      FieldName = 'address1'
      ReadOnly = True
      Size = 60
    end
    object QueryThirdInvoiceaddress2: TWideStringField
      FieldName = 'address2'
      ReadOnly = True
      Size = 60
    end
    object QueryThirdInvoicepostcode: TWideStringField
      FieldName = 'postcode'
      ReadOnly = True
      Size = 10
    end
    object QueryThirdInvoicecity: TWideStringField
      FieldName = 'city'
      ReadOnly = True
      Size = 60
    end
    object QueryThirdInvoicephonenumber: TWideStringField
      FieldName = 'phonenumber'
      ReadOnly = True
    end
    object QueryThirdInvoicecountry: TWideStringField
      FieldName = 'country'
      ReadOnly = True
      Size = 60
    end
    object QueryThirdInvoiceemail: TWideStringField
      FieldName = 'email'
      ReadOnly = True
      Size = 60
    end
    object QueryThirdInvoicetaxoffice: TWideStringField
      FieldName = 'taxoffice'
      ReadOnly = True
      Size = 60
    end
    object QueryThirdInvoiceoccupation: TWideStringField
      FieldName = 'occupation'
      ReadOnly = True
      Size = 120
    end
    object QueryThirdInvoicevatno: TWideStringField
      FieldName = 'vatno'
      ReadOnly = True
      Size = 30
    end
    object QueryThirdInvoicelastname: TWideStringField
      FieldName = 'lastname'
      ReadOnly = True
      Size = 128
    end
    object QueryThirdInvoicefirstname: TWideStringField
      FieldName = 'firstname'
      ReadOnly = True
      Required = True
      Size = 65530
    end
    object QueryThirdInvoicecount: TLargeintField
      FieldName = 'count(*)'
      ReadOnly = True
      Required = True
    end
    object QueryThirdInvoiceisReceipt: TBooleanField
      FieldName = 'isReceipt'
    end
  end
  object DSThirdInvoice: TDataSource
    AutoEdit = False
    DataSet = QueryThirdInvoice
    Left = 400
    Top = 376
  end
  object QueryAcceptedCustomer: TIBQuery
    Database = FrmMain.database
    Transaction = FrmMain.IBTransaction1
    AfterOpen = QueryLocalCustomerAfterOpen
    BufferChunks = 1000
    CachedUpdates = False
    DataSource = DSLocalCustomer
    ParamCheck = True
    SQL.Strings = (
      'SELECT * FROM CUSTOMER_CS_ACCEPTED'
      'WHERE CUST_ID = :CUST_ID')
    Left = 496
    Top = 312
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'CUST_ID'
        ParamType = ptUnknown
      end>
    object QueryAcceptedCustomerCUST_ID: TIntegerField
      FieldName = 'CUST_ID'
      Origin = '"CUSTOMER_CS_ACCEPTED"."CUST_ID"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QueryAcceptedCustomerAFM: TIBStringField
      FieldName = 'AFM'
      Origin = '"CUSTOMER_CS_ACCEPTED"."AFM"'
    end
    object QueryAcceptedCustomerNAME: TIBStringField
      FieldName = 'NAME'
      Origin = '"CUSTOMER_CS_ACCEPTED"."NAME"'
      Required = True
      Size = 50
    end
    object QueryAcceptedCustomerADDRESS1: TIBStringField
      FieldName = 'ADDRESS1'
      Origin = '"CUSTOMER_CS_ACCEPTED"."ADDRESS1"'
      Size = 50
    end
    object QueryAcceptedCustomerADDRESS2: TIBStringField
      FieldName = 'ADDRESS2'
      Origin = '"CUSTOMER_CS_ACCEPTED"."ADDRESS2"'
      Size = 50
    end
    object QueryAcceptedCustomerCITY: TIBStringField
      FieldName = 'CITY'
      Origin = '"CUSTOMER_CS_ACCEPTED"."CITY"'
      Size = 30
    end
    object QueryAcceptedCustomerPOSTCODE: TIBStringField
      FieldName = 'POSTCODE'
      Origin = '"CUSTOMER_CS_ACCEPTED"."POSTCODE"'
      Size = 10
    end
    object QueryAcceptedCustomerPHONE1: TIBStringField
      FieldName = 'PHONE1'
      Origin = '"CUSTOMER_CS_ACCEPTED"."PHONE1"'
      Size = 30
    end
    object QueryAcceptedCustomerPHONE2: TIBStringField
      FieldName = 'PHONE2'
      Origin = '"CUSTOMER_CS_ACCEPTED"."PHONE2"'
      Size = 30
    end
    object QueryAcceptedCustomerFAX: TIBStringField
      FieldName = 'FAX'
      Origin = '"CUSTOMER_CS_ACCEPTED"."FAX"'
      Size = 30
    end
    object QueryAcceptedCustomerOCCUPATION: TIBStringField
      FieldName = 'OCCUPATION'
      Origin = '"CUSTOMER_CS_ACCEPTED"."OCCUPATION"'
      Size = 120
    end
    object QueryAcceptedCustomerTAXOFFICE: TIBStringField
      FieldName = 'TAXOFFICE'
      Origin = '"CUSTOMER_CS_ACCEPTED"."TAXOFFICE"'
      Size = 30
    end
    object QueryAcceptedCustomerDETAILS: TWideMemoField
      FieldName = 'DETAILS'
      Origin = '"CUSTOMER_CS_ACCEPTED"."DETAILS"'
      ProviderFlags = [pfInUpdate]
      BlobType = ftWideMemo
      Size = 8
    end
    object QueryAcceptedCustomerDISCOUNT: TIBBCDField
      FieldName = 'DISCOUNT'
      Origin = '"CUSTOMER_CS_ACCEPTED"."DISCOUNT"'
      Precision = 18
      Size = 2
    end
    object QueryAcceptedCustomerEMAIL: TIBStringField
      FieldName = 'EMAIL'
      Origin = '"CUSTOMER_CS_ACCEPTED"."EMAIL"'
      Size = 120
    end
    object QueryAcceptedCustomerCOUNTRY: TIBStringField
      FieldName = 'COUNTRY'
      Origin = '"CUSTOMER_CS_ACCEPTED"."COUNTRY"'
      Size = 40
    end
    object QueryAcceptedCustomerALT_CUSTID: TIntegerField
      FieldName = 'ALT_CUSTID'
      Origin = '"CUSTOMER_CS_ACCEPTED"."ALT_CUSTID"'
    end
    object QueryAcceptedCustomerPAYMETH_ID: TIntegerField
      FieldName = 'PAYMETH_ID'
      Origin = '"CUSTOMER_CS_ACCEPTED"."PAYMETH_ID"'
    end
    object QueryAcceptedCustomerVAT_VIES: TIBStringField
      FieldName = 'VAT_VIES'
      Origin = '"CUSTOMER_CS_ACCEPTED"."VAT_VIES"'
      Size = 30
    end
    object QueryAcceptedCustomerSECONDARY_EMAIL: TIBStringField
      FieldName = 'SECONDARY_EMAIL'
      Origin = '"CUSTOMER_CS_ACCEPTED"."SECONDARY_EMAIL"'
      Size = 120
    end
  end
end
