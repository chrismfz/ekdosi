...
| ~~**MYD-006**~~ **DONE (#413, +#416 guide)** | Classification defaults had no per-business policy: goods defaulted to `category1_1` (εμπορεύματα), wrong for a manufacturer (own products = `category1_2`). **Shipped:** `companies.business_activity_type` + `ClassificationGuidance` + the go-live gate that FAILS until a policy is chosen + credit notes inherit the original's classification + on-form/list visibility + the «Οδηγός κωδικών myDATA» glossary. | **Code done.** Remaining is the ~30-min **config** step: each live tenant selects its activity type (the go-live gate now forces it). No feature left. |
...
> **Update (2026-09-02, later):** of that day-one list, **PROV-003 print (#406), OBS-001
> (#405), MYD-004 0% (#410), MYD-007 code (#410) and MYD-006 (#413 + the «Οδηγός κωδικών»
> #416) are SHIPPED.** What remains for cutover: the dry-run (in progress), the accountant's
> §8.3 confirmation for MYD-007, each live tenant selecting its `business_activity_type`
> (MYD-006 config — the go-live gate now forces it), and PROV-006 **only if** retail files
> via the provider on day one.
