<x-filament-panels::page>
    ...
    <x-filament::section>
        @if (count($r['rows']) === 0)
            <div class="text-center fi-color-gray py-6 text-sm">Δεν βρέθηκαν πωλήσεις ειδών για το {{ $r['year'] }}.</div>
        @else
                ...
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="text-left text-xs uppercase text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-white/10">
                            <th class="py-2 pr-4">Είδος / Υπηρεσία</th>
                            <th class="py-2 px-3 text-right">Ποσότητα</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-white/10">
                        @foreach ($this->displayRows() as $row)
                            <tr @class(['bg-gray-50 dark:bg-white/5 font-medium' => ($row['is_more'] ?? false)])>
                                <td class="py-2 pr-4 font-medium">
                                    {{ $row['label'] }}
                                    @if (! ($row['is_more'] ?? false))
                                        <span class="block text-xs fi-color-gray">
                                            {{ $row['category'] }}@if (! empty($row['sku'])) · {{ $row['sku'] }}@endif
                                        </span>
                                    @endif
                                </td>
                                <td class="py-2 px-3 text-right font-mono whitespace-nowrap fi-color-gray">{{ $this->fmtQty($row['qty'], $row['unit'] ?? null) }}</td>
            ...
            <div class="mt-3 text-xs fi-color-gray leading-relaxed">
                Καθαρές πωλήσεις ζωντανών παραστατικών ανά είδος/υπηρεσία· τα πιστωτικά αφαιρούνται (αξία & ποσότητα).
                Οι γραμμές χωρίς συνδεδεμένο προϊόν ομαδοποιούνται από την περιγραφή τους — η τελική παρένθεση
                περιόδου (π.χ. «(1/9/2026-31/8/2027)») αφαιρείται ώστε οι ανανεώσεις του ίδιου πακέτου να μετρούν
                μαζί. Τα σύνολα συμφωνούν με το «Έσοδα ανά κατηγορία».
            </div>
        @endif
    </x-filament::section>
</x-filament-panels::page>
