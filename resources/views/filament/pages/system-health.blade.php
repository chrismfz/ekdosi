    ...
    <x-filament::section>
        ...
        @if (($u['update_available'] ?? false) && ! $this->inAppApplyArmed() && $this->updateCommand())
            <div class="mt-3 rounded-lg border border-warning-200 bg-warning-50 p-3 text-sm text-warning-700 dark:border-warning-800 dark:bg-warning-950/40 dark:text-warning-400">
                ...
                <pre class="mt-2 overflow-x-auto rounded bg-gray-900 p-2 text-xs text-gray-100">{{ $this->updateCommand() }}</pre>
                <div class="mt-2">
                    Το script κάνει στιγμιότυπο ΒΔ → maintenance → checkout → composer →
                    migrate → optimize → shield → queue:restart → ops:health, με
                    <code>deploy/rollback.sh</code> για επαναφορά. Λεπτομέρειες:
                    <code>docs/updates-runbook.md</code>.
                </div>
            </div>
        @endif
