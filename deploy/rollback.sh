...
stop_queue_worker() {
  if [[ -n "${QUEUE_STOP_CMD:-}" ]]; then echo "▶ Draining queue worker"; eval "${QUEUE_STOP_CMD}";
  elif _have_unit; then echo "▶ Draining queue worker (systemd: ${QUEUE_SERVICE})"; systemctl stop "${QUEUE_SERVICE}" || echo "⚠ systemctl stop ${QUEUE_SERVICE} failed — set QUEUE_STOP_CMD; worker may write mid-restore." >&2;
  else echo "⚠ No queue-worker stop hook — set QUEUE_STOP_CMD; a live worker could write into the DB mid-restore." >&2; fi
}
start_queue_worker() {
  if [[ -n "${QUEUE_START_CMD:-}" ]]; then echo "▶ Starting queue worker"; eval "${QUEUE_START_CMD}";
