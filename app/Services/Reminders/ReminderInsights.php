...
final class ReminderInsights
{
    ...
    /**
     * @return array{awaiting: int, failed: int, sent30: int, paid_after: array{count: int, amount: float}}
     */
        ...
        return match ($gap) {
            ...
            self::GAP_BLOCKED => $this->planner->candidates($company)
                ->filter(fn (Invoice $i): bool => ReminderPlanner::dueDateOf($i) !== null
                    ...
                    && ($i->customer !== null && (blank($i->customer->email) || ! $i->customer->reminders_enabled)))
                ->values(),
            default => collect(),
        };
    }
