...
- **Άτυπη (μη φορολογική) σειρά** — σημαία «Άτυπη σειρά» στο είδος παραστατικού (π.χ. «ΕΣΩ» για τα δικά μας,
  «ΔΟΚ» για δοκιμές). Τα έγγραφά της περνούν κανονικά τη ροή (staging → πρόχειρο → οριστικοποίηση → προχωρά η
  επόμενη χρέωση υπηρεσίας / ανανέωση domain) αλλά **ποτέ** στο myDATA ή σε πάροχο, και **δεν μετράνε** σε πωλήσεις,
  ΦΠΑ, υπόλοιπα, καρτέλα, υπενθυμίσεις, dunning, πληρωμές (ένα κοινό φίλτρο στο `InvoiceScope::live()`). Δική τους
  αρίθμηση στην οριστικοποίηση (και σε tenant μέσω παρόχου), PDF «ΑΤΥΠΟ — ΔΕΝ ΑΠΟΤΕΛΕΙ ΦΟΡΟΛΟΓΙΚΟ ΣΤΟΙΧΕΙΟ», ποτέ
  email/πύλη/UBL/πιστωτικό. Η σημαία κλειδώνει μόλις η σειρά έχει εκδοθέντα. **Προεπιλεγμένο είδος παραστατικού ανά
  πελάτη** (προ-συμπληρώνει νέα παραστατικά και υπηρεσίες). **«Μετατροπή σε φορολογικό»** (άτυπο → πρόχειρο
  ΤΠΥ/ΤΙΜ/ΑΛΠ, σημερινή ημερομηνία, χωρίς νέα ανανέωση/κίνηση αποθέματος· το άτυπο μένει ίχνος) και αναφορά
  **Λογιστικά → «Αξία άτυπων»** (ανά πελάτη/είδος/σειρά + CSV). `docs/non-billable-services.md`.
...
- **QR + PDF** (Blade/dompdf) — **γλώσσα ανά παραστατικό** (Ελληνικά/Αγγλικά/**Δίγλωσσο
  GR-EN**), per-invoice/quote επιλογή· `App\Support\Pdf\PdfLabels` localizes μόνο τις ετικέτες
  (όχι ποσά/περιεχόμενο), σε invoice + quote **+ Καρτέλα (statement) + Απόδειξη Είσπραξης + Δελτίο
  Αποστολής**. Statement/receipt ακολουθούν τη **ζωντανή** γλώσσα του πελάτη· το **ΔΑ είναι frozen**
  στη snapshotted χώρα παραλήπτη (όπως το τιμολόγιο). Το **CMR** μένει αγγλικό (διεθνής standard φόρμα). Η γλώσσα του **PDF** μένει **παγωμένη στο έγγραφο**
  (ρητή επιλογή → snapshotted χώρα παραλήπτη, μέσω `PdfLabels`), ώστε ένα εκδοθέν PDF να μη
...
- **Έλεγχος αιτίας §8.3 ↔ τύπου παραστατικού** — `VatExemptionGuidance::typeConflict` (πίνακας `TYPE_CONFLICTS`).
  **Μπλόκο** (φόρμα + έκδοση/`AadeInvoiceDocument`, direct & πάροχος) μόνο για ό,τι δεν στέκει ποτέ: 16 (άρθρο 45,
  εγχώρια αντιστροφή) σε 1.2/2.2/1.3/2.3 και 14 (άρθρο 33, ενδοκοιν. αγαθά) εκτός 1.2· λέει και τη σωστή αιτία.
  **Προειδοποίηση** (εικονίδιο στη γραμμή) για τα ύποπτα (4 σε 1.1/2.1/1.2/1.3, 14/8 σε 2.2, 8 σε 1.2, OSS/IOSS· και ειδοποίηση όταν αλλαγή τύπου τα προκαλεί). Πιστωτικά/λιανική εκτός.
...
- **Έλεγχος ρυθμίσεων** (tab) — structured insight πάνω στο `MyDataConfigAudit`: ετοιμότητα
  ...
  λίστα Invoice Types.
- **Αδέσποτα πωλήσεων → σύνδεση ή καταχώριση** (Σελίδα ΜΑΡΚ, `App\Services\MyData\Orphans\*`): για ένα
  παραστατικό που υπάρχει στο myDATA χωρίς τοπικό, **«Ποιο τοπικό μπορεί να είναι;»** (`OrphanMatcher` — τοπικά
  χωρίς ΜΑΡΚ, κατάταξη σειρά/ΑΑ → ποσό → ΑΦΜ → ημ/νία, + ειδοποίηση «ίδιος αριθμός με άλλο ΜΑΡΚ») →
  **«Σύνδεση»** μόνο με το ίδιο παραστατικό (`OrphanLinker`: INSERT row + mirror όπως μια διαβίβαση, χωρίς επαναποστολή/
  email· κλειδώνει όπως ο submitter)· αλλιώς
  **«Καταχώριση τοπικά»** (`OrphanImporter`: όπως δηλώθηκε — σειρά/ΑΑ, ΜΑΡΚ, QR, μία γραμμή ανά γραμμή ΑΑΔΕ με ΦΠΑ/
  εξαίρεση/E3· καθαρό/ΦΠΑ/σύνολο στο λεπτό· ο μετρητής της σειράς προχωρά πέρα από τον ΑΑ· `origin = mydata_orphan`
  → χωρίς balance snapshot, εκτός αυτόματων υπενθυμίσεων· πιστωτικά / παρακρατήσεις / μη αριθμητικός ΑΑ → χειροκίνητα).
...
- **Συμπλήρωση παλαιότερων ετών από CLI** (`mydata:import-expenses --tenant=SLUG --year=YYYY…`) — τα έξοδα
  (προμηθευτών + δικά μας 13/14/17.x) ολόκληρων ετών, ανά τρίμηνο, idempotent (ίδιοι `ExpenseImporter`
  κανόνες με την κονσόλα)· για όταν η εισαγωγή ξεκίνησε μετά τα βιβλία (αλλιώς τα «Φορολογικά» υπερεκτιμούν το κέρδος).
...
- **VIES (EU)** — επαλήθευση/άντληση μη-GR ενδοκοινοτικών ΑΦΜ (`ViesLookup`) +
  **reverse-charge hint** (0% + §8.3 από τον τύπο: υπηρεσία 2.2→«4 — άρθρο 18», αγαθά 1.2→«14 — άρθρο 33»).
...
- **Καρτέλα**: ledger κινήσεων, aging, **YoY**, charts, εξαγωγή **PDF/CSV** + email·
  ...
  **Modal «Κατανομή»** (ομαδοποιημένη είσπραξη/έμβασμα): εκτός από την κατανομή ανά τιμολόγιο, δείχνει
  και **«από πού ήρθε»** — κανάλι/προέλευση (Ηλεκτρονική πύλη · Eurobank / Χειροκίνητα, μέσω
  `Payment::channelLabel`), τρόπο και κωδικό συναλλαγής — lazy από τα `payment_ids`, μόνο τα κοινά του
  group. Έτσι ο operator βλέπει την προέλευση χωρίς να ανοίγει το μενού «Πληρωμές».
...
- **Import cleanup — αντιστοίχιση εισαγόμενων «έναντι» (EPS:) σε ανοιχτά τιμολόγια**
  (`payments:apply-imported-credits --company=SLUG [--customer=ΑΦΜ] [--dry-run]`): σε πελάτες Epsilon
  (πληρωμές on-account, όχι ανά τιμολόγιο) «δένει» FIFO την έναντι-πίστωση στα ανοιχτά τιμολόγια —
  **net-zero, idempotent, μόνο EPS: γραμμές**, με `--dry-run` προεπισκόπηση· ώστε τα ιστορικά-πληρωμένα
  να παύουν να δείχνουν «ανοιχτά» και η αυτόματη «Είσπραξη» να μην τα στοχεύει λάθος.
...
- **Υπενθυμίσεις πληρωμής (dunning, στυλ WHMCS)** — «πριν τη λήξη» + 1η/2η/3η βαθμίδα σε ημέρες μετά τη
  ...
  Στην **«Ηλικίωση οφειλών»** (`ReminderInsights`): κάρτες υπενθυμίσεων (προς έγκριση / αποτυχίες / εστάλησαν /
  εξοφλήθηκαν μετά από υπενθύμιση), **«Δεν υπενθυμίζονται»** (πρόχειρα που δεν στάλθηκαν, WHMCS, legacy, χωρίς
  email/opt-out, εκτός κριτηρίων — με λίστα), «Τελ. υπενθύμιση» ανά πελάτη, **«Υπενθύμιση τώρα»** (χειροκίνητη, ανά
  παραστατικό). Ποτέ υπενθύμιση τιμολογίου σε πελάτη που δεν χρωστάει συνολικά (έναντι/πίστωση).
...
- **Φορολογικά** (`TaxOverview`, perm `View:TaxOverview`) — «τι χρωστάμε» σε μία σελίδα, με επιλογή έτους: ΦΠΑ ανά
  τρίμηνο + μήνα (πιστωτικό → επόμενο τρίμηνο), **εκτίμηση φόρου εισοδήματος** για ΟΕ/ΕΕ/ΙΚΕ/ΑΕ (`IncomeTaxEstimate`:
  κέρδος από το Βιβλίο Εσόδων-Εξόδων + 17.3/17.4 ως έσοδα, × συντελεστή, − παρακρατήσεις, + προκαταβολή, − περσινή
  βεβαιωμένη προκαταβολή — ΠΟΤΕ μαντεψιά, η δική μας εκτίμηση μόνο ως υπόδειξη → υπόλοιπο, μηνιαία αποταμίευση,
  προβολή 31/12 από 30 ημέρες και μετά, ⚠ όταν τα έξοδα ενός έτους < 10% των εσόδων), έξοδα ανά κατηγορία (μισθοδοσία /
  ΕΦΚΑ / αποσβέσεις με «έως»), πίνακας 6 ετών. «Φορολογικό προφίλ» (`IncomeTaxProfile`: συντελεστής 22%, προκαταβολή 80%,
  βεβαιωμένες προκαταβολές ανά έτος) από όποιον έχει «Ρυθμίσεις εταιρίας». Εκτίμηση για προγραμματισμό, όχι δήλωση.
  **Πηγή ανά έτος:** κλεισμένο έτος με στιγμιότυπο Ε3 (`E3YearSnapshot`, `E3YearTotals`: «Ανανέωση Ε3 (ΑΑΔΕ)» /
  `mydata:e3-snapshot`) → έσοδα/έξοδα/πάγια από το Ε3 της ΑΑΔΕ (τελικός χαρακτηρισμός λογιστή)· αλλιώς και για το
  τρέχον έτος → τοπικά (χωρίς αγορές παγίων E3_882/883). Η άλλη πλευρά φαίνεται ως διασταύρωση.
...
- **Εισαγωγή CSV — πελάτες / προϊόντα / προμηθευτές** (`App\Services\Import\*`, κουμπί «Εισαγωγή CSV» στις
  λίστες): αυτόματη αναγνώριση στηλών (ελληνικά/αγγλικά ή οι στήλες του δικού μας export) + **πρότυπο CSV**
  (`;`, για ελληνικό Excel) + **προεπισκόπηση** πριν την εισαγωγή (νέα / συμπλήρωση κενών / χωρίς αλλαγή /
  σφάλματα ανά γραμμή). Υπάρχουσα εγγραφή (ίδιο ΑΦΜ · ίδιος κωδικός/barcode) παίρνει **μόνο τα κενά** της —
  τίποτα δεν αντικαθίσταται, ξανατρέξιμο. Έλεγχος ψηφίου ελέγχου ΑΦΜ, ΦΠΑ ποτέ «μαντεμένο» (άγνωστος
  συντελεστής = σφάλμα), Windows-1253 / BOM / `,` `;` Tab, «12,50» και «1.234,56» (το διφορούμενο «1.200» απορρίπτεται),
  ΑΦΜ που το Excel έκοψε το αρχικό 0. Οι γραμμές με σφάλμα παραλείπονται· σφάλμα βάσης = δεν γράφεται τίποτα.
  Η συμπλήρωση υπάρχουσας εγγραφής ακολουθεί το δικαίωμα επεξεργασίας.
...
- **Προτιμολόγια — ορατότητα (SHIPPED):** widget «Προτιμολόγια» (απλήρωτα / πληρωμένα-προς-έκδοση) +
  φίλτρο «Προτιμολόγια» στη λίστα παραστατικών. Απαραίτητο επειδή είναι εκτός «Απαιτήσεων» εξ ορισμού.
- **Προτιμολόγιο (SHIPPED):** «Προσφορά στον πελάτη» σε draft → κλειδώνει, γίνεται ορατό και πληρωτέο
  στην πύλη, χωρίς να εκδοθεί (χωρίς ΑΑ/myDATA). Ο πελάτης το πληρώνει ή το εξοφλεί από την πίστωσή του·
  το τελικό παραστατικό εκδίδεται μετά. Για επαναλαμβανόμενες υπηρεσίες: μηδέν ακυρωτικά για υπηρεσίες
  που ο πελάτης τελικά δεν θέλει. PDF/πύλη το σημαίνουν ρητά ως μη-φορολογικό.
- **Χρήση πίστωσης (SHIPPED):** από την Καρτέλα πελάτη, από την καρτέλα «Πληρωμές» του παραστατικού, ΚΑΙ
  από την πύλη (self-service). Re-point υπάρχοντος χρήματος, net-zero.
...
## 22. Προσωπικό / Άδειες (ΕΡΓΑΝΗ Φάση 1 — `docs/ergani/README.md`)
- **Εργαζόμενοι** (company_admin): ΑΦΜ/ονοματεπώνυμο όπως στο ΕΡΓΑΝΗ, ημέρες κανονικής άδειας/έτος, σύνδεση με λογαριασμό panel.
- **Άδειες**: αίτημα (ο ίδιος ή ο admin για λογαριασμό του) → έγκριση/απόρριψη/ανάκληση μέσω `LeaveWorkflow` (lock, μία πηγή
  side effects). Είδη = κωδικοί ΕΡΓΑΝΗ `WTOLeave` (ΑΔΚΑΝ, ΑΔΑΣ, …). Εργάσιμες ημέρες αυτόματα (Σ/Κ + εθνικές + τοπικές αργίες),
  επικάλυψη απορρίπτεται, υπόλοιπο κανονικής ανά έτος. Email στον λογιστή σε έγκριση/ανάκληση· καμπανάκι σε εγκρίνοντες/αιτούντα.
- **Ημερολόγιο αδειών**: εργαζόμενοι × ημέρες μήνα, αργίες σκιασμένες· οι συνάδελφοι βλέπουν «Άδεια» χωρίς είδος (GDPR —
  η ασθένεια είναι δεδομένο υγείας) και όχι τα εκκρεμή τους.
- **Τοπικές αργίες** ανά εταιρεία (σταθερή / σχετική με Πάσχα / μία φορά) — δεν υπάρχει επίσημη λίστα.
- **Ρόλος `ergani`** («Προσωπικό — μόνο άδειες») για μη-χειριστές: default-deny σε κάθε άλλη οθόνη, όχι dashboard/Βοηθός/
  ειδοποιήσεις τιμολογίων (`ErganiStaff`). Χειριστές: αίτημα + ημερολόγιο. Έγκριση: company_admin / super_admin.
...
- **Tablet «ρολόι»** (`/card-kiosk`, `CardKioskController`, `WorkCardKioskDevice`): ενεργοποίηση ΣΥΣΚΕΥΗΣ από admin
  ...
  «Σημείο κάρτας».
- **Οδηγοί χρήσης**: checklist «Ξεκίνημα Προσωπικού» (`StaffSetupChecklist`, διπλώνεται ως «✓ όλα έτοιμα» όταν ολοκληρωθεί), οδηγίες/κατάσταση
  στο «Χτύπημα κάρτας», «Ξεκλείδωμα PIN», ενότητα «Προσωπικό — ΕΡΓΑΝΗ» στις Ρυθμίσεις εταιρείας (company_admin).
- **Σύνδεση εργαζομένων με λογαριασμούς** (`EmployeeAccountMatcher`): προτάσεις με email/ονοματεπώνυμο (μόνο μοναδικές,
  μόνο χρήστες της εταιρείας, ποτέ ήδη συνδεδεμένοι), επιβεβαίωση ανά γραμμή ή μαζικά· προειδοποίηση για λογαριασμό χωρίς ρόλο.
- **Εισαγωγή από ΕΡΓΑΝΗ** (`ErganiEmployeeImporter`, `ErganiClient::service()`): `EX_BASE_05` από την Παραγωγή (read-only),
  ελαχιστοποίηση δεδομένων (ΑΦΜ/ονοματεπώνυμο/παράρτημα/πρόσληψη — όχι ΑΜΚΑ/ταυτότητα/αποδοχές), εφαρμογή μόνο από φρέσκια ανάγνωση.
- **Υπερωρίες** (`OvertimeService`, `overtime_declarations`, opt-in `ergani_submit_overtime`): «Νέα υπερωρία» → `WTOOv`
  (`f_type` `ΥΠ`) μόνο πριν την έναρξη, κλείδωμα ανά εργαζόμενο κατά των επικαλύψεων, ίδια θωράκιση (claim/Αβέβαιο/audit),
...
- **Οδηγός «Πέρασμα σε Παραγωγή»** (`ErganiGoLive`, `ErganiGoLiveMail`): έλεγχοι + υποχρεωτική επιβεβαίωση λογιστή + email·
  `companies.ergani_production_since/_by_user_id`. Το `ergani_mode` αλλάζει μόνο από εκεί.
- **Ορατότητα ΕΡΓΑΝΗ**: `ErganiStatusWidget` (dashboard — «Θέλει προσοχή» + υπερωρίες σήμερα/αύριο), υπερωρίες στο
  ημερολόγιο αδειών, `ErganiTrialBar` (render hook στις σελίδες που δηλώνουν), φύλακας `ergani:watch` (`IsInCardSector`
  από Παραγωγή, εβδομαδιαίο, ειδοποίηση στην αλλαγή).
- Επόμενα (BACKLOG): πρόγραμμα εργαζομένου (`EX_BASE_08/07` — **μπλοκαρισμένο** όσο η εταιρεία δεν είναι στην κάρτα),
  διασταύρωση αδειών, `WTOLeaveC` (ορθή επανάληψη).
