...
- **Τέλη / παρακρατήσεις / φόροι** — withholding (§8.4), χαρτόσημο/τέλη/λοιποί/
  ...
  **«Πρότυπα τελών»**: selective-import έτοιμων προϊόντων με θεσμικό τέλος (σακούλα/πλαστικά/
  ανακύκλωσης/διαμονής, προ-ρυθμισμένα §8.5) στη λίστα Προϊόντων.
...
- **Φύλαξη έκδοσης (`WhmcsFilingGuard`)** — choke-point πριν δεσμευτεί ΑΑ που ΚΡΑΤΑ (HOLD) στα
  ...
  ασυμφωνία συντελεστή ΦΠΑ (WHMCS `taxrate` vs default) και ασυμφωνία μικτού συνόλου με το WHMCS total
  (πέρα από ανοχή στρογγυλοποίησης)· επιπλέον το `whmcs:auto-issue` εξαιρεί rows που το legacy ekdosi
  έχει ήδη τιμολογήσει (`legacy_invoiced`), αποτρέποντας διπλή υποβολή στο dual-run.
