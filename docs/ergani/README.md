...
- **Φύλακας προσωπικού** (`ergani:watch`, εβδομαδιαίο, **ενεργό από προεπιλογή** — διακόπτης στο «Χρονοπρογραμματισμός»): διαβάζει
  `EX_BASE_05` από την Παραγωγή μόνο στη μνήμη· κρατά ΜΟΝΟ ΑΦΜ/ονοματεπώνυμο/τοπικό id των διαφορών (`companies.ergani_roster_diff`,
  τελευταίο στιγμιότυπο, αντικαθίσταται κάθε εβδομάδα)· τα καμπανάκια έχουν μόνο αριθμούς. Κενό/απρόσμενο δυναμικό = σφάλμα (ποτέ
  «έφυγαν όλοι»)· καμπανάκι μόνο όταν εμφανίζεται ΝΕΑ διαφορά.
