...
- **`renew` = ΜΕΤΑ ΤΗΝ ΠΛΗΡΩΜΗ — ΚΛΕΙΔΩΜΕΝΟ** (ιδιοκτήτης 2026-09-23: «πάντα με την πληρωμή»· η ανανέωση στον
  ...
  **⚠ Δεν επιβάλλεται ακόμα:** αν οριστικοποιηθεί **απλήρωτο** παραστατικό ανανέωσης (επί πιστώσει, ή κατά λάθος),
  το hook ανανεώνει ήδη στην έκδοση. Κανόνας (ιδιοκτήτης): **ανανέωση = οριστικοποιημένο ΚΑΙ εξοφλημένο** —
  αλλιώς «σε αναμονή πληρωμής» και ανανέωση όταν εξοφληθεί. Εξόφληση = καταχωρισμένη πληρωμή (και η χειροκίνητη
  «εξοφλήθηκε» του operator). Παγίδα: το `InvoiceBalance` θεωρεί τα μετρητοίς (`due_days=0`) «πληρωμένα στην
  έκδοση» — για το guard αυτό δεν αρκεί. (BACKLOG)
