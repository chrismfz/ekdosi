...
## 8. Πληρωμές & χρήματα
**Legacy:** `FAddPayment`, `PAYMENT` — πληρωμές **μόνο σε επίπεδο πελάτη**
(CUST_ID, ημ/νία, ποσό, σημείωση)· **καμία** σύνδεση με τιμολόγιο, κανένας
τρόπος, καμία είσπραξη/κατανομή/επιστροφή.

**Νέο:** ✅ **Πλήρης AR σουίτα**, με **`InvoiceBalance` ως μοναδική πηγή** για
paid/credited/balance/status (cache στήλες γράφονται μόνο από αυτό· έλεγχος
συνέπειας dashboard ↔ ledger ↔ caches με test):
- **Cockpit ανά τιμολόγιο** (tab «Πληρωμές»): προσθήκη/επεξεργασία/διαγραφή,
  «Πλήρης εξόφληση» / «Μερική» / «Σήμανση ως ανεξόφλητο».
- **Είσπραξη/Έμβασμα** (FIFO): ένα ποσό κατανέμεται στα ανοιχτά τιμολόγια,
  υπόλοιπο → **on-account πίστωση/προκαταβολή**· **ομαδοποίηση** στην Καρτέλα
  («Έμβασμα €X» + drill-down κατανομής).
- **Χρήση πίστωσης** (#1): εφαρμογή υπάρχουσας on-account πίστωσης σε επόμενο
  τιμολόγιο (net-zero) + **Χειροκίνητη κατανομή** (#2): ακριβές ποσό ανά τιμολόγιο.
- **Επιστροφές/refunds** (#3): χρήμα **πίσω** στον πελάτη (`kind='refund'`),
  nets out σε όλα τα money surfaces — κλείνει τον κύκλο «μέσα → πίσω».
- **Τραπεζικοί λογαριασμοί** (σε ποιον μπήκαν τα χρήματα + λογαριασμός κατάθεσης
  στο PDF), **κωδικός συναλλαγής** (Stripe/PayPal/ref τράπεζας).
- **Ληξιπρόθεσμα/Due**: ημ/νία λήξης (`issued_at + due_days`), badge + filter στη
  λίστα, dashboard widget, ημερήσιες ειδοποιήσεις (bell, χωρίς email).
