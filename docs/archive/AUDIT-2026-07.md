...
- [x] **MON-5 · LOW — ✅ RESOLVED 2026-07-11** — Νέο `InvoiceScope::excludeUnissuedDrafts()` σε όλα τα money surfaces (`baseInvoices`/receivables/top-customers + `Customer::scopeWithOutstandingBalance` + `CustomerLedgerBuilder`): τα unissued sale-drafts ΔΕΝ μετράνε πια σε τζίρο/εισπρακτέα/ΦΠΑ/Καρτέλα. Κρατάμε credit-note drafts (μειώνουν, σκόπιμο) + legacy drafts. Ορατότητα: `draftsPipeline()` + tile «Πρόχειρα (προτιμολόγια)» (count+αξία, click→drafts list). Pro-forma numbering (ΠΡΟΤ-N) → BACKLOG (service-manager φάση). ~10 fixtures→active, tests, full suite πράσινο.
...
- [x] **MON-7 · LOW — ✅ RESOLVED 2026-07-11** (operator warning όταν το gross δεν κάνει round-trip σε 2dp· `InvoiceForm` afterStateUpdated· test) — Gross-edit: στρογγυλοποίηση net σε 2dp στο form (`InvoiceForm::netFromGross`) → 10,00€ @24% ξαναϋπολογίζεται 9,99€. Εγγενές στο `decimal(14,2)`· ενημέρωση χειριστή ή 4dp ενδιάμεσο.
...
- [x] **DOC-8 · LOW — ✅ RESOLVED 2026-07-11** — Ο `MailTemplateRenderer` κάνει πλέον backslash-escape του markdown (ASCII punctuation) στις interpolated ΤΙΜΕΣ του body (customer_name/tenant_name/…) — όχι στο operator template (κρατά επίτηδες markdown) ούτε στο verify_url/mark_section (μένουν λειτουργικά)· το subject (plain text) δεν αγγίζεται. Ένα «[x](url)» όνομα εμφανίζεται πλέον literal αντί για live link. Tests με CommonMark parse.
- [x] **DOC-9 · LOW — ✅ RESOLVED 2026-07-11** — Το action label «Email PDF to customer» → «Αποστολή PDF στον πελάτη». Το ledger view **ήδη** ελληνικό (`Money::eur` = `number_format(…,',','.')`, συνεπές με το PDF — το finding ήταν stale). CMR σε αγγλικό format = συνειδητό (διεθνές έγγραφο)· long-domain overflow = κοσμετικό edge, αφήνεται.
...
- [x] **WH-9 · LOW — ✅ RESOLVED 2026-07-11** (plugin v0.42.0: `whereNotExists` στο `mod_ekdosi_invoice_marks` στο `paid_unfiled` feed· τα υπόλοιπα (replay-nonce, re-stage hint, EU reverse-charge single reason) μένουν deferred ως documented) — Το feed `paid_unfiled` μετά το legacy cutover μεγαλώνει για πάντα (δεν εξαιρεί ids με mark στο `mod_ekdosi_invoice_marks`) — O(N) 15λεπτη δουλειά, idempotent αλλά άσκοπη. One-line join. Επίσης: χωρίς replay-nonce στα HMAC endpoints (mitigated — όλα idempotent)· held rows audit-frozen χωρίς «re-stage» hint· EU reverse-charge εκφράζεται μόνο με ΜΙΑ tenant-wide αιτία απαλλαγής (κρατιέται σωστά, capability gap).
...
- [x] **OPS-12 · LOW — ✅ RESOLVED 2026-07-11** — Idempotency: κάθε dispatch φέρει σταθερό `send_key` (uuid, serialized· επιβιώνει στα retries)· αν προηγούμενο attempt με το ίδιο key άφησε `sending` (hard crash μετά το SMTP accept) ή `sent`, το retry ΔΕΝ ξαναστέλνει (reconcile → sent, bias at-most-once). Νέα στήλη `invoice_mail_log.send_key`. SPF/DKIM fallback: το warning log έγινε actionable («fix the tenant SMTP»).
- [x] **OPS-13 · LOW — ✅ RESOLVED 2026-07-12** — (α) Οι δύο per-tenant sweeps (`whmcs-fetch-all`, `mydata-reconcile-all`) τρέχουν πλέον μέσα από `$sweepTenants` με per-tenant isolation: ένα uncaught exception ΕΝΟΣ tenant δεν αμπαλάρει το υπόλοιπο sweep + καταγράφεται ως per-tenant failure (`report()` + `recordWhmcsFetch/recordMyDataReconcile(exit 1)`), αντί να αφήνει stale «ok». (β) Staleness: το `OperatorHealthReport` σημειώνει `stale` έναν ENABLED sweep που δεν έχει καταγράψει run > 26h (`sweepIsStale`, gated στο ίδιο `SystemSettings->bool("schedule.…")` με το scheduler)· `OperatorHealthSeverity` βγάζει warning (failed > stale precedence)· badge «κόλλησε» στη σελίδα Υγεία. Tests: `TenantSweepStalenessTest` (+4), `OperatorHealthSeverityTest` (+2).
- [x] **OPS-14 · LOW — ✅ RESOLVED 2026-07-11** — `.env.example` τεκμηριώνει το «single = ένα ατέρμονο αρχείο» + `LOG_STACK=daily`/`LOG_DAILY_DAYS` (built-in rotation)· INSTALL.md §Logs με logrotate stanza (OS-level) — δύο δρόμοι, zero behavior change σε υπάρχοντα prod.
- [x] **OPS-15 · LOW — ✅ RESOLVED 2026-07-12** — (α) Το `go-live-check` per-tenant backup gate ΔΕΝ περνάει πια με μόνο το `enabled` flag: απαιτεί ΑΠΟΔΕΙΞΗ πρόσφατου επιτυχημένου run (`company_backup_runs.status=ok`)· καμία επιτυχημένη εκτέλεση → WARN «τρέξε δοκιμαστικό backup + restore drill», stale (>8 ημ.) → WARN. (β) `docs/updates-runbook.md` §Restore drill απέκτησε πίνακα **cadence** (πριν go-live per-tenant / τριμηνιαίο off-site / μετά από αλλαγή pipeline / μετά από WARN του gate) + «κατέγραψε κάθε drill». Tests: `GoLiveCheckTest` (+3).
...
- [x] **SET-4 · LOW — ✅ RESOLVED 2026-07-11** — Προστέθηκαν στο `.env.example` τα missing (schedule flags QUEUE_HEARTBEAT/MYDATA_CONSOLE_REFRESH/MYDATA_FETCH_EXPENSES/AI_REMINDERS, όλα τα cron overrides *_CRON, BACKUP_LOCAL_DISK, MYDATA_INDOUBT_GRACE_MINUTES, και το νέο EKDOSI_UPDATE_* + GITHUB_TOKEN block). `BACKUP_ARCHIVE_PASSWORD` ήταν ήδη παρόν.
- [x] **SET-5 · LOW — ✅ RESOLVED 2026-07-12** — Νέα aggregate `MyDataLookupSeeder::seedStandardLookups()` (single source για create/switch/install)· το `EditCompany::afterSave()` την τρέχει όταν το `einvoice_provider` γυρίζει σε ελληνική τιμολόγηση (`gr-mydata`/`gr-provider`) → ο tenant που ξεκίνησε `none`/PEPPOL δεν μένει με άδεια lookups. Idempotent/fill-empty (Greek→Greek edit = no-op, notification μόνο όταν όντως σπάρθηκε κάτι)· `einvoice_provider` super_admin-only μέσω αυτής της σελίδας. Tests: `CompanyAutoSeedTest` (+2).
- [x] **SET-6 · LOW (docs) — ✅ RESOLVED 2026-07-11** — CLAUDE.md gross-edit «NOT yet wired» → «✅ wired (G7) + MON-7 warn»· «auto-calc percentage amounts still open» → «✅ `RecomputeInvoiceTaxes` on-save, μένει μόνο preset-expansion»· `DeliveryNoteSubmitter` docblock «NEEDS SANDBOX VALIDATION» → «✅ validated 2026-06-10».
