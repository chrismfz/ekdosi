...
- **⚠ Guard «ανανέωση domain μόνο εξοφλημένη» — money, πριν ανοίξει ζωντανή ανανέωση σε registrar.** Απόφαση:
  ...
  Κανόνας: **οριστικοποιημένο ΚΑΙ εξοφλημένο** (καταχωρισμένη πληρωμή, και η χειροκίνητη «εξοφλήθηκε»)· απλήρωτο →
  «σε αναμονή πληρωμής» + ανανέωση όταν εξοφληθεί· τα μετρητοίς (`due_days=0`, «paid at issue») δεν αρκούν.
  `domains/README.md` §6.1.
