...
- **Φάσεις 1–3 → SHIPPED** (FEATURES §22): εργαζόμενοι/άδειες/ημερολόγιο/ρόλος `ergani`, δήλωση `WTOLeave`, κάρτα `WRKCardSE`
  + tablet «ρολόι», σύνδεση λογαριασμών, **εισαγωγή από `EX_BASE_05`** (v2.7.0). MyIP **δεν** είναι υπόχρεη κάρτας
  (`EX_BASE_01.IsInCardSector=0`). Άδεια που διασχίζει έτος → μοιράζεται ανά εργάσιμες (fixed).
- **Σειρά (συμφωνήθηκε 2026-09-26):** (1) ~~υπερωρία `WTOOv`~~ SHIPPED · (2) πρόγραμμα εργαζομένου `EX_BASE_08/07` — **BLOCKED (εξωτερικό)**: και στην Παραγωγή
  «Criteria doesn't meet requirements» = διαθέσιμο μόνο σε εργοδότες στην κάρτα· ξεκλειδώνει όταν το πιάσει ο φύλακας `ergani:watch`.
- **Ιδέες ορατότητας/insights (όλες read-only προς ΕΡΓΑΝΗ, «θεμιτές» — owner 2026-09-26):** ~~κάρτα «ΕΡΓΑΝΗ» στο dashboard~~,
  ~~μπάρα «ΔΟΚΙΜΑΣΤΙΚΟ»~~, ~~φύλακας `IsInCardSector`~~, ~~υπερωρίες στο ημερολόγιο~~ SHIPPED · ~~«Ιστορικό ΕΡΓΑΝΗ»
  (UI πάνω στο `ergani_submissions` + PDF)~~ SHIPPED · **διασταύρωση αδειών ekdosi↔ΕΡΓΑΝΗ** (⚠ το `Lookup_Submissions` είναι μόνο ο κατάλογος εντύπων — δηλώσεις τρίτων φαίνονται
  μάλλον μόνο μέσω `EX_BASE_07`, άρα **blocked** όπως το πρόγραμμα· lookup υποβολών,
  `schemas/Lookup_Submissions.json` — πιάνει δηλώσεις λογιστή/διπλές) · ~~εβδομαδιαίος φύλακας
  δυναμικού (`EX_BASE_05` diff)~~ SHIPPED · ~~PDF ΕΡΓΑΝΗ στο email λογιστή / «Οι άδειές μου»~~ SHIPPED · ~~οδηγός «Πέρασμα σε Παραγωγή»~~ SHIPPED
  (test Παραγωγής, ΑΦΜ/παράρτημα, **ο λογιστής σταματά να δηλώνει** — κίνδυνος διπλής δήλωσης). Να καλύψει και τις
  **μελλοντικές υπερωρίες/άδειες που καταχωρήθηκαν στο trial** (το email τότε ζήτησε χειροκίνητη δήλωση): μετά το πέρασμα,
  ένα νέο «Νέα υπερωρία» για τις ίδιες ώρες δεν μπλοκάρεται — προειδοποίηση/επιβεβαίωση (review υπερωριών, P2).
- **Αλλαγή κωδικών e-ΕΦΚΑ ενώ είναι ήδη σε Παραγωγή** (review go-live, P2): το απλό save της Εταιρείας αλλάζει username/password
  χωρίς να ξαναγίνει ο έλεγχος ΑΦΜ του οδηγού → μια λάθος σύνδεση (άλλου εργοδότη) θα δήλωνε πραγματικά. Λύση: στο save σε
  Παραγωγή με αλλαγμένους κωδικούς, ξανά `EX_BASE_01` + ταίριασμα ΑΦΜ (ή υποχρεωτικό «Test σύνδεσης» πριν αποθηκευτούν).
- **Συγκρούσεις άδειας ↔ CRM (DEFERRED, owner 2026-09-27 — «too much για αρχή»):** άδεια που πέφτει πάνω σε «επόμενο βήμα»
  lead του ίδιου χειριστή → προειδοποίηση στη φόρμα αίτησης/έγκρισης άδειας (με links να μετατεθεί/ανατεθεί) · ορισμός/drag
  επόμενου βήματος σε μέρα άδειας/αργίας του χειριστή → προειδοποίηση · καμπανάκι μετά την έγκριση με τα leads μέσα στην άδεια ·
  σκίαση ημερών άδειας στο «Ημερολόγιο leads» ανά χειριστή. Δεδομένα: `leads.next_action_at` + `assigned_user_id` ↔
  `employees.user_id`. Ποτέ μπλοκάρισμα — μόνο ορατότητα· συνάδελφοι βλέπουν «σε άδεια», όχι είδος.
- Ανοιχτά (μικρά): ICS feed ημερολογίου (→ επόμενο, μαζί με τα «επόμενα βήματα» leads) · ανά-παράρτημα αργίες.
---
