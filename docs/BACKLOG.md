...
- **Σειρά (συμφωνήθηκε 2026-09-26):** (1) ~~υπερωρία `WTOOv`~~ SHIPPED · (2) πρόγραμμα εργαζομένου `EX_BASE_08/07` (ημερολόγιο εργάσιμων
  για αίτηση άδειας — στο trial «Criteria doesn't meet requirements», ξαναδοκιμή στην Παραγωγή, μόνο ανάγνωση).
- **Ιδέες ορατότητας/insights (όλες read-only προς ΕΡΓΑΝΗ, «θεμιτές» — owner 2026-09-26):** κάρτα «ΕΡΓΑΝΗ» στο dashboard
  (περιβάλλον, τελευταία επιτυχία, Αβέβαια/αποτυχίες, εγκεκριμένη άδεια που ξεκινά χωρίς δήλωση) · «Ιστορικό ΕΡΓΑΝΗ»
  (UI πάνω στο `ergani_submissions` + PDF) · μπάρα «ΔΟΚΙΜΑΣΤΙΚΟ» · **διασταύρωση αδειών ekdosi↔ΕΡΓΑΝΗ** (lookup υποβολών,
  `schemas/Lookup_Submissions.json` — πιάνει δηλώσεις λογιστή/διπλές) · εβδομαδιαίοι φύλακες `IsInCardSector` και
