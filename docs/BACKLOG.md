...
## 🆚 vs Epsilon — roadmap κενών

> Roadmap «τι μας λείπει vs ο ανταγωνισμός» (Epsilon Smart) — **folded εδώ από το πρώην ξεχωριστό
> «vs Epsilon» backlog** (2026-09-17· ένα ενιαίο backlog πλέον). Ό,τι έχει γίνει ζει στο `CHANGELOG.md` /
> `FEATURES.md`· το «γιατί» μιας απόφασης → `docs/CLAUDE-history.md`. (Αρχική VERIFIED σύγκριση 16/09/2026,
> διασταυρωμένη με τον κώδικα — file:line.) Το λεπτομερές tech-debt / open-items ζει στις υπόλοιπες ενότητες
> αυτού του αρχείου — εδώ είναι η roadmap-όψη «vs ανταγωνισμός», με pointers όπου το ίδιο work
> παρακολουθείται ήδη παρακάτω.
>
...
- [x] **Email i18n (M) — SHIPPED:** τα 5 customer-facing Mailables + τα 8 mail views + subjects → `lang/{el,en}/mail.php` (parity)· κάθε dispatcher κάνει `->locale()` μέσω `CustomerLanguage::forDocumentMail`/`forCustomerMail`· `MailTemplateRenderer` απέκτησε EN default invoice body + locale-aware MARK section (μόνο για μη-custom template)· el verbatim· en-render test (`MailLocaleTest`). Λεπτομέρειες → CHANGELOG/FEATURES.
  - [ ] P2: το subject του `QuoteMailLog` (audit) μένει ελληνικό «Προσφορά …» — cosmetic, operator-facing· το πραγματικό email subject (envelope) μεταφράζεται.
  - [ ] P2 (ticket edge): η γλώσσα του reply/feedback email ακολουθεί τον **συνδεδεμένο πελάτη** του ticket (`forCustomerMail`), ενώ ο παραλήπτης είναι `requester_email ?: customer->email`. Αν έγραψε άλλο πρόσωπο (π.χ. Έλληνας συνεργάτης σε ticket ξένου πελάτη), το σώμα βγαίνει στη γλώσσα του πελάτη, όχι του παραλήπτη. Δεν γνωρίζουμε τη γλώσσα ενός ad-hoc requester· ο πελάτης είναι λογικό default (και βελτίωση vs το προ-i18n «πάντα ελληνικά»). Άσε το μέχρι να εμφανιστεί πραγματικά.
  - [ ] P2 (DRY): το both→en collapse υπάρχει και στο `CustomerLanguage::forCustomerMail` και inline στο `forDocumentMail`· αν αλλάξει ποτέ ο κανόνας «email body = μία γλώσσα», δύο σημεία. Trivial· extract ένα private helper αν ενοχλήσει.
  - [ ] P2 (edge): subject vs body localization αποφασίζονται ανεξάρτητα ανά template (`localizeComposed` per-call). Custom subject + default body σε en παραλήπτη → το μόνο composed στο subject (`invoice_type` fallback «Document»/«Παραστατικό») μπορεί να διαφέρει γλώσσα από το body. Σχεδόν ποτέ (το type name είναι συνήθως set)· edge-only.
- [ ] **IDEA (M) — κεντρικός editor κειμένων/ετικετών (WHMCS-style «template manager»):** ένα σημείο στο μενού «Ρυθμίσεις → Κείμενα/Ετικέτες» να βλέπεις + edit ΟΛΑ τα customer-facing strings (PDF labels `PdfLabels::MAP`, email strings `lang/{el,en}/mail.php`, portal `lang/{el,en}/portal.php`), el/en, per-tenant ή global. Σχήμα: πίνακας `label_overrides` (`company_id` nullable=global, `group`, `key`, `locale`, `value`) + override-layer στο `PdfLabels`/`__()` (DB πρώτα, **fallback στα code defaults** ώστε κενό override να μη σπάει ποτέ + νέα keys να έρχονται με τον κώδικα) + Filament σελίδα (grouped, inline edit, super/company-admin gate). Τα invoice email subject/body είναι ήδη tenant-editable (`companies.mail_*_template`) — αυτό γενικεύει το ίδιο pattern σε όλα. Ζητήθηκε 2026-09-19.
- [x] **PDF completion (S-M) — SHIPPED:** `PdfLabels` → statement + receipt (live customer language, `forCustomer`) + delivery-note (FROZEN στη snapshotted `recipient_country`, + localized pager + defensive `$L` default για direct `view()`). CMR σκόπιμα αγγλικό (διεθνής standard φόρμα). el byte-verbatim· test `PdfLocaleTest`. Λεπτομέρειες → CHANGELOG/FEATURES.
  - [ ] P2: τα ledger event-type labels (`CustomerLedgerBuilder::eventTypeLabel`) + τα ΔΑ audit event/action labels (`$e->typeLabel()`/`$m->actionLabel()`) + τα myDATA DeliveryCodes (σκοπός/τρόπος/μονάδα) μένουν ελληνικά σε en/both PDF — content/code labels, όχι chrome (όπως οι §8.3 citations). Localize αν χρειαστεί foreign audience (μεγαλύτερο: αγγίζει model methods κοινές με το panel).
  - [ ] P2 (cosmetic, ΚΟΙΝΟ invoice+ΔΑ): σε `both` PDF ο canvas pager βγάζει interleaved «Σελίδα / Page X από / of Y» (concat δύο 'both' labels). Ίδιο pattern στο `InvoicePdfRenderer::drawPager` — μην αποκλίνει μόνο το ΔΑ· φτιάξ' τα μαζί (per-language line ή el-only pager) αν ενοχλήσει.
- [x] **Stamp-at-issue (S) — SHIPPED:** νέος `CustomerLanguage::stampForCustomer()` παγώνει τη **ρητή** προτίμηση του πελάτη (`customers.language` → el/en/both, αλλιώς null) πάνω στο έγγραφο στα σημεία έκδοσης, όταν το «Γλώσσα PDF» είναι auto. **Απόφαση:** stamp μόνο τη ρητή προτίμηση (ΟΧΙ όλο το `forCustomer`) — η country/company resolution είναι ήδη frozen μέσω της snapshotted `country` του εγγράφου, οπότε το stamp προσθέτει μόνο το explicit override· κρατά `null=auto` (κανένα churn `null→el` σε κάθε ελληνικό doc). Σημεία: CreateInvoice/CreateQuote (σέβονται operator override), StageServiceRenewal + WHMCS draft (από πελάτη), IssueCreditNote/ConvertQuoteToInvoice/ReissueInvoiceAsDraft (inherit frozen source). ETL imports εξαιρούνται (ιστορικά). Test `LanguageStampTest`. Λεπτομέρειες → CHANGELOG.
  - [ ] P2 (edge): country-less πελάτης non-GR-default tenant χωρίς ρητή προτίμηση → το doc `country` γίνεται default 'GR' στα create sites → 'el' (αγνοεί το `default_language` του tenant). Σπάνιο (οι πελάτες έχουν χώρα)· αν ενοχλήσει, resolve το company-default tier στο stamp ή στο PdfLabels null-resolution.
...
_(Το transport half tracked και παρακάτω: «TIER 4 — PEPPOL Phase 2 + EE Access-Point» + «🔵 Big features → PEPPOL Phase 2» — ίδια parked-2027 απόφαση.)_
...
→ βλ. **«Dunning ladder»** (Master priority index **TIER 2** + ενότητα **«🧾 ERP-parity ideas»** παρακάτω) — ίδιο
work, tracked εκεί (κλιμακωτές υπενθυμίσεις 3/7/15/30 ημ., templates ανά σκαλί, opt-out ανά πελάτη). Οι ειδικές
όψεις που έμεναν εδώ: **Φάση B (M)** ενέργεια→task με ημ/νία+υπεύθυνο στο ημερολόγιο + ιστορικό επαφών στην Καρτέλα·
**Φάση Γ (S)** κλιμάκωση +7/+15/+30 με το `send_customer_statement`· P2 δικό του `Update`-ability για το collection
metadata (τώρα κάτω από `View:AgedReceivables`).
...
- **Πυλώνας E — Support / Ticket system** _(NEW, TODO-eval — μας ξέφυγε από τον epic)._ WHMCS-style
  ...
  **Buy-vs-build (2026-09-06 eval):**
  ...
  - **Πλήρης ανάλυση + απόφαση → `docs/ticket-system.md`** (2 στρώματα: ticket domain + mail
  ...
  - **Συγκεκριμένο design (schema + state machine + mail flow + 2 UIs + PR breakdown) →
    `docs/ticket-system.md`** (build-our-own, thin 6-table domain, reuse των υπαρχόντων
...
- **Menu / Information Architecture — πριν πληθύνουν οι πυλώνες** _(NEW, epic-wide· ήδη πιεστικό)._
  **Πλήρης στόχος-χάρτης (κάθε σημερινό screen + μελλοντικό, mapped) → `docs/menu-ia.md`.** Το nav
  ...
  - **Επιβεβαίωση από το ίδιο το WHMCS** (screenshots 2026-09-06): χωρίζει **Configuration** (Support
    ...
    split. Λεπτομέρειες στο `docs/ticket-system.md` «WHMCS parity».
...
### Προτιμολόγιο — ανοιχτά σχεδιαστικά (2026-09-20)
Το προτιμολόγιο ΕΓΙΝΕ (PR #609: flag `offered_at`, ορατό+πληρωτέο στην πύλη, χρήση πίστωσης σε
πολλαπλά, widget + φίλτρο ορατότητας). Μένουν δύο **σχεδιαστικές** αποφάσεις — καμία δεν είναι bug.

**Α) Λήξη προσφοράς + cascade στην υπηρεσία (θέλει συζήτηση, ΟΧΙ απλό κουμπί).**
Το αρχικό πλαίσιο ήταν «κουμπί Απόρριψη για τον πελάτη». Ο operator το επαναδιατύπωσε σωστά: για
προτιμολόγιο **από υπηρεσία** δεν είναι κουμπί, είναι **μηχανή καταστάσεων με ημερομηνίες**.
Σενάριο αναφοράς:
- υπηρεσία 15-01-2025 → 15-01-2026
- προτιμολόγιο εκδίδεται ως **υπενθύμιση** ~7-8 ημέρες πριν (π.χ. 07-01-2026)
- **περίοδος χάριτος** ~1 εβδομάδα μετά τη λήξη (π.χ. έως 22-01-2026)
- 23-01-2026: το προτιμολόγιο **ακυρώνεται**, η υπηρεσία **suspended**
- +Χ ημέρες (π.χ. 20): **terminated**

Συνέπειες που πρέπει να λυθούν πριν γραφτεί κώδικας:
- Το προτιμολόγιο χρειάζεται **ημερομηνία λήξης** (`offer_expires_at`), όχι μόνο `offered_at`.
- Χρειάζεται **scheduled job** που λήγει προσφορές και προχωρά το συμβόλαιο (suspend → terminate).
- Η ακύρωση προσφοράς είναι **ακίνδυνη λογιστικά** (δεν είναι παραστατικό, τίποτα στην ΑΑΔΕ) — αυτός
  ήταν άλλωστε όλος ο λόγος ύπαρξης του προτιμολογίου.
- Τι γίνεται αν πληρωθεί **μέσα** στη χάρη αλλά μετά τη λήξη; Ξεπαγώνει η υπηρεσία;
- Το κουμπί «Δεν το χρειάζομαι» του πελάτη γίνεται **επιτάχυνση** αυτής της μηχανής (άμεση λήξη), όχι
  ξεχωριστό χαρακτηριστικό — και τότε αξίζει, γιατί σου λέει «απέρριψε» αντί να περιμένεις τη χάρη.

**Β) «Εισερχόμενα από Πύλη» — η πολιτική έκδοσης μετά την είσπραξη.**
Η ιδέα του operator: όπως υπάρχει είσοδος από **WHMCS**, να υπάρχει είσοδος από **Πύλη**, και ανάλογα
με την περίπτωση ο operator αποφασίζει: **Τιμολόγιο / Απόδειξη / Ακύρωση / Αρχειοθέτηση** (το τελευταίο
για εσωτερικά ή φιλικά, ακριβώς όπως ήδη γίνεται στο WHMCS inbox).

Γιατί ταιριάζει: κρατά τη ρητή αρχή «τα παραστατικά είναι νομικά σημαντικά — draft-first, ΟΧΙ
auto-issuing», και δίνει την ουρά απόφασης που λείπει.

**ΣΗΜΑΝΤΙΚΗ ΔΙΑΦΟΡΑ από το WHMCS inbox — μην αντιγραφεί τυφλά:** εκεί τα δεδομένα είναι **εξωτερικά**
και σταδιοποιούνται σε `pending_whmcs_invoices` ΠΡΙΝ γίνουν παραστατικό. Εδώ το έγγραφο **υπάρχει ήδη**
ως draft· δεν χρειάζεται δεύτερος πίνακας staging. Χρειάζεται **επιφάνεια απόφασης** πάνω στα
πληρωμένα προτιμολόγια — το φίλτρο «Προτιμολόγια → Πληρωμένα» και το widget είναι ήδη η αρχή της·
λείπουν οι ενέργειες (έκδοση ως ΤΠΥ / ως ΑΠΥ ανάλογα με τον αντισυμβαλλόμενο, ακύρωση, αρχειοθέτηση).

**ΕΚΚΡΕΜΕΙ ΓΙΑ ΤΟΝ ΛΟΓΙΣΤΗ (μπλοκάρει το Β):** πόσος χρόνος επιτρέπεται από την **είσπραξη** μέχρι
την **έκδοση** του παραστατικού, όταν εισπράττεται προκαταβολικά μέσω προτιμολογίου; Αν η απάντηση
είναι «με την είσπραξη», η ουρά περιορίζεται σε λεπτά και το auto-issue γίνεται σχεδόν αναπόφευκτο·
αν δίνεται περιθώριο (τέλος μήνα / με την ολοκλήρωση), η ουρά είναι η σωστή λύση.

**Γ) Ειδοποίηση στη χρήση πίστωσης — ΑΠΟΡΡΙΦΘΗΚΕ (σωστά).**
Προτάθηκε καμπανάκι όταν ο πελάτης εξοφλεί με πίστωση (ασυμμετρία με το gateway, που ειδοποιεί).
Απορρίφθηκε από τον operator με σωστό σκεπτικό, επαληθευμένο: το «Ιστορικό» καταγράφει ήδη
**causer = ο ίδιος ο πελάτης** (όχι «Σύστημα»), το split της on-account γραμμής, τη νέα δεμένη γραμμή
με `invoice_id`, και το `ΕΦΑ-` reference τα ομαδοποιεί στην Καρτέλα. Η ουσιαστική διαφορά: η πληρωμή
μέσω gateway είναι **νέο χρήμα που φτάνει απαρατήρητο** (γι' αυτό χτυπά καμπανάκι)· η χρήση πίστωσης
είναι **χρήμα που ήδη έφτασε και μετακινείται** — δεν υπάρχει νέο γεγονός να μαθευτεί.
