...
- **`InvoSignTransport::cancel()` μόνο για 9.3** — ποστάρει πάντα στο `iNVOSign_CancelDeliveryNote.php`, που ακυρώνει ΜΟΝΟ δελτία αποστολής 9.3· ένα 2.1/11.x επιστρέφει `[283]` (και το direct-AADE cancel `[249]` «posted by provider»). _By design η ακύρωση provider-2.1 είναι αδύνατη — reversal = πιστωτικό 5.1, και το UI **ήδη** το γκρεϊτάρει σε 9.3-only (`ViewInvoice::cancel_at_mydata`)._ Gap: αν κάποιο μη-UI μονοπάτι (π.χ. `GrProviderSubmitter::cancel()` από automation/bulk) κληθεί για 2.1, βγάζει opaque `[283]` αντί για καθαρό guard «provider-2.1 δεν ακυρώνεται — έκδοσε πιστωτικό». Fix: pre-check τύπου στο `GrProviderSubmitter::cancel()`/transport (9.3 → CancelDeliveryNote· αλλιώς hard-refuse με το μήνυμα του credit-note). Sandbox-observed 2026-07-07 (βλ. `mydata-sandbox-myd2-retry-2026-07-07.md`).
