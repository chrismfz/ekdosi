...
**Verdict: ✅ μικρά αλλά πραγματικά (S).**
- [ ] **Πληρωμή με κενό ποσό (#377):** `payments.amount` nullable + το ETL περνά 0/NULL παρακάμπτοντας τον guard
  (`TenantRowUpserter` raw insert, MON-8 bypass) → κενή γραμμή. Data fix + validation.
- [ ] **Ρητό tiebreak** στο sort της Καρτέλας (`(ημ/νία, created_at, τύπος, id)`) — όλες οι imported πληρωμές έχουν
  ίδιο `created_at` (14:29 artifact)· σήμερα σωστό λόγω stable-sort, αλλά εύθραυστο (cosmetic)
- [ ] Γραμμή «Υπόλοιπο από μεταφορά» σε φιλτραρισμένη περίοδο (τώρα σημείωση αντ' αυτού)
