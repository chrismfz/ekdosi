...
- [x] **OPS-7 · MEDIUM · ΕΠΙΒΕΒΑΙΩΜΕΝΟ μηχανισμός — ✅ FIXED 2026-07-05** (`db-snapshot` με `--add-drop-database --databases` → clean-slate restore που ρίχνει και ορφανό πίνακα κακού migration· το snapshot μεταφέρθηκε ΜΕΤΑ το `artisan down` — snapshot-failure = clean abort με `up`+worker restart) — restore χωρίς schema reset σκότωνε το επόμενο deploy («table already exists»)· snapshot πριν το `down` = παράθυρο χαμένων writes.
