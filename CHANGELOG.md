...
- **Παραστατικό: χειροκίνητη «Αποστολή σε συστήσαντα/άλλον».** Νέο κουμπί μέσα στο παραστατικό που
  στέλνει το PDF του **εκδοθέντος** παραστατικού σε **άλλον** παραλήπτη — τυπικά τον reseller/agency
  που **σύστησε** τον πελάτη (`referred_by_customer_id`, «ήρθε από …», όπως το lead→convert). Εμφανίζεται
  **μόνο** όταν υπάρχει συστήσας με email (αλλιώς δεν εμφανίζεται, δεν φορτώνει τα απλά παραστατικά),
  είναι **προσυμπληρωμένο** με το email του συστήσαντα αλλά **επεξεργάσιμο** (custom παραλήπτης), υπό
  πλήρη χειροκίνητο έλεγχο του operator. Χρησιμοποιεί την **ίδια** μηχανή `SendInvoiceEmail` μέσω νέου
  recipient override: αυτό το targeted αντίγραφο πάει **μόνο** στον παραλήπτη (το CC λογιστή του πελάτη
  παραλείπεται· το audit BCC του tenant μένει) και καταγράφεται κανονικά στο «Ιστορικό αποστολών».
...
### Changed
- **«Αποστολή PDF στον πελάτη» — προαιρετικό custom email παραλήπτη πάνω στο ίδιο κουμπί.** Το modal
  αποστολής απέκτησε προαιρετικό πεδίο «Αποστολή σε άλλο email»: **κενό → στον πελάτη** (η κανονική ροή,
  με το CC δευτερεύοντος email του), **συμπληρωμένο → στοχευμένο αντίγραφο** σε custom διεύθυνση (π.χ.
  λογιστή) χωρίς CC στον πελάτη. Πλέον ορατό σε **κάθε** εκδοθέν παραστατικό (και όταν ο πελάτης δεν
  έχει καταχωρημένο email — τότε το πεδίο απαιτείται), ώστε το custom send να είναι πάντα προσβάσιμο και
  όχι μόνο στο κρυφό «Αποστολή σε συστήσαντα/άλλον». Ίδια μηχανή `SendInvoiceEmail` recipient override
  (targeted αντίγραφο, audit BCC κρατείται, παραλήπτης στο log). Το «Αποστολή σε συστήσαντα/άλλον»
  παραμένει ως shortcut με προσυμπλήρωση του συστήσαντα.
