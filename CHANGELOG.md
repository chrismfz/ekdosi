...
- **Δομημένη γραμμή log σε κάθε επιτυχή υποβολή (OBS-001 tail).** Οι submitters λογάριζαν μόνο
  ...
  από το ένα success choke-point κάθε submitter (απευθείας myDATA + πάροχος): invcode **και** ΜΑΡΚ στο
  μήνυμα (grep σε οποιοδήποτε), κανάλι/τύπος/διάρκεια στο context. Ποτέ δεν πετάει — ένα log δεν χαλάει
  υποβολή που ήδη πέτυχε.
