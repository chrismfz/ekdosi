...
### Added
- **«Άμεση τιμολόγηση» — αρχικοποίηση από το WHMCS πεδίο «γκρινιάρης» ΜΟΝΟ στη δημιουργία πελάτη.** Όταν ο
  χειριστής δημιουργεί έναν πελάτη από ένα σταδιοποιημένο WHMCS invoice («Δημ. πελάτη»/«Εισαγωγή από ΑΦΜ»),
  το `customers.needs_immediate_invoice` **προ-συμπληρώνεται** από το mapped WHMCS custom field `griniaris`
  (ίδιο truthy set με το `wantsinvoice`). Απόφαση **(γ) «το WHMCS σπέρνει, ο χειριστής κρατά τον έλεγχο»**:
  γίνεται **μόνο στη δημιουργία** — ένας ήδη υπάρχων πελάτης **ΠΟΤΕ** δεν αλλάζει flag από re-sync (ο χειριστής
  τον κατέχει). Απόντο/ξεμαρκαρισμένο πεδίο → flag μένει OFF (ποτέ δεν το εξαναγκάζει). Αναβιώνει scaffolding
  που ως τώρα δεν διάβαζε κανείς (`needs_immediate_invoice` + `whmcs_custom_field_map.griniaris`).
