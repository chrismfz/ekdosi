...
- **«Άμεση τιμολόγηση» — συγχρονισμός από το WHMCS πεδίο «γκρινιάρης» (WHMCS = πηγή αλήθειας).** Το
  ...
  μη «χάνεται» μια χειροκίνητη αλλαγή στο επόμενο sync. Αναβιώνει scaffolding που ως τώρα δεν διάβαζε κανείς
  (`needs_immediate_invoice` + `whmcs_custom_field_map.griniaris`).
