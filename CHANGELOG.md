...
- **Έλεγχος αιτίας απαλλαγής ΦΠΑ (§8.3) ↔ τύπου παραστατικού.** Μια 0% γραμμή με αιτία που δεν στέκει ποτέ για τον
  ...
  δέχεται τέτοιο παραστατικό, άρα η λάθος νομική αιτία περνούσε σιωπηλά (MYD-007). Αλλαγή τύπου στη φόρμα ξαναβγάζει
  την αιτία των 0% γραμμών (όσες πρότεινε ο παλιός τύπος ή δεν στέκουν στον νέο). ⚠ Tenant με 0% κατηγορία ΦΠΑ σε αιτία
  16 (παλιά προεπιλογή): τα 2.2 χωρίς αιτία γραμμής (π.χ. από WHMCS) θα σταματήσουν — το `mydata:preflight` /
  «Έλεγχος ρυθμίσεων» το δείχνει πλέον ως προειδοποίηση· όρισε την κατηγορία σε 4.
...
### Fixed
- **WHMCS auto-issue: ο έλεγχος 0% γραμμών πριν την αποθήκευση παρακαμπτόταν για tenants μέσω παρόχου.** Έτρεχε μόνο
  αν `mydata_mode` ≠ off, όμως ο πάροχος διαβιβάζει με `mydata_mode` off — ένα 0% χωρίς (ή με αντιφατική) αιτία
  δημιουργούσε το παραστατικό και αποτύγχανε μετά στη διαβίβαση. Κριτήριο πλέον: «διαβιβάζει πραγματικά» (submitter factory).
