...
- **`ekdosi:go-live-check`: τέλος το false-green στον πάροχο.** Το gate έλεγχε μόνο `empty($key)`, οπότε
  έγραφε **pass** για κλειδί που δεν αντιστοιχεί σε κανένα transport (→ `NullProviderTransport`, δηλαδή
  tenant που δεν μπορεί να υποβάλει καθόλου). Πλέον επαληθεύει ότι το κλειδί ΛΥΝΕΤΑΙ σε ενεργό transport.
...
- **Η καρτέλα «Πάροχος (ΥΠΑΗΕΣ)» λέει πλέον αν το κάθε στοιχείο ΕΙΝΑΙ αποθηκευμένο.** Ένα μασκαρισμένο
  ...
  με τα 4 τελευταία ψηφία (ώστε να ξεχωρίζουν δύο tokens χωρίς να αποκαλύπτεται κανένα· ≤4 χαρακτήρες →
  πλήρης μάσκα) ή «⚠ Δεν έχει αποθηκευτεί».
