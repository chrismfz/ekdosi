...
### Changed
- **«Το ημερολόγιό μου» (ICS):** επιλογή «Όλα τα leads της εταιρείας» (με τον χειριστή) — ενεργή από προεπιλογή για admins σε **νέα** links (τα υπάρχοντα δεν διευρύνονται σιωπηλά)·
  οι εγκρίνοντες βλέπουν και τις **εκκρεμείς** άδειες όλων («Άδεια (σε αναμονή) — όνομα»· ποτέ το είδος). Δεύτερος δρόμος:
  «📅 Βάλε αυτό το ημερολόγιο στο κινητό σου» κάτω από το Ημερολόγιο αδειών και το Ημερολόγιο leads.

### Added
- **«Το ημερολόγιό μου» (ICS)** — προσωπικό link συνδρομής, μόνο ανάγνωση, για Thunderbird / Google Calendar / κινητό: τα
  «επόμενα βήματα» των leads μου, οι άδειές μου (με είδος), ποιοι συνάδελφοι λείπουν (μόνο «Άδεια» — το είδος δεν βγαίνει
  ποτέ σε εξωτερικό ημερολόγιο), αργίες, οι υπερωρίες μου. Κουμπί «Ημερολόγιο στο κινητό / Thunderbird» στο Ημερολόγιο αδειών
  και στο Ημερολόγιο leads: επιλογές περιεχομένου, «Νέο link», «Ανάκληση». Τα δικαιώματα ελέγχονται σε κάθε ανάγνωση.

### Fixed
- **«Το ημερολόγιό μου» (ICS):** στο πρώτο άνοιγμα (νέο link) οι επιλογές έδειχναν όλες κλειστές (το feed είχε τις σωστές
  προεπιλογές στη βάση) — ένα «Αποθήκευση επιλογών» από εκεί τις έκλεινε όλες. Τώρα δείχνουν τις πραγματικές προεπιλογές.

### Added
- **Φύλακας προσωπικού** (στο εβδομαδιαίο `ergani:watch`, μόνο ανάγνωση `EX_BASE_05` από την Παραγωγή): συγκρίνει το δυναμικό
  του ΕΡΓΑΝΗ με τους «Εργαζόμενους» — νέοι στο ΕΡΓΑΝΗ, ενεργοί εκεί αλλά ανενεργοί/διαγραμμένοι εδώ, ενεργοί εδώ αλλά όχι εκεί,
  χωρίς ΑΦΜ. Αποθηκεύεται, εμφανίζεται στο «Θέλει προσοχή» της κάρτας ΕΡΓΑΝΗ, καμπανάκι στους admins **μόνο όταν αλλάξει**
  η εικόνα. Δεν αλλάζει κανέναν αυτόματα. Οι δύο έλεγχοι του φύλακα (κάρτα / προσωπικό) τρέχουν ανεξάρτητα.

### Added
- **«Ιστορικό ΕΡΓΑΝΗ»** (Προσωπικό, admins): κάθε δήλωση/ανάκληση που έκανε το ekdosi — πότε, ποιος, τι, εργαζόμενος (link),
  περιβάλλον, αποτέλεσμα, πρωτόκολλο, «Λεπτομέρειες» (τι στάλθηκε/τι απάντησε) και το **επίσημο PDF** του ΕΡΓΑΝΗ. Μόνο ανάγνωση.
- **PDF ΕΡΓΑΝΗ**: συνημμένο στο email του λογιστή για κάθε δήλωση **Παραγωγής** (άδεια/υπερωρία)· ο εργαζόμενος κατεβάζει
  το PDF της δικής του δηλωμένης άδειας.
- **Οδηγός «Πέρασμα σε Παραγωγή»** (Εταιρεία → ΕΡΓΑΝΗ): το περιβάλλον δεν αλλάζει πια από απλό πεδίο· ο οδηγός ελέγχει
  σύνδεση Παραγωγής (ΑΦΜ), ΑΦΜ όλων των ενεργών εργαζομένων, δείχνει μελλοντικά δηλωμένα μόνο στο δοκιμαστικό, ζητά
  υποχρεωτικά «ο λογιστής ενημερώθηκε» και στέλνει (προαιρετικά) email στον λογιστή· καταγράφει πότε/ποιος. «Επιστροφή σε Δοκιμαστικό».

### Changed
- **Επαναφορά εταιρείας από bundle:** το ΕΡΓΑΝΗ επανέρχεται πάντα σε **Δοκιμαστικό** (πραγματικές δηλώσεις μόνο μετά τον
  οδηγό, στο νέο VM)· το «ποιος/πότε πέρασε σε Παραγωγή» δεν μεταφέρεται.

### Added
- **«Η ομάδα σήμερα»** στο dashboard: ποιος λείπει σήμερα και την υπόλοιπη εβδομάδα (οι συνάδελφοι βλέπουν ποιος, όχι το
  είδος), «Περιμένουν έγκριση» με link (εγκρίνοντες) και «Μέσα τώρα» από την κάρτα όταν χρησιμοποιείται.
- **«Το υπόλοιπό μου»** στη φόρμα άδειας: «Μένουν 10 από 20 → με αυτή την αίτηση 7», ανά έτος όταν περνά την Πρωτοχρονιά·
  σήμανση όταν ξεπερνά το υπόλοιπο (δεν μπλοκάρει — αποφασίζει ο εγκρίνων).

### Changed
- **Προτάσεις σύνδεσης λογαριασμών:** ταιριάζουν και ελληνικά ↔ greeklish («Αντιβαλιδης Ηλιας» ↔ «Ilias Antivalidis»).
- «Μέσα τώρα» (dashboard + tablet): είσοδος που δεν είναι σημερινή δείχνει και ημερομηνία (π.χ. «25/09 21:53»)· «Υπερωρίες»:
  δικό τους εικονίδιο, και η κενή σελίδα εξηγεί πώς ενεργοποιείται η δήλωση όταν είναι κλειστή.

### Added
- **ΕΡΓΑΝΗ — ορατότητα:** κάρτα «ΕΡΓΑΝΗ» στο dashboard (admins): περιβάλλον, τι δηλώνεται αυτόματα, τελευταία επιτυχία,
  «υπόχρεη κάρτας», **«Θέλει προσοχή»** (αποτυχημένες/αβέβαιες δηλώσεις αδειών/κάρτας/υπερωριών, εγκεκριμένη άδεια που
  ξεκινά έως μεθαύριο χωρίς δήλωση — με links) και οι υπερωρίες σήμερα/αύριο. **Υπερωρίες στο ημερολόγιο αδειών** («+2ω»,
  «!» όταν δεν δηλώθηκε — για admins και τον ίδιο). Μπάρα **«ΔΟΚΙΜΑΣΤΙΚΟ ΕΡΓΑΝΗ»** στις σελίδες που δηλώνουν.
- **Φύλακας ψηφιακής κάρτας** (`ergani:watch`, εβδομαδιαίο, μόνο ανάγνωση από την Παραγωγή): αποθηκεύει το
  `IsInCardSector` κάθε εταιρείας και ειδοποιεί τους admins την ημέρα που θα γίνει «ναι». Διακόπτης στο «Χρονοπρογραμματισμός».

### Added
- **Υπερωρίες → ΕΡΓΑΝΗ (`WTOOv`):** σελίδα «Υπερωρίες» (company_admin) με «Νέα υπερωρία» (εργαζόμενος, ημέρα, από–έως)
  που δηλώνεται αμέσως στο ΕΡΓΑΝΗ — **μόνο πριν ξεκινήσει** (το ΕΡΓΑΝΗ απορρίπτει την εκπρόθεσμη), χωρίς επικαλύψεις, δεν
  ανακαλείται. Email στον λογιστή για τη μισθοδοσία (και όταν ΔΕΝ δηλώθηκε), καμπανάκι στους admins σε αποτυχία, επανάληψη
  με επιβεβαίωση στο «Αβέβαιο». Opt-in «Δήλωση υπερωριών στο ΕΡΓΑΝΗ» στην καρτέλα ΕΡΓΑΝΗ. Επαληθεύτηκε στο trial.

### Fixed
- **ΕΡΓΑΝΗ lookups με παραμέτρους** (`ErganiClient::service`) έστελναν `{Name, Value}` αντί για το επαληθευμένο
  `{ParameterName, ParameterValue}` (δεν είχε φανεί — το `EX_BASE_05` καλείται χωρίς παραμέτρους).
- **Υπόλοιπο κανονικής άδειας:** άδεια που διασχίζει την Πρωτοχρονιά (π.χ. 29/12–5/1) χρεωνόταν ολόκληρη στο έτος έναρξης·
  τώρα μοιράζεται στα δύο έτη ανά εργάσιμες ημέρες (και όταν ο εγκρίνων άλλαξε τις ημέρες με το χέρι, τα μέρη αθροίζουν σωστά).

## [2.7.0] - 2026-09-26

### Added
- **«Εισαγωγή από ΕΡΓΑΝΗ»** στους Εργαζόμενους: διαβάζει το τρέχον δυναμικό (`EX_BASE_05`, πάντα από την Παραγωγή —
  μόνο ανάγνωση), δείχνει Νέος / Υπάρχει / Διαγραμμένος + «ενεργοί εδώ αλλά όχι στο ΕΡΓΑΝΗ» και προσθέτει τους
  επιλεγμένους. Κρατά ΜΟΝΟ ΑΦΜ, ονοματεπώνυμο, παράρτημα, ημ. πρόσληψης· δεν αλλάζει ονόματα υπαρχόντων, δεν διαγράφει.
- **Εργαζόμενοι ↔ λογαριασμοί:** η στήλη «Λογαριασμός» δείχνει όνομα + email (ποιος είναι ποιος) και «⚠ χωρίς ρόλο για
  άδειες» όταν ο χρήστης δεν μπορεί να ζητήσει άδεια· για τους ασύνδετους «Πρόταση: …» (ίδιο email, αλλιώς ίδιο
  ονοματεπώνυμο — σειρά/τόνοι/κεφαλαία αδιάφορα, μόνο μοναδικό ταίριασμα) με «Σύνδεση» ανά γραμμή ή μαζικά.
- **Προσωπικό — ευκολία χρήσης:** checklist «Ξεκίνημα Προσωπικού» στους Εργαζόμενους (παραμένει, διπλωμένο όταν ολοκληρωθεί· τι λείπει: ΕΡΓΑΝΗ, email λογιστή,
  εργαζόμενοι, λογαριασμοί, PIN, tablet)· «Ξεκλείδωμα PIN» + νέο PIN καθαρίζει το κλείδωμα· στήλες Κάρτα/PIN· ενότητα
  «Προσωπικό — ΕΡΓΑΝΗ» στις Ρυθμίσεις εταιρείας (email λογιστή, κάρτα μόνο στο γραφείο) για τον company_admin· έγκριση/
  απόρριψη άδειας κατευθείαν στη γραμμή· «Είστε ΜΕΣΑ από …/ΕΚΤΟΣ» + οδηγίες στο «Χτύπημα κάρτας»· μήνυμα «QR έληξε»·
  tablet: κλείσιμο PIN μετά από 30'' αδράνειας, «Καταχώριση», «Εκτός γραφείου»· βήματα ενεργοποίησης tablet.
- **ΕΡΓΑΝΗ Φάση 3 — Ψηφιακή Κάρτα Εργασίας (`WRKCardSE`):** σελίδα «Κάρτα εργασίας» για τον υπάλληλο (ένα κουμπί
  Είσοδος/Έξοδος — operator ή ρόλος `ergani`), «Σημείο κάρτας (QR)» για tablet γραφείου (QR που αλλάζει κάθε 30'' =
  απόδειξη παρουσίας· προαιρετικά «μόνο μέσω QR»), «Κάρτες εργασίας» για τον admin (λίστα, επανάληψη, εκπρόθεσμη
  υποβολή με αιτιολογία, χειροκίνητη κίνηση). Opt-in «Αυτόματη δήλωση ψηφιακής κάρτας» + ένδειξη «Ψηφιακή κάρτα» ανά
  εργαζόμενο. Ίδιες ασφάλειες με τις άδειες (claim, «Άγνωστο», audit)· διπλό χτύπημα < 1' απορρίπτεται· έξοδος μετά
  τα μεσάνυχτα κρατά την ημέρα της εισόδου. Επαληθεύτηκε live στο δοκιμαστικό ΕΡΓΑΝΗ.
- **«Ρολόι» στο tablet του γραφείου (χωρίς κινητό, χωρίς login):** `/card-kiosk` σε ενεργοποιημένη συσκευή δείχνει
  **πίνακα παρουσίας** (ποιος είναι μέσα, από πότε) + όνομα → **PIN** → Είσοδος/Έξοδος (+ QR για κινητά). Συσκευές
  **ανά tablet** (όνομα, τελευταία φορά, IP, ανάκληση μία-μία / όλες), token μόνο ως hash, cookie με κυλιόμενη διάρκεια.
  PIN 4–6 ψηφίων (hash, ορίζεται στην καρτέλα εργαζομένου)· κλείδωμα που διπλασιάζεται, παύση ανά tablet σε πολλά
  λάθη (+ ειδοποίηση διαχειριστών), ξεχωριστά rate limits προβολής/χτυπήματος.

### Added
- **ΕΡΓΑΝΗ Φάση 2 — αυτόματη δήλωση αδειών:** με τον διακόπτη «Αυτόματη δήλωση εγκεκριμένων αδειών» (καρτέλα ΕΡΓΑΝΗ), κάθε
  έγκριση δηλώνεται στο ΕΡΓΑΝΗ ΙΙ (`WTOLeave`, μία εγγραφή ανά εργάσιμη ημέρα) και κάθε ανάκληση ανακαλείται
  (`CancelSubmittedDocument`, στο περιβάλλον όπου δηλώθηκε). Πρωτόκολλο/περιβάλλον/σφάλμα στην άδεια, **PDF ΕΡΓΑΝΗ**,
  επανάληψη δήλωσης/ανάκλησης, append-only ιστορικό κλήσεων (`ergani_submissions`), token cache (όχι login ανά κλήση — 429).
  Το email λογιστή αναφέρει το πρωτόκολλο («δεν χρειάζεται νέα δήλωση» στην παραγωγή· στο δοκιμαστικό ζητά κανονική δήλωση).
  Ασφάλειες κατά διπλής δήλωσης: atomic claim ανά άδεια, timeout/5xx = «Άγνωστο» (όχι «Αποτυχία» — επανάληψη μόνο με
  ρητή επιβεβαίωση ή **καταχώριση πρωτοκόλλου** που βρέθηκε στο ΕΡΓΑΝΗ), έλεγχος ημερών ↔ ημερομηνιών, το ιδιωτικό σχόλιο
  του εργαζομένου δεν στέλνεται. Επαληθεύτηκε end-to-end στο δοκιμαστικό ΕΡΓΑΝΗ (δήλωση → PDF → ανάκληση).

### Changed
- **Προσωπικό — ονόματα/κείμενα:** «Κάρτα εργασίας» → «Χτύπημα κάρτας» (κρυφό από όσους δεν είναι εργαζόμενοι),
  «Κάρτες εργασίας» → «Κινήσεις κάρτας», ρόλος «Προσωπικό (άδειες & κάρτα)», πηγή «Γραφείο (tablet/QR)» / «Κινητό /
  υπολογιστής», ΕΡΓΑΝΗ «Αβέβαιο — ελέγξτε στο ΕΡΓΑΝΗ» / «(δοκιμαστικό)». Η ενεργοποίηση tablet αποσυνδέει αυτόματα τον
  διαχειριστή. Ημερολόγιο: ΚΑΝ/ΑΣΘ/… ανά είδος, «•» για συναδέλφους. Προσωπικό βλέπει εξ ορισμού «Όλες» τις άδειές του.
  ΑΦΜ υποχρεωτικό για ψηφιακή κάρτα.
- **Προσωπικό → καρτέλα «ΕΡΓΑΝΗ» στην Εταιρεία** (super_admin, ίδιο μοτίβο με Υποστήριξη/Domains): διακόπτης
  on/off του πυλώνα (κρύβει άδειες/ημερολόγιο/εργαζόμενους/αργίες), email λογιστή (μεταφέρθηκε από τις «Ρυθμίσεις
  εταιρείας»), περιβάλλον **Δοκιμαστικό/Παραγωγή** + κωδικοί e-ΕΦΚΑ (κρυπτογραφημένα) + **«Test σύνδεσης»** (ΕΡΓΑΝΗ ΙΙ
  `EX_BASE_01`: επωνυμία/ΑΦΜ/υπόχρεος κάρτας). Εταιρείες που ήδη έχουν εργαζόμενους ενεργοποιούνται αυτόματα.

### Added
- **Προσωπικό / Άδειες (ΕΡΓΑΝΗ Φάση 1):** «Εργαζόμενοι», «Άδειες» (αίτημα → έγκριση/απόρριψη/ανάκληση, κωδικοί άδειας
  ΕΡΓΑΝΗ, εργάσιμες ημέρες αυτόματα), «Ημερολόγιο αδειών» (εργαζόμενοι × ημέρες, αργίες), «Τοπικές αργίες» ανά εταιρεία
  (εθνικές + Πάσχα υπολογίζονται), υπόλοιπο κανονικής άδειας, **email στον λογιστή** σε έγκριση/ανάκληση
  (`companies.leave_notify_email`), καμπανάκι σε εγκρίνοντες/αιτούντα. Έρευνα ΕΡΓΑΝΗ ΙΙ API → `docs/ergani/`.
- **Ρόλος `ergani` — «Προσωπικό (μόνο άδειες)»** για υπαλλήλους που δεν είναι χειριστές: βλέπουν ΜΟΝΟ άδειες/ημερολόγιο
  (default-deny middleware `RestrictErganiStaff`), όχι dashboard/Βοηθό/ειδοποιήσεις τιμολογίων. Οι χειριστές παίρνουν
  επίσης αίτημα άδειας + ημερολόγιο· έγκριση = company_admin/super_admin.

### Changed
- Οι tenant-wide ειδοποιήσεις (νέο WHMCS τιμολόγιο, πληρωμές, ληξιπρόθεσμα, tickets fallback, observer τιμολογίων)
  πάνε σε `ErganiStaff::staffRecipients()` αντί για όλους τους χρήστες της εταιρείας.

## [2.6.0] - 2026-09-25

### Added
- **Κατάλογος Συνδυασμένης Ονοματολογίας (ΣΟ) για το TARIC.** Νέος πίνακας `cn_codes` με τους **9.791 κωδικούς ΣΟ 2026** και
  ελληνικές περιγραφές (επίσημη ανοιχτή διανομή της ΕΕ, data.europa.eu, CC BY 4.0) — φορτώνεται αυτόματα στο migrate. Στη
  φόρμα είδους ο «Κωδικός TARIC / ΣΟ» γίνεται **αναζήτηση** (λέξεις ή κωδικός). Σελίδα **«Κωδικοί ΣΟ / TARIC»** (Είδη &
  Προμήθειες): φορτωμένο έτος, **είδη με καταργημένο κωδικό**, και (super_admin) **«Ενημέρωση από ΕΕ»** — κατεβάζει τη ΣΟ του
  έτους στο παρασκήνιο (queue, ειδοποίηση στο τέλος). CLI: `cn:import --year=YYYY [--file=…]`.

## [2.5.0] - 2026-09-25

### Added
- **Ψηφιακή Διακίνηση «ίδια μέσα» (Β' Φάση, 12/10/2026): «Παραδόθηκε (ίδιο όχημα)»** σε δελτίο 9.3 και ΤΔΑ — δήλωση
  αποτελέσματος (ConfirmDeliveryOutcome: πλήρης / αποτυχία) όταν ΕΜΕΙΣ κάναμε την «Έναρξη διακίνησης» (= μεταφορέας).
  Νέο πεδίο **«Μη υπόχρεος παραλήπτης»** (ιδιώτης / χωρίς ERP → header `nonObligatedRecipient`): τότε η πλήρης παράδοση
  κλείνει τη διακίνηση αμέσως· σε υπόχρεη επιχείρηση μένει **«Παραδόθηκε — αναμένεται ο παραλήπτης»** μέχρι να σκανάρει
  το QR. Φίλτρο **«Ανεπιβεβαίωτα (> 7 ημέρες)»** στα δελτία. Προειδοποίηση στο «Χωρίς ψηφιακή διακίνηση» (δεν είναι νόμιμη
  διέξοδος για B2B με δικό μας όχημα· λιανική με ΑΛΠ = χωρίς δελτίο). Sandbox 2026-09-25.

- **TARIC / Ενιαία Κωδικοποίηση Ειδών (υποχρεωτική 1/1/2027, Α.1094/2026):** πεδίο «Κωδικός TARIC / ΣΟ» στα είδη
  (δεκτά 8 ψηφία ΣΟ → +«00», ή 10 — το myDATA θέλει ακριβώς 10, [101]), snapshot `taric_code` + `item_code` (SKU) στις
  γραμμές τιμολογίων/δελτίων (κατά τη δημιουργία / αλλαγή είδους — ποτέ εκ των υστέρων), αποστολή `TaricNo`/`itemCode`
  μόνο σε **ΤΔΑ και 9.x** (§5.4 — ίδιος κανόνας με το itemDescr· όχι σε απλό τιμολόγιο). Sandbox: 9.3 + ΤΔΑ δεκτά.
- **Προγραμματισμένος «Έλεγχος κατάστασης» ψηφιακής διακίνησης** (`delivery:refresh-status`, κάθε 3 ώρες, default ON,
  read-only): ξαναδιαβάζει από την ΑΑΔΕ τα ανοιχτά δελτία/ΤΔΑ (εκδόθηκε / σε διακίνηση / αναμένεται ο παραλήπτης /
  επιστροφή) — φαίνεται μόνο του όταν ο πελάτης σκανάρει το QR. Ρυθμίζεται στον «Χρονοπρογραμματιστή».

### Changed
- **DeliveredByCarrier + FULL → «Παραδόθηκε — αναμένεται ο παραλήπτης»** (ήταν «Παραδόθηκε»): η ΑΑΔΕ ολοκληρώνει μόνο όταν
  ο υπόχρεος παραλήπτης επιβεβαιώσει (QR)· ο «Έλεγχος κατάστασης» το γυρίζει σε «Παραδόθηκε» όταν γίνει.
...
### Added
- **«Φορολογικά» (Λογιστικά): ό,τι χρωστάμε σε ΦΠΑ και φόρο εισοδήματος, σε μία σελίδα, ανά έτος.** ΦΠΑ ανά τρίμηνο + μήνα με
  μεταφορά του πιστωτικού στο επόμενο τρίμηνο· **εκτίμηση φόρου εισοδήματος** (κέρδος × συντελεστής, − παρακρατήσεις, +
  προκαταβολή επόμενου έτους, − περσινή προκαταβολή → υπόλοιπο + μηνιαία αποταμίευση + γραμμική προβολή 31/12)· έξοδα ανά
  κατηγορία (μισθοδοσία/ΕΦΚΑ/αποσβέσεις όπως τα περνά ο λογιστής, με «έως» ημ/νία)· πίνακας ανά έτος. «Φορολογικό προφίλ»
  (συντελεστής, % προκαταβολής, βεβαιωμένη προκαταβολή ανά έτος) σε `companies.income_tax_profile`. Perm `View:TaxOverview`
  → `shield:generate` + re-provision μετά το deploy.
- **«Φορολογικά»: για κλεισμένα έτη πηγή = το Ε3 της ΑΑΔΕ** (ο τελικός χαρακτηρισμός του λογιστή — μια μεταγενέστερη
  αναταξινόμηση, π.χ. 17.5 → αγορά παγίου, δεν φτάνει ποτέ στις τοπικές γραμμές). Στιγμιότυπο ανά έτος στον νέο πίνακα
  `e3_year_snapshots` (κουμπί «Ανανέωση Ε3 (ΑΑΔΕ)» ή `mydata:e3-snapshot --tenant=SLUG --year=…`)· έσοδα = category1_*
  (χωρίς τρίτων/πληροφοριακά), έξοδα = category2_* χωρίς αγορές παγίων. Τα τοπικά φαίνονται δίπλα ως διασταύρωση· το
  τρέχον έτος μένει τοπικό. Επαληθευμένο στη myip 2025 (−9.457,70, όπως ο λογιστής).
- **`mydata:import-expenses --tenant=SLUG --year=2023 --year=2024 [--only=suppliers|self]`** — συμπλήρωση εξόδων από το
  myDATA για ολόκληρα έτη, ανά τρίμηνο (με παύση, λόγω ορίου ΑΑΔΕ), με τις ίδιες idempotent υπηρεσίες της κονσόλας
  (προμηθευτών + δικά μας 13/14/17.x)· σε 429 σταματά κρατώντας ό,τι μπήκε — ξανατρέχει με ασφάλεια.
- **Κονσόλες myDATA: διάστημα «Προηγούμενο έτος»** — για συμπλήρωση εξόδων παλαιότερων ετών (π.χ. πριν ξεκινήσει η εισαγωγή από myDATA· αλλιώς το κέρδος στα «Φορολογικά» φουσκώνει).
- **MCP / Βοηθός `income_vs_expense`: `expense_breakdown`** — τα έξοδα ανά οικονομική κατηγορία (προμηθευτές / χειροκίνητα /
  μισθοδοσία / ΕΦΚΑ / αποσβέσεις / …) με `last_date`, ώστε να φαίνεται ως πότε έχει περάσει ο λογιστής τη μισθοδοσία.
- **«Μετατροπή σε φορολογικό» για άτυπα + αναφορά «Αξία άτυπων».** Ένα εκδομένο άτυπο (ΕΣΩ/ΔΟΚ — π.χ. ένας φίλος που
  τελικά πληρώνει, ένας νέος πελάτης που ζητά απόδειξη/τιμολόγιο) γίνεται **πρόχειρο** φορολογικό του είδους που
  διαλέγεις, με σημερινή ημερομηνία· το εκδίδεις κανονικά (πρώτα μπορείς να αλλάξεις πελάτη/ΑΦΜ). Το άτυπο μένει ως
  ίχνος («Μετατράπηκε σε…» / «Από άτυπο») και παγώνει· μία φορά (ξανά μόνο αν ακυρωθεί το φορολογικό). Η υπηρεσία δεν
  ξαναπροχωρά και το απόθεμα δεν ξανακινείται (μία παράδοση, μία κίνηση). Νέα σελίδα **Λογιστικά → «Αξία άτυπων»**:
  ό,τι δόθηκε χωρίς χρέωση ανά πελάτη/είδος/σειρά ανά έτος (τα μετατραπέντα χωριστά) + CSV ανά παραστατικό για τον
  λογιστή. *(Μετά το deploy: `shield:generate` + re-provision για το `View:InformalValueReport`.)*
- **Άτυπη (μη φορολογική) σειρά + προεπιλεγμένο είδος παραστατικού ανά πελάτη.** Νέα σημαία «Άτυπη σειρά» στο είδος
  παραστατικού (π.χ. «ΕΣΩ» για δικά μας εσωτερικά, «ΔΟΚ» για δοκιμές): τα έγγραφά της ακολουθούν κανονικά τη ροή
  (ανανέωση υπηρεσίας/domain, ημερομηνίες) αλλά δεν διαβιβάζονται ποτέ (myDATA/πάροχος) και δεν μετράνε σε καμία
  αναφορά χρημάτων/ΦΠΑ, υπόλοιπο, υπενθύμιση ή dunning. Αρίθμηση στην οριστικοποίηση σε κάθε tenant, PDF «ΑΤΥΠΟ», όχι
  email/πύλη/UBL/πιστωτικό/πληρωμή (η πληρωμή μπαίνει έναντι λογαριασμού· η μαζική υποβολή τα παραλείπει)· η σημαία κλειδώνει όταν η σειρά έχει οποιοδήποτε παραστατικό/δελτίο ή είναι προεπιλογή
  του WHMCS inbox, ένα αριθμημένο ή πληρωμένο παραστατικό δεν αλλάζει ποτέ από άτυπη σε φορολογική σειρά (ή
  ανάποδα), και ένα διαβιβασμένο (ΜΑΡΚ) ή πιστωτικό δεν μπαίνει ποτέ σε άτυπη σειρά (ούτε μέσω ορφανών). Ο πελάτης αποκτά «Προεπιλεγμένο είδος
  παραστατικού» που προ-συμπληρώνει νέα παραστατικά (και από την Καρτέλα) και υπηρεσίες, μαζί με τις προεπιλογές του
  είδους (π.χ. η εταιρεία μας → ΕΣΩ)· αλλαγή πελάτη αντικαθιστά μια προ-συμπληρωμένη σειρά (όχι μια που διάλεξε ο
  χειριστής). **Fixed** μαζί: η «Πλήρης εξόφληση» στην καρτέλα Πληρωμές έχει πλέον τους ίδιους φραγμούς με την
  «Καταχώριση πληρωμής» (όχι πρόχειρο/πιστωτικό/ακυρωμένο), και η φόρμα Πληρωμής δεν προσφέρει ακυρωμένα ή πιστωτικά.
- **Έλεγχος αιτίας απαλλαγής ΦΠΑ (§8.3) ↔ τύπου παραστατικού.** Μια 0% γραμμή με αιτία που δεν στέκει ποτέ για τον
  τύπο — 16 (άρθρο 45, εγχώρια αντιστροφή) σε ενδοκοινοτικό/τρίτης χώρας τύπο, ή 14 (άρθρο 33, ενδοκοιν. αγαθά) εκτός
  1.2 — δεν αποθηκεύεται στη φόρμα και δεν διαβιβάζεται (direct & πάροχος), με μήνυμα που λέει τη σωστή αιτία (π.χ.
  2.2→4). Τα ύποπτα (π.χ. 4 σε εγχώριο 1.1, 14 σε 2.2) δείχνουν προειδοποίηση στη γραμμή χωρίς να μπλοκάρουν. Η ΑΑΔΕ
  δέχεται τέτοιο παραστατικό, άρα η λάθος νομική αιτία περνούσε σιωπηλά (MYD-007). Αλλαγή τύπου στη φόρμα: μια 0%
  γραμμή χωρίς αιτία παίρνει την πρόταση του νέου τύπου, μια αιτία αδύνατη για τον νέο τύπο αντικαθίσταται/καθαρίζει,
  μια έγκυρη μένει (αν γίνεται ύποπτη, εμφανίζεται ειδοποίηση). ⚠ Tenant με 0% κατηγορία ΦΠΑ σε αιτία 16 (παλιά προεπιλογή): τα 2.2 χωρίς αιτία γραμμής (π.χ.
  WHMCS πρόχειρο που εκδίδεται χωρίς επεξεργασία) θα σταματήσουν με σαφές μήνυμα — όρισε την κατηγορία σε 4.
...
### Fixed
- **Εκτίμηση φόρου: οι αγορές παγίων (E3_882/883) δεν αφαιρούνται ως έξοδα της χρήσης** — εκπίπτουν μέσω αποσβέσεων.
  Το Βιβλίο Εσόδων-Εξόδων τις κρατά (καταχωρίζονται εκεί)· MCP `income_vs_expense` → `expense.capex_net`.
- **`VatPeriodReport::monthsOfQuarter()` με `CarbonImmutable` επέστρεφε τον ίδιο μήνα τρεις φορές** (ο δρομέας δεν
  προχωρούσε)· τώρα ο δρομέας ξανα-ανατίθεται και δουλεύει με mutable και immutable ημερομηνίες.
- **Σημειώσεις πελάτη: τα κουμπιά «Άνοιγμα / Επεξεργασία / Διαγραφή» ανά σημείωση εμφανίζονται ξανά.** Ήταν ορισμένα αλλά
  δεν τυπώνονταν ποτέ (`contentGrid` χωρίς layout → θέση «μετά το περιεχόμενο» που ο απλός πίνακας δεν αποδίδει).
- **Η εισαγωγή εταιρείας (bundle) με επανεκδομένο παραστατικό δεν αποτυγχάνει πια.** Ο σύνδεσμος «επανέκδοση από»
  δεν ξαναδενόταν στα νέα ids και η εισαγωγή σταματούσε σε παραβίαση FK· τώρα ξαναδένεται όπως τα πιστωτικά (το ίδιο
  και ο νέος σύνδεσμος «Από άτυπο»).
- **Η «Επανέκδοση» κρατά την αιτία απαλλαγής ΦΠΑ κάθε γραμμής.** Το αντίγραφο έχανε την αιτία §8.3 μιας γραμμής 0%
  (π.χ. «4 — ενδοκοινοτική υπηρεσία») και την κατηγορία εσόδων, οπότε μια επανέκδοση χωρίς διόρθωση πήγαινε στο
  myDATA με την προεπιλεγμένη 0% αιτία. Ο κρυφός χαρακτηρισμός §8.6 σκόπιμα δεν αντιγράφεται (ξαναϋπολογίζεται, ώστε
  να ισχύει ένα διορθωμένο mapping). Ένα κοινό σημείο αντιγραφής γραμμής (`InvoiceLine::copyAttributes()`) για
  επανέκδοση και μετατροπή· μια επανέκδοση μετατροπής κρατά και τον σύνδεσμο «Από άτυπο».
- **Ένα φορολογικό που εκδόθηκε με αριθμό στην οριστικοποίηση δεν γυρίζει πια σε πρόχειρο.** Σε tenant χωρίς
  διαβίβαση (π.χ. Nixpal) η οριστικοποίηση είναι η έκδοση· ως τώρα ένα εκδομένο (ήδη στον πελάτη/λογιστή) μπορούσε —
  με «Επαναφορά σε πρόχειρο» ή ακύρωση → «Επαναφορά» — να αλλάξει και να ξαναοριστικοποιηθεί με τον ίδιο αριθμό.
  Τώρα η πρώτη κρύβεται και η δεύτερη το φέρνει πίσω «Ενεργό». Διόρθωση = ακύρωση και νέο· το λέει και το μήνυμα της
  οριστικοποίησης. Ένα **άτυπο** (εσωτερικό) μένει ελεύθερο: επαναφορά, αλλαγή, νέα οριστικοποίηση με τον ίδιο
  αριθμό. Σε tenant που διαβιβάζει, ένα απορριφθέν μένει διορθώσιμο, αλλά όχι μια αποστολή «σε αμφιβολία» (μπορεί να
  έχει ήδη ΜΑΡΚ — η επανυποβολή το συμφιλιώνει).
- **Η επανέκδοση μιας παλιότερης ανανέωσης δεν προχωρά ξανά την υπηρεσία.** Αν ένα παραστατικό ανανέωσης γύριζε σε
  πρόχειρο (ή ακύρωση → «Επαναφορά») και οριστικοποιούνταν ξανά αφού είχε ήδη εκδοθεί η επόμενη ανανέωση, η «επόμενη
  χρέωση» της υπηρεσίας προχωρούσε μία περίοδο ακόμα — μια περίοδος δεν χρεωνόταν ποτέ. Τώρα μόνο η τελευταία
  ανανέωση την προχωρά.
...
### Changed
- **Υπηρεσίες: οι «Σημειώσεις» βγήκαν σε δική τους ενότητα** (όχι πια κρυμμένες στο κλειστό «Παροχή & χρονισμός»)
  και φαίνονται και στην **προβολή** της υπηρεσίας (όταν υπάρχουν), με τις αλλαγές γραμμής.
