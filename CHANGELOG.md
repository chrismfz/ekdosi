...
- **Post-review σκληραγώγηση των δύο παραπάνω (adversarial review, κανένα P0/P1):** (α) το dedup
  ...
  απόπειρα πλαστογραφίας στο «Log πύλης»· (η) σε αποτυχία digest το log ξεχωρίζει πλέον «λάθος
  σειρά πεδίων» από «λάθος shared secret» (ξαναϋπολογίζει το digest με τη σειρά που ήρθε — diagnostic
  μόνο, δεν εξουσιοδοτεί τίποτα), ώστε μία sandbox χρέωση να απαντά οριστικά· (θ) κάθε vPOS return γράφει πλέον
...
### Added
- **Αδέσποτα myDATA: «Σύνδεση» με υπάρχον ή «Καταχώριση τοπικά».** Στη σελίδα ΜΑΡΚ ενός αδέσποτου (υπάρχει στο
  myDATA, όχι τοπικά): λίστα πιθανών τοπικών παραστατικών χωρίς ΜΑΡΚ (σειρά/ΑΑ, ποσό, ΑΦΜ, ημερομηνία). **«Σύνδεση»**
  μόνο με το ίδιο παραστατικό (εκδοθέν, ίδια σειρά/ΑΑ, ημερομηνία, τύπος, αντισυμβαλλόμενος, ποσό) — γράφει ΜΑΡΚ/QR/
  κατάσταση όπως μια διαβίβαση, χωρίς επαναποστολή ή email· κάτι που απλώς μοιάζει δείχνεται ως πιθανή διπλή έκδοση.
  Αν δεν ταιριάζει κανένα, **«Καταχώριση τοπικά»**: δημιουργεί το παραστατικό όπως δηλώθηκε (σειρά/ΑΑ, ΜΑΡΚ, QR, μία
  γραμμή ανά γραμμή του myDATA με ΦΠΑ και χαρακτηρισμό Ε3 — χωρίς περιγραφές, το myDATA δεν τις κρατά), ως ήδη
  διαβιβασμένο ή ακυρωμένο· ο μετρητής της σειράς προχωρά ώστε ο ίδιος ΑΑ να μη δοθεί ξανά. Σημαδεύεται ως εισαγωγή
  (`invoices.origin`): χωρίς «Νέο υπόλοιπο» και χωρίς αυτόματες υπενθυμίσεις· ο τρόπος πληρωμής επιλέγεται ρητά. Πιστωτικά,
  παρακρατήσεις, άλλος τύπος myDATA και σύνολα που δεν συμφωνούν ακριβώς στο λεπτό απορρίπτονται με τον λόγο. Αντικαθιστά το placeholder κουμπί.
  **Μετά το deploy: `php artisan migrate`** (νέα στήλη `invoices.origin`).
- **«Ηλικίωση οφειλών» — εικόνα υπενθυμίσεων + «Υπενθύμιση τώρα».** Κάρτες: προς έγκριση, αποτυχίες, εστάλησαν (30 ημ.),
  εξοφλήθηκαν μετά από υπενθύμιση (90 ημ.)· και **«Δεν υπενθυμίζονται»** (πρόχειρα που δεν στάλθηκαν ποτέ, WHMCS, παλιά
  από τη μετάπτωση, πελάτες χωρίς email/opt-out) με λίστα των παραστατικών. Στήλη «Τελ. υπενθύμιση» ανά πελάτη και κουμπί
  **«Υπενθύμιση τώρα»**: διαλέγεις ποια ανοιχτά παραστατικά του πελάτη (τα ληξιπρόθεσμα προ-επιλεγμένα) παίρνουν email
  τώρα — δουλεύει και με τις αυτόματες κλειστές (όχι σε πελάτη χωρίς email / με opt-out, ούτε δεύτερη φορά την ίδια
  μέρα)· αντικαθιστά την αυτόματη που περίμενε έγκριση, και μετά η αυτόματη του ίδιου παραστατικού περιμένει 3 ημέρες.
  Κάρτα «Εκτός κριτηρίων» (έληγαν πριν το «από» / κάτω από το ελάχιστο). Μια υπενθύμιση τιμολογίου ζητά το πολύ ό,τι
  χρωστάει ο πελάτης συνολικά (έναντι/πίστωση που δεν έχει κατανεμηθεί αφαιρείται — 0 = καμία υπενθύμιση). Κανόνας για
  όλες: ποτέ δύο υπενθυμίσεις του ίδιου παραστατικού την ίδια μέρα ή ταυτόχρονα.
- **Υπενθυμίσεις πληρωμής (dunning) για τιμολόγια — στυλ WHMCS.** Ανά εταιρεία (Ρυθμίσεις εταιρείας → «Υπενθυμίσεις
  πληρωμής»): προαιρετική «πριν τη λήξη» + 1η / 2η / 3η (τελευταία) σε Χ ημέρες μετά τη λήξη, ελάχιστο υπόλοιπο,
  συνημμένο PDF, δικά σου κείμενα ανά βαθμίδα (placeholders· για πελάτες στη γλώσσα της εταιρείας), λειτουργία **προς έγκριση** (default) ή **αυτόματη**.
  Ένα email ανά παραστατικό, μόνο για δικά μας ανεξόφλητα επί πιστώσει τιμολόγια + προτιμολόγια που στάλθηκαν
  (όχι WHMCS / legacy / τοις μετρητοίς), opt-out ανά πελάτη. Νέα σελίδα **Λογιστικά → Υπενθυμίσεις** (τι στάλθηκε,
  τι απέτυχε, τι παραλείφθηκε και γιατί, «Επόμενες 14 ημ.», αποστολή/παράλειψη/προεπισκόπηση). Ημερήσια εκτέλεση
  `invoices:send-reminders [--tenant] [--dry-run]` (schedule `EKDOSI_SCHEDULE_INVOICE_REMINDERS`, 09:00). Ό,τι
  εξοφληθεί στο μεταξύ ακυρώνεται πριν φύγει· το «Από» ορίζει ημερομηνία ώστε η ενεργοποίηση να μη στείλει μαζικά
  για παλιές οφειλές. Ο operator βλέπει/στέλνει/παραλείπει (όχι ρυθμίσεις). **Μετά το deploy: `shield:generate` +
  re-provision ρόλων.**
- **Εισαγωγή CSV πελατών / προϊόντων / προμηθευτών.** Κουμπί «Εισαγωγή CSV» στις λίστες: αυτόματη αναγνώριση
  στηλών (ελληνικά/αγγλικά/δικό μας export), πρότυπο CSV, προεπισκόπηση ανά γραμμή πριν την εισαγωγή. Υπάρχουσες
  εγγραφές (ίδιο ΑΦΜ / κωδικός / barcode) συμπληρώνονται μόνο στα κενά. Έλεγχος ψηφίου ελέγχου ΑΦΜ (`Afm::greekChecksumOk`),
  ΦΠΑ μόνο από υπάρχουσα κατηγορία, αρχεία ελληνικού Excel (Windows-1253, `;`, «12,50», ΑΦΜ χωρίς το αρχικό 0).
  Γραμμές με σφάλμα παραλείπονται· σφάλμα βάσης = δεν γράφεται τίποτα.
- **Πελάτες: badge «Χωρίς ταυτότητα ΑΦΜ» + φίλτρο για parked εγγραφές.** Ένας legacy «υποκατάστημα» twin
  που μοιράζεται ΑΦΜ με τον κρατημένο πελάτη εισάγεται από το ETL **χωρίς ταυτότητα ΑΦΜ** (`afm_key` null,
  `afm_key_parked`). Μέχρι τώρα το parked state φαινόταν μόνο στο `customers:afm-duplicates`/stdout· τώρα
  υπάρχει **badge** στη λίστα (κενό για κανονικές γραμμές) + **φίλτρο** ώστε ο χειριστής να τους βρει και να
  τους διορθώσει/συγχωνεύσει μετά από μια εισαγωγή.
- **`ops:health`: επίμονη αποτυχία του `delivery:fetch-inbound` πλέον αναδεικνύεται.** Ο read-only 6ωρος
  poll σιωπά μια μεμονωμένη (transient) αποτυχία σκόπιμα, αλλά μια **επίμονη** (λάθος/ληγμένα creds → τα
  εισερχόμενα ΔΑ σταματούν να στέηζονται) μετρᾶται ανά tenant (consecutive-failures) και μετά από N στη
  σειρά (~24ω) γίνεται **error log + warning στο `ops:health`** (νέα ενότητα «Delivery inbound»). Ποτέ
  critical — το staging δεν είναι data-loss· έκδοση/reconcile ανεπηρέαστα.
- **Ρυθμίσεις εταιρείας (self-service): «Προεπιλεγμένη γλώσσα επικοινωνίας».** Το `default_language` (fallback
  γλώσσα email όταν ο πελάτης δεν έχει ρητή γλώσσα/χώρα) ήταν μόνο στο super-admin `CompanyResource`· μπήκε
  στο whitelist του `CompanySettings` ώστε να το ρυθμίζει ο company_admin (business identity, όχι credential).
- **Καρτέλα πελάτη: άνοιγμα + επεξεργασία σημείωσης inline από τη section «Σημειώσεις (εσωτερικές)».**
  Μέχρι τώρα η section έδειχνε μόνο excerpts + ένα γενικό link «Άνοιγμα σημειώσεων»· δεν μπορούσες να δεις μία
  σημείωση **ολόκληρη** ούτε να την επεξεργαστείς χωρίς να πας στη σελίδα Σημειώσεων ή στην «Επεξεργασία πελάτη».
  Πλέον κάθε σημείωση έχει **«Άνοιγμα»** (modal με το πλήρες κείμενο, markdown) και **«Επεξεργασία»** (inline, με το
  ΊΔΙΟ note-form/write-path της σελίδας Σημειώσεων — νέο shared `ManagesCustomerNotes`). Το «Επεξεργασία» κρύβεται
  σε imported («backup») σημειώσεις και χωρίς δικαίωμα ενημέρωσης· ο δείκτης της σημείωσης είναι client-supplied
  argument και επικυρώνεται tenant/customer-scoped (fail-closed 404) πριν από open/edit.
...
- **WHMCS εισερχόμενα «Τρίτος»: εισαγωγή δικαιούχου-τρίτου ως πελάτη ekdosi (ΑΑΔΕ) + ορατότητα στο popup.**
  ...
  **«Εισαγωγή τρίτων (ΑΑΔΕ)»** (στο «…») δημιουργεί/συνδέει τον ekdosi πελάτη για κάθε routed δικαιούχο μέσω
  του νέου `WhmcsCustomerCreator::createFromContact()` — **στοιχεία από ΑΑΔΕ/GSIS** όταν το ΑΦΜ είναι έγκυρο
  (αλλιώς όσα δήλωσε ο πελάτης στο WHMCS), **idempotent** ανά ΑΦΜ (χωρίς διπλότυπα). Κρατά το **email του
  τρίτου** αν το συμπλήρωσε ο reseller (GSIS δεν το έχει· gap-fill και σε υπάρχοντα πελάτη, ποτέ overwrite)
  και σημειώνει **ποιος reseller τον έφερε** (`referred_by_customer_id`, όπως το lead→convert). Σε single-party
  row συνδέει και το inbox row στον τρίτο ώστε το «Δημιουργία Παραστατικού» να χρεώσει τον σωστό. (Το
  auto-issue ήδη κάνει auto-import στο ingest μέσω `ContactCustomerResolver`· η νέα ενέργεια είναι η manual,
  AADE-εμπλουτισμένη οδός.)
- **Παραστατικό: «Αποστολή PDF με email» με προαιρετικό custom παραλήπτη.** Το κουμπί αποστολής μέσα στο
  παραστατικό απέκτησε προαιρετικό πεδίο «Αποστολή σε άλλο email»: **κενό → στον πελάτη** (η κανονική ροή,
  με το CC δευτερεύοντος email του), **συμπληρωμένο → στοχευμένο αντίγραφο** σε custom διεύθυνση (π.χ.
  λογιστή) — μόνο σε αυτόν, χωρίς CC στον πελάτη. Διαθέσιμο σε **κάθε** εκδοθέν παραστατικό (και όταν ο
  πελάτης δεν έχει καταχωρημένο email — τότε το πεδίο απαιτείται). Αν ο πελάτης έχει συστήσαντα
  (`referred_by_customer_id`, «ήρθε από …»), το email του **προβάλλεται ως hint** στο modal ώστε ο operator
  να στείλει και εκεί (αντικαθιστά το χωριστό κουμπί «Αποστολή σε συστήσαντα/άλλον»). Χρησιμοποιεί την
  **ίδια** μηχανή `SendInvoiceEmail` μέσω recipient override: το targeted αντίγραφο παραλείπει το CC λογιστή
  του πελάτη, κρατά το audit BCC του tenant, και καταγράφεται κανονικά στο «Ιστορικό αποστολών».
- **Admin Καρτέλα: «από πού ήρθε» στο modal «Κατανομή» μιας ομαδοποιημένης είσπραξης.** Ο operator
  έβλεπε «Είσπραξη ΠΛ-…» χωρίς να ξέρει το κανάλι/προέλευση χωρίς να σκαλίσει στις Πληρωμές· τώρα το
  modal «Κατανομή» δείχνει **κανάλι** (Ηλεκτρονική πύλη · Eurobank / Χειροκίνητα, μέσω του κοινού
  `Payment::channelLabel` seam), **τρόπο** και **κωδικό συναλλαγής** — φορτωμένα **lazy** από τα
  `payment_ids` της group-γραμμής (καμία επιπλέον query στη λίστα, ο builder μένει lean). Προβάλλονται
  μόνο τα **κοινά** στοιχεία του group (πεδίο που διαφέρει κρύβεται· το κανάλι γίνεται «Πολλαπλά κανάλια»).
- **Πύλη: «άτυπη απόδειξη» ανά πληρωμή/είσπραξη/έμβασμα + σύνολα καρτέλας.** Στην «Η καρτέλα μου»
  (`/user/statement`) οι γραμμές πληρωμών/εισπράξεων/επιστροφών γίνονται links προς μια read-only
  σελίδα (`/user/receipt/{id}`) που δείχνει **από πού ήρθε** (κανάλι: Ηλεκτρονική πύλη · Eurobank /
  Χειροκίνητα), τρόπο, λογαριασμό, κωδικό συναλλαγής, αναφορά, αιτιολογία και **ποια παραστατικά
  εξόφλησε** (με links). Ένα ομαδοποιημένο έμβασμα ξανα-ανοίγει από την αναφορά του· τα κοινά μόνο
  στοιχεία προβάλλονται (πεδίο που διαφέρει ανά μέλος παραλείπεται). Fail-closed με το ίδιο grant
  boundary. Προστέθηκε και **τελική γραμμή συνόλων** στην καρτέλα (Χρεώσεις − Πιστωτικά − Πληρωμές =
  Υπόλοιπο), ίδια λογική με το admin (τα τοις μετρητοίς δεν προσμετρώνται). Δίγλωσσο (`portal.receipt.*`).
- **Πύλη: online HTML προβολή παραστατικού (σαν WHMCS) — link από το όνομα.** Στο `/user`, το
  όνομα του παραστατικού σε «Τα παραστατικά μου» ΚΑΙ στις γραμμές παραστατικών της «Η καρτέλα μου»
  γίνεται link προς μια read-only σελίδα (`/user/document/{id}`) που δείχνει στοιχεία εκδότη/λήπτη,
  γραμμές, σύνολα (καθαρή/ΦΠΑ/παρακράτηση/σύνολο/πληρωτέο), ΜΑΡΚ + «Επαλήθευση», κατάσταση πληρωμής/
  υπόλοιπο, και τα **συνδεδεμένα παραστατικά** (πιστωτικό↔αρχικό, εισπράξεις/πληρωμές) — δομημένο ώστε
  να μπει αργότερα και το link Υπηρεσία↔Παραστατικό. Το επίσημο PDF παραμένει το νόμιμο artifact (κουμπί).
  **Fail-closed** με το ίδιο grant boundary (`CustomerDocumentFeed`) όπως το PDF route· κάθε linked-doc
  link ξανα-ελέγχεται· οι μη-ορατές (draft/ακυρωμένα) γραμμές της καρτέλας δεν linkάρουν (κανένα dead 404).
  Δίγλωσσο (el/en, `portal.document.*`).
...
- **Ορατότητα προτιμολογίων: widget «Προτιμολόγια» + φίλτρο στη λίστα.** Τα προσφερμένα
  ...
  καθένα: **απλήρωτα** (ποιος δεν πλήρωσε — κυνήγα ή ανάκαλεσε) και **πληρωμένα** (η ουρά εγγράφων που
  περιμένουν **έκδοση** — τα χρήματα ήρθαν, το παραστατικό όχι). Κάθε μέγεθος οδηγεί στη λίστα
  φιλτραρισμένη· νέο `DashboardMetrics::proformaPipeline()`. Το widget κρύβεται όταν ο tenant δεν έχει
  προσφέρει ποτέ κανένα.
- **«Χρήση πίστωσης» σε ΠΟΛΛΑΠΛΑ παραστατικά με μία κίνηση** από την πύλη. Ο πελάτης έχει ένα
  «καλάθι» πίστωσης και συνήθως θέλει να το ξοδέψει, όχι να το μοιράσει στο χέρι: διαλέγει όσα
  παραστατικά θέλει και η πίστωση πάει στο καθένα με τη σειρά της σελίδας, capped ανά παραστατικό,
  και σταματά καθαρά όταν εξαντληθεί. Το πεδίο ποσού έφυγε — ζητούσε νούμερο που θα μειωνόταν σιωπηλά.

- **«Προτιμολόγιο»: ο πελάτης μπορεί να εξοφλήσει draft ΠΡΙΝ εκδοθεί.** Οι ανανεώσεις υπηρεσιών
  σταδιοποιούνται ως drafts (`StageServiceRenewal`), αλλά ο πελάτης δεν τα έβλεπε καν — οπότε ο μόνος
  δρόμος ήταν «έκδοσε και μετά ακύρωσε ό,τι δεν θέλει», που παράγει ρεύμα ΑΚΥ/πιστωτικών: ακριβώς το
  μοτίβο που τραβά την προσοχή της ΑΑΔΕ. Νέα ενέργεια **«Προσφορά στον πελάτη»** κάνει ένα draft
  προτιμολόγιο: κλειδώνει για επεξεργασία, γίνεται ορατό στην πύλη και **πληρωτέο** εκεί (ή εξοφλήσιμο
  από την πίστωση του πελάτη). **Παραμένει draft** — χωρίς ΑΑ, χωρίς myDATA, εκτός κάθε money total.
  Επειδή το προτιμολόγιο και το τελικό τιμολόγιο είναι **η ίδια γραμμή**, η πληρωμή είναι ήδη σωστά
  δεμένη όταν εκδοθεί: καμία μεταφορά προκαταβολής. «Ανάκληση προσφοράς» το ξαναανοίγει.
  Υλοποιήθηκε ως **flag** (`invoices.offered_at`), όχι τέταρτη τιμή του `local_status`: το τελευταίο
  είναι load-bearing σε 206 σημεία / 72 αρχεία, και μια λάθος κρίση σε money site είναι σιωπηλό λάθος
  χρημάτων.
- **«Χρήση πίστωσης» πάνω στο παραστατικό** (καρτέλα «Πληρωμές»), όχι μόνο στην Καρτέλα πελάτη — εκεί
  που ο operator πραγματικά αντιλαμβάνεται την ανάγκη. Ίδιο choke-point (`PaymentAllocator::applyCredit`,
  re-point net-zero, ποτέ νέο χρήμα).
- **«Χρήση πίστωσης» και από την πύλη**: ο πελάτης δένει μόνος του τα χρήματα που ήδη μας έχει πάνω σε
  δικό του παραστατικό ή προτιμολόγιο. Δεν δημιουργείται χρήμα, οπότε δεν χρειάζεται έγκριση· οι
  φραγμοί είναι η ιδιοκτησία (grant-scoped) και τα caps του allocator.

### Fixed
- **Οριστική διαγραφή πελάτη δεν παίρνει πια μαζί πληρωμές/παραστατικά.** Τα FKs κάνουν CASCADE τις πληρωμές
  και SET NULL τον πελάτη στα τιμολόγια (που δεν κρατούν αντίγραφο επωνυμίας/ΑΦΜ)· το bulk «Οριστική διαγραφή»
  ήταν χωρίς έλεγχο. Τώρα το μοντέλο αρνείται (κάθε διαδρομή: panel/CLI/MCP) όσο υπάρχουν παραστατικά, πληρωμές,
  προσφορές, δελτία, CMR, συμβόλαια ή domains· το bulk τους παραλείπει. Η απλή (soft) διαγραφή μένει.
- **Reverse charge για παλιούς πελάτες ΕΕ με χώρα σε ελεύθερο κείμενο** («ΙΤΑΛΙΑ»): το predicate διάβαζε το raw
  `country` αντί για το κανονικοποιημένο ISO, οπότε έβγαζε default 24% αντί για 0%/αντίστροφη χρέωση.
...
- **WHMCS εισερχόμενα με coupon/promotion: η αρνητική γραμμή έκπτωσης κρατούσε το τιμολόγιο (η ΑΑΔΕ
  ...
  zombee20 - 20.00% Έκπτωση» −58,00 €, Sub Total 232,00). Η myDATA floor-άρει τα netValue/vatAmount στο 0,
  οπότε ο guard κρατούσε τη γραμμή και ζητούσε χειροκίνητη τακτοποίηση. **Fix:** ο `WhmcsInvoiceMapper`
  πλέον **ενσωματώνει** την έκπτωση στις θετικές γραμμές **της ίδιας φορολογικής μεταχείρισης** (taxed 0/1)
  ως line-level `discount %`, αναλογικά ανά ποσό — έτσι το παραστατικό δεν έχει αρνητική γραμμή και κλείνει
  με το WHMCS (μία γραμμή net 232,00 / ΦΠΑ 55,68 / μεικτό 287,68, έκπτωση 20%). Fold **μόνο** όταν το tax
  group έχει θετική χρέωση να την απορροφήσει ΚΑΙ η έκπτωση είναι γνήσια μικρότερη από τη χρέωση· ό,τι δεν
  απορροφάται (π.χ. ≥100% ή έκπτωση σε άλλο tax group χωρίς χρέωση) μένει αρνητικό → ο guard **εξακολουθεί**
  να το κρατά. Θωρακίσεις (post-review): fold μόνο στο **whole-invoice** path (file/createDraft), **όχι** στο
  per-party split (εκεί δεν τρέχει το totals-reconcile)· απόρριψη έκπτωσης που στρογγυλοποιείται σε **≥100%**
  (καμία €0 γραμμή)· και η **άμεση τιμολόγηση ΔΕΝ** εκδίδει αυτόματα τιμολόγιο με ενσωματωμένη έκπτωση
  (indistinguishable από χειροκίνητη πίστωση → κρατείται για έλεγχο· η χειροκίνητη «Δημιουργία Παραστατικού»
  το βγάζει κανονικά).
...
- **`invoices:recompute-balances --company=SLUG` δεν έκανε ΤΙΠΟΤΑ.** Το option δεχόταν μόνο company
  **id**, ενώ το `CLAUDE.md` το τεκμηριώνει με slug (`--company=myip`) — άρα η τεκμηριωμένη εντολή
  ...
  backfill. Δέχεται πλέον **slug ή id** και βγάζει σφάλμα σε άγνωστη εταιρία. (Στο devbox η διόρθωση
  ξανα-υπολόγισε **6786** παραστατικά που περίμεναν από το ETL.)
- **Τα σφάλματα της πύλης δεν εμφανίζονταν πουθενά.** Κάθε portal action που απαντά με
  `back()->withErrors()` (χρήση πίστωσης, προφίλ, κωδικός) έκανε σιωπηλό reload: το layout render-αρε
  μόνο το `session('status')`. Μπήκε μία φορά στο layout, ώστε καμία νέα customer surface να μην το
  ξεχάσει.
- **Το προτιμολόγιο εμφανιζόταν στην «Καρτέλα μου» ως κανονικό παραστατικό.** Το badge είχε μπει μόνο
  στη λίστα παραστατικών· τώρα και στην κατάσταση κινήσεων (`is_proforma` ταξιδεύει από τον
  `CustomerLedgerBuilder` μέχρι το badge).

- **Πληρωμένο προτιμολόγιο έριχνε καρτέλα και απαιτήσεις στο αρνητικό.** Οι balance surfaces
  μετρούσαν την πληρωμή του αλλά όχι τη χρέωσή του (το `excludeUnissuedDrafts()` πετούσε το
  μη-εκδοθέν draft): ο πελάτης έβλεπε «πίστωση» που το `availableCredit()` αρνιόταν να ξοδέψει και
  ο operator αρνητική απαίτηση. Νέο `InvoiceScope::excludeUnpaidUnissuedDrafts()` για τις **βάσεις
  οφειλών** μόνο: ένα **απλήρωτο** προτιμολόγιο δεν είναι απαίτηση (η υπηρεσία ακυρώνεται εκατέρωθεν
  πριν την πληρωμή), αλλά μόλις **κρατήσει χρήμα** μπαίνει κι αυτό ώστε χρέωση και πίστωση να
  μηδενίζονται μαζί — το ίδιο money-trail exception που ήδη χρησιμοποιεί ο κανόνας τοις μετρητοίς.
  Τζίρος/ΦΠΑ/credit-note bases μένουν στο `excludeUnissuedDrafts()`: πληρωμένο προτιμολόγιο **δεν**
  είναι έσοδο μέχρι να εκδοθεί.

- **Τοις μετρητοίς «εξόφληση κατά την έκδοση» ίσχυε και σε ΜΗ εκδοθέντα.** Ένα draft με `due_days=0`
  εμφανιζόταν «Εξοφλημένο» χωρίς να έχει πληρωθεί τίποτα (και χωρίς να έχει καν εκδοθεί) — αυτό έκανε
  ένα απλήρωτο προτιμολόγιο να μοιάζει πληρωμένο, και εμπόδιζε το `applyCredit()`, που κόβει σε αυτό το
  υπόλοιπο και έβλεπε μηδέν. Ο κανόνας απαιτεί πλέον `local_status = 'active'`: ό,τι δεν εκδόθηκε δεν
  μπορεί να «εξοφλήθηκε κατά την έκδοση».

- **«Log πύλης»: διάγνωση ανά εγγραφή μέσα στο panel.** Το *γιατί* μιας απορριφθείσας είσπραξης ζούσε
  μόνο στο `laravel.log` — δηλαδή κάθε διάγνωση απαιτούσε SSH, που ο operator δεν έχει. Νέα στήλη
  `payment_gateway_events.diagnostics` (JSON) κρατά δομημένα: τη σειρά πεδίων που έστειλε η τράπεζα, τη
  σειρά που περιμέναμε, ποια πεδία δεν αναγνωρίσαμε, και — σε αποτυχία υπογραφής — αν το shared secret
  επαληθεύεται με τη σειρά της τράπεζας (άρα φταίει η λίστα μας) ή όχι (άρα φταίει το secret). Νέα
  ενέργεια «Λεπτομέρειες» ανά γραμμή τα δείχνει σε ελληνικά με badges ανά πεδίο, και νέο φίλτρο
  «Διάγνωση» απομονώνει τα πρωτοκολλικά προβλήματα. **Ονόματα πεδίων μόνο, ποτέ τιμές** (μεταφέρουν
  δεδομένα παραγγελίας του πελάτη). Έτσι η επιβεβαίωση του `RETURN_FIELD_ORDER` με μία sandbox χρέωση
  γίνεται από το panel.
...
- **Αντιστοίχιση εισαγόμενων «έναντι» πιστώσεων → ανοιχτά τιμολόγια (import cleanup).** Νέα εντολή
  ...
  όχι ανά τιμολόγιο, «δένει» FIFO (παλαιότερα-πρώτα) τη διαθέσιμη έναντι-πίστωση πάνω στα ανοιχτά τιμολόγια
  με **re-point** των γραμμών πληρωμής. **Net-zero** στο υπόλοιπο πελάτη, **idempotent** (ξανα-τρέξιμο no-op),
  σκοπευμένο **μόνο** στις `EPS:` γραμμές (ποτέ γνήσια προκαταβολή), με `--dry-run` που δείχνει το πλήρες
  πλάνο χωρίς εγγραφή. Λύνει το ότι κάθε εισαγόμενο τιμολόγιο έδειχνε «χωρίς πληρωμή» και η αυτόματη
  «Είσπραξη (έμβασμα)» τα στόχευε λάθος. Το `applyCredit`/`availableCredit` δέχονται πλέον optional
  `txPrefix` (family scope) + shared `reference`. Διατηρεί το `transaction_id` κάθε γραμμής, ώστε ένα
  re-import να παραμένει idempotent.
...
- **Στατιστικά χρέωσης ανά συμβόλαιο + retro-link (#11).** Η προβολή μιας **Υπηρεσίας** δείχνει τώρα:
  ...
  πρόχειρα, και **ιστορικό τιμής καταλόγου** (από το audit log). Νέο tab **«Ανανεώσεις»** = read-only λίστα
  των παραστατικών του συμβολαίου (link στο καθένα). Νέα ενέργεια **«Σύνδεση υπάρχοντος παραστατικού»**:
  «κουμπώνει» ένα ήδη-εκδομένο παραστατικό του ίδιου πελάτη στη σύμβαση (η περίπτωση «το πούλησα χειροκίνητα
  και ξέχασα να το κάνω συνδρομή») — θέτει μόνο το `service_contract_id` (ποτέ money/lifecycle), tenant+
  customer scoped και re-checked στο write. Έτσι μετά μετρά κανονικά στο ιστορικό/έσοδο και ανανεώνεται.
- **Εργασίες Είσπραξης — Φάση A (#5).** Η «Ηλικίωση οφειλών» γίνεται *λειτουργική*: κουμπί **«Εργασία
  είσπραξης»** ανά πελάτη (ανάθεση σε operator, επόμενο βήμα + ημ/νία, σημείωση, ☑ «καταγραφή επαφής σήμερα»)
  + νέες στήλες **«Επόμενο βήμα»** και **«Τελ. επαφή»**. Καθαρά record-keeping — καμία ειδοποίηση/κλιμάκωση
  (αυτά είναι Φάση Γ)· χωρίς per-event ιστορικό (Φάση B). Κατάσταση σε στήλες στο `customers`
  (`collection_assigned_to` → users nullOnDelete, `collection_next_step_at/_note`, `collection_last_contact_at`,
  `collection_note`). Tenant-scoped (ο πελάτης φορτώνεται WHERE company_id = tenant). Ίδιο gate με τη σελίδα
  (`View:AgedReceivables` — καμία νέα permission).
...
- **Αναφορά «Έσοδα ανά κατηγορία» (#2, Phase 1).** Νέα σελίδα στα «Λογιστικά»: καθαρά/ΦΠΑ/μεικτά
  ...
  **εξαγωγή CSV**. Η κατηγορία κάθε γραμμής προκύπτει: `invoice_lines.product_category_id` (σφραγίδα
  από WHMCS) → αλλιώς η κατηγορία του προϊόντος → αλλιώς «Αταξινόμητα». Τα πιστωτικά αφαιρούνται,
  μόνο ζωντανά/εκδοθέντα παραστατικά. Νέα permission `View:RevenueByCategoryReport` (shield:generate +
...
- **Αντιστοίχιση WHMCS → κατηγορία εσόδων: προειδοποίηση αντί για σιωπηλή απόρριψη.** Αν ο χειριστής όριζε
  ...
  το «Αποθήκευση» εμφανίζει **warning** που ονομάζει τις ομάδες + κατηγορίες που δεν αποθηκεύτηκαν και πώς να
  διορθωθεί (όρισε και τη §8.6). Καμία αλλαγή στο NOT NULL coupling — μόνο ορατότητα.
- **Perf: index `mydata_marks(company_id, provider_key, id)`** για το provider-quota lookup (dashboard card +
  `GrProviderSubmitter::warnIfLowProviderQuota`, που πριν σάρωναν το PK ανάποδα). Μία additive migration.
- **«Άντληση από ΑΑΔΕ» (φόρμα πελάτη) — diff-aware + picker ανά πεδίο.** Το κουμπί συμπληρώνει αμέσως τα
  **κενά** πεδία (όπως πριν), αλλά πλέον, αν κάποιο **ήδη συμπληρωμένο** πεδίο διαφέρει από το μητρώο (π.χ.
  ο πελάτης μετακόμισε → νέα διεύθυνση), ανοίγει **modal επιλογής ανά πεδίο** («δικό μας → ΑΑΔΕ») ώστε ο
  χειριστής να διαλέξει τι θα αντικατασταθεί — αντί να αγνοεί σιωπηλά την αλλαγή (η παλιά «γέμισε-μόνο-κενά»
  συμπεριφορά που ανάγκαζε το «σβήσε → αποθήκευσε → ξανα-άντλησε» για να μπει η νέα διεύθυνση). Ό,τι δεν
  τσεκαριστεί μένει ως έχει. Το «Διόρθωση από ΑΑΔΕ» παραμένει overwrite-όλων χωρίς ερώτηση. Νέο shared
  `AadeFormFill::splitFillsAndConflicts` + page trait `ResolvesAadeFormConflicts` (CreateCustomer/EditCustomer).
...
- **Σελίδα προβολής παραστατικού: οι γραμμές στο σώμα (φάση 3).** Οι γραμμές του παραστατικού μπήκαν
  ...
  (Περιγραφή/Μ.Μ./Ποσότ./Τιμή/Έκπτ.%/ΦΠΑ%/Καθαρή/Μεικτή/E3), κρατά την ανά-γραμμή ταξινόμηση E3 (ίδιος
  `IncomeClassResolver` με την υποβολή). Αφαιρέθηκε ο `LinesRelationManager` από τα tabs — έτσι το
  **«Πληρωμές» γίνεται το πρώτο tab**.
- **Σελίδα προβολής παραστατικού: πιο λιτά τα πάνω cards (φάση 2).** Στο «Identification» έφυγαν τα
  «Series» (ΤΠΥ) και «ΑΑ» (6664) — είναι ακριβώς τα δύο μισά του «Code» (ΤΠΥ6664), οπότε ήταν διπλά·
  το «Type» (πλήρες όνομα τύπου) μένει. Στο «Customer» τα δευτερεύοντα στοιχεία (ΦΠΑ VIES,
  Δραστηριότητα, διεύθυνση, πόλη/ΤΚ/χώρα, εγκατάσταση) μετακινήθηκαν σε **modal «Πλήρη στοιχεία»**
  (όπως το detail των «Εισερχόμενα») — inline μένουν μόνο επωνυμία + ΑΦΜ + ο σύνδεσμος ζωντανής
  καρτέλας, ώστε το card να είναι σαφώς κοντύτερο. Καθαρά διάταξη/εμφάνιση.
- **Σελίδα προβολής παραστατικού: πιο συμπαγή τα πάνω cards.** Τα «Identification» και «Customer»
  γίνονται `compact` (λιγότερο padding), και το «Live customer record» εμφανίζεται πλέον ως **σύντομο
  link** («Άνοιγμα εγγραφής» + εικονίδιο) αντί για ολόκληρη την επωνυμία (που «έσπαγε» σε 5–7 γραμμές
  δίπλα στο νόμιμο snapshot «Name on invoice») — έτσι το card «Πελάτης» μικραίνει αισθητά καθ' ύψος και
  ελευθερώνεται χώρος από κάτω. Το link οδηγεί στη ζωντανή καρτέλα πελάτη (μόνο για μη-διαγραμμένο
  πελάτη· διαγραμμένος/χωρίς πελάτη → «—», χωρίς link)· καμία απώλεια πληροφορίας.
- **Καρτέλα πελάτη: πιο «λογιστική» ανάγνωση (display-only, καμία αλλαγή στα ποσά).** Οι κινήσεις
  εμφανίζονται πλέον **χρονολογικά (παλιά→νέα) εξ ορισμού** — διαβάζεται σαν καρτέλα λογιστηρίου, με
  το «Υπόλοιπο» να χτίζεται από πάνω προς τα κάτω και **γραμμή συνόλων** στο τέλος (Χρεώσεις επί
  πιστώσει − Πιστωτικά − Πληρωμές = Υπόλοιπο, κλείνει από τα ίδια stats). Νέα στήλη **«Κατάσταση»**
  ανά παραστατικό: **Μετρητά / Επί πιστώσει** + αν υπάρχει **πραγματική πληρωμή**, ώστε να φαίνεται
  *γιατί* κινήθηκε ή όχι το υπόλοιπο (τα τοις μετρητοίς εξοφλούνται στην έκδοση, δεν προσμετρώνται —
  ποτέ διπλομέτρηση). Τα δύο γραφήματα (έσοδα/υπόλοιπο ανά έτος) πήγαν **κάτω από τον πίνακα** ως
  compact μινιατούρες (capped height, δίπλα-δίπλα) αντί να τον σκεπάζουν από πάνω.
- **Καρτέλα polish v2:** η **εξαγωγή CSV**, το **statement PDF** και η **πύλη «Η καρτέλα μου»**
  εμφανίζονται πλέον **χρονολογικά (παλιά→νέα)** ώστε να ταιριάζουν με την οθόνη (κοινό
  `CustomerLedgerResult::chronologicalLedger()`). Ρητό, ντετερμινιστικό tiebreak στην ταξινόμηση
  κινήσεων (ημ/νία → created → τύπος → id → αναφορά), και **γραμμή «Υπόλοιπο από μεταφορά»** όταν
  υπάρχει φίλτρο περιόδου (το κλείσιμο του προηγούμενου έτους).
...
- **WHMCS bridge: μια 0%-ΦΠΑ γραμμή (Apply Tax off στο προϊόν) ΔΕΝ εκδίδεται πλέον αυτόματα.** Ένα
  παλιό/grandfathered WHMCS προϊόν με «Apply Tax» unchecked έρχεται ως `taxed=0` και το ekdosi το
  χαρτογραφεί πιστά σε 0% ΦΠΑ — που για εγχώρια υπηρεσία είναι λάθος (οφείλει 24%), ενώ για
  ενδοκοινοτικό reverse-charge είναι σωστό. Η **άμεση τιμολόγηση** (`whmcs:auto-issue`) δεν μπορεί να
  ξεχωρίσει τις δύο περιπτώσεις, οπότε πλέον **κρατάει** κάθε τέτοια γραμμή για χειροκίνητο έλεγχο
  (νέος `WhmcsFilingGuard::assertNoUntaxedForUnattendedIssue`, `file(unattended: true)`) — ακόμη κι
  αν ο tenant έχει configured λόγο απαλλαγής. Στη χειροκίνητη **«Δημιουργία προσχεδίου»** εμφανίζεται
  **soft warning** ώστε ο χειριστής να ορίσει τον σωστό συντελεστή (24% με gross-edit για ίδιο τελικό)
  ή να επιβεβαιώσει την απαλλαγή πριν την οριστικοποίηση. Η χειροκίνητη ροή δεν μπλοκάρεται.
- **PDF παραστατικού: πιο συμπαγές τέλος σελίδας (λιγότερος κατακόρυφος χώρος για πολλά είδη).** Το
  «Υπόλοιπο πελάτη» μπαίνει πλέον **στην ίδια ζώνη, αριστερά από τα σύνολα** (αντί να στοιβάζεται από
  κάτω), και το block στοιχείων παρόχου (ΥΠΑΗΕΣ: πάροχος/ΜΑΡΚ/UID/υπογραφή) **μετακινήθηκε στο τέλος**,
  μαζί με το QR/URL επαλήθευσης myDATA — έτσι όλα τα «περί υπογραφών» είναι συγκεντρωμένα στο κάτω
...
- **«Σημείωση γραμμής» εισάγεται ξανά — και εμφανίζεται στην προβολή.** Ο πίνακας γραμμών στη φόρμα
  ...
  Προστέθηκε στήλη «Σημείωση» (το `notes` μπήκε εντός των στηλών). Στην **προβολή παραστατικού**, η
  σημείωση γραμμής εμφανίζεται πλέον κάτω από την περιγραφή (πλάγιο, muted), όπως στο PDF.
- **Αιτία απαλλαγής ΦΠΑ (§8.3) 0% γραμμής: επιλέξιμη πλέον στη φόρμα.** Ίδιο pattern με τη «Σημείωση» —
  το `vat_exemption_category` ήταν πεδίο *εκτός* των στηλών του table-repeater, οπότε **δεν renderαριζόταν
  κελί**. Γέμιζε μόνο μέσω auto-suggest (`VatExemptionGuidance::recommendForType()`), που επιστρέφει αιτία
  μόνο για τύπους 2.2/1.2/1.3· για εγχώριο/τρίτης-χώρας τύπο (1.1/2.1/2.3 → `null`) μια 0% γραμμή απαιτούσε
  αιτία (`->required()`, AADE [217]) σε **αόρατο** πεδίο → ο χειριστής κολλούσε χωρίς control. Προστέθηκε
  στήλη «Αιτία 0%»: το §8.3 Select εμφανίζεται μόνο σε 0% γραμμή (κενό κελί αλλιώς) και επιλέγεται με το
  χέρι· το auto-suggest παραμένει (προ-γεμίζει 2.2/1.2/1.3). Καμία αλλαγή σε money/lifecycle/persistence.
- **Αναφορά «Έσοδα ανά κατηγορία»: η έκπτωση παραστατικού δεν αγνοείται πλέον.** Η αναφορά αθροίζει τα
  ...
  γραμμές. Για κάθε παραστατικό με έκπτωση παραστατικού τα έσοδα/ΦΠΑ/μεικτά **υπερεκτιμούνταν** και δεν
  συμφωνούσαν με το τζίρο (document value / dashboard). Πλέον το factor εφαρμόζεται ανά γραμμή (ισοδύναμο
  με το `net_total`), οπότε η αναφορά δείχνει την πραγματική αξία εγγράφου.
- **Ημερομηνίες στο panel σε ελληνική μορφή (η/μ/έτος), όχι μήνας/ημέρα/έτος.** Οι date pickers του
  Filament χρησιμοποιούσαν εξ ορισμού **native browser control**, που εμφανίζει την ημερομηνία με το
  locale του browser — οπότε σε browser en-US η «Ημερομηνία έκδοσης» φαινόταν ανάποδα (M/D/Y) στην
...
- **Καρτέλα: το «Υπόλοιπο τέλους έτους» δεν φουσκώνει πια από τοις-μετρητοίς πωλήσεις.** Το
  ...
  πραγματικό υπόλοιπο (`stats.balance`) και τη running-balance της καρτέλας. Πλέον χρησιμοποιεί τον
  ίδιο gate, ώστε `year_end_balance == υπόλοιπο == τελική running balance`. Διορθώνει και τη νέα
  «Υπόλοιπο από μεταφορά», και τα προϋπάρχοντα: γράφημα υπολοίπου, κάρτα «Υπόλοιπο τέλους {έτος}»,
  πίνακα ετών στο PDF. (Ο τζίρος net/gross μετρά κανονικά όλα τα παραστατικά.)
- **Καρτέλα: κενές γραμμές πληρωμής (#377) δεν εμφανίζονται πια.** Legacy ETL raw-inserts (που
  παρακάμπτουν τον MON-8 guard του μοντέλου) είχαν αφήσει πληρωμές με μηδενικό/NULL ποσό → κενή
  γραμμή στην καρτέλα. Πλέον παραλείπονται στο render (0 στο υπόλοιπο/σύνολα — καμία αλλαγή σε ποσό).
...
- **Πελάτες — περισσότερη πληροφορία στη λίστα (στήλες δραστηριότητας + φίλτρα ποιότητας).** Νέες
  **sortable** στήλες: **«Αρ. Παρ/ων»** (πλήθος εκδομένων παραστατικών — χωρίς πρόχειρα/ακυρωμένα/πιστωτικά·
  ...
  καθαρή αξία πωλήσεων μείον πιστωτικά, χωρίς ΦΠΑ, όπως το βιβλίο εσόδων· default off) και **«Τελ. παρ/κό»**
  (ημ/νία τελευταίου παραστατικού — default off, σήμα αδράνειας/churn). Το
  **email** έγινε sortable και προστέθηκε φίλτρο **«Email: με/χωρίς»** (κενό ή NULL = χωρίς) για να εντοπίζεις
  πελάτες χωρίς email τώρα που το παραστατικό το στέλνει και ο Πάροχος. Νέο φίλτρο **«Δραστηριότητα: με/χωρίς
  παραστατικά»** (διαβάζει το ίδιο alias με τη στήλη, δεν αποκλίνουν). Το **τηλέφωνο** έγινε default off. Όλα
  υπολογίζονται σε SQL (ένα grouped sub-select, `Customer::scopeWithInvoiceStats`) → sortable χωρίς per-row PHP.
- **Αυτόματος συγχρονισμός Προμηθευτών & Πελατών από myDATA (scheduled).** Οι `suppliers:sync` (μητρώο
  προμηθευτών από τα ΑΦΜ εκδοτών των `RequestDocs`) και `customers:sync` (μητρώο πελατών από τους counterpart
  ΑΦΜ των πωλήσεων) — που μέχρι τώρα ήταν **μόνο** χειροκίνητα (κουμπί «Άντληση από ΑΑΔΕ» / CLI) — τρέχουν πλέον
  και ως **scheduled tasks** ανά myDATA-readable εταιρεία (μέσω `TenantScheduleSweep`, όπως το reconcile).
  Read-from-AADE, γράφουν **ΜΟΝΟ** νέους προμηθευτές/πελάτες (idempotent, καμία επαφή με τιμολόγια/έξοδα/χρήμα).
  **Default OFF** (γράφουν μητρώο → opt-in), ελεγχόμενα ανά-εκτέλεση από τη σελίδα «Χρονοπρογραμματιστής» (νέα
  ενότητα «myDATA — μητρώα»). Παράθυρα: προμηθευτές = τελευταίος μήνας, πελάτες = τελευταίοι 12 μήνες.
- **WHMCS «Εισερχόμενα»: μαζική «Αρχειοθέτηση επιλεγμένων».** Δίπλα στη μαζική «Διαγραφή» μπήκε μαζική
  «Αρχειοθέτηση»: τσεκάρεις πολλά τιμολόγια και τα αρχειοθετείς μαζί (π.χ. δικά μας/φίλων) αντί για ένα-ένα.
  Αρχειοθετεί όσες είναι «προς έλεγχο»/«σε αναμονή» (εσωτερικό status `rejected`), με μία προαιρετική κοινή
  σημείωση· ό,τι έχει εκδοθεί/αρχειοθετηθεί παραλείπεται με μέτρηση. **Operator-level** (δικαίωμα Update), σε
  αντίθεση με τη μαζική διαγραφή που μένει admin-only. Σε αντίθεση με τη διαγραφή, η αρχειοθέτηση ΔΕΝ επιστρέφει
  στο επόμενο sync.
- **WHMCS mass-pay (συγκεντρωτικό πληρωμής): «Ενοποίηση σε ένα» ή «Ανάλυση σε επιμέρους».** Όταν ένας πελάτης
  πληρώνει πολλά προτιμολόγια μαζί, το WHMCS φτιάχνει ένα «container» invoice (γραμμές `type=Invoice` που δείχνουν
  τα επιμέρους, χωρίς δικό του ΦΠΑ) — π.χ. #32310 → #32280/#32263/#32256. Το ekdosi το κρατούσε «Σε αναμονή» και
  αρνιόταν την έκδοση (σωστά — αλλιώς διπλομέτρηση + 0% ΦΠΑ). Πλέον ο χειριστής το λύνει με ένα κουμπί:
  **«Ενοποίηση σε ένα»** ενώνει τις **πραγματικές** γραμμές των επιμέρους (WHMCS μηδενίζει το `total` του τέκνου
  στο mass-pay, οπότε η αλήθεια είναι τα line items) σε **ΕΝΑ** παραστατικό (νόμιμο συγκεντρωτικό, ΕΛΠ ν.4308/2014
  άρθρο 6) — μία κατάθεση → ένα τιμολόγιο· ή **«Ανάλυση σε επιμέρους»** βγάζει το κάθε τιμολόγιο ξεχωριστά. Το
  net/gross ανακατασκευάζεται από το reference gross του mass-pay (522.34 + ΦΠΑ 125.36 = 647.70 στο πραγματικό
  #32310). Τιμολόγιο **τρίτου πελάτη** εξαιρείται από την ενοποίηση (→ «Ανάλυση»). Το mass-pay ποτέ δεν φιλάρεται·
  κλείνει με νέα κατάσταση **«Ολοκληρώθηκε»** (`resolved`, νέο tab). **Write-back 1→N:** το MARK του ενοποιημένου
  γράφεται πίσω σε **όλα** τα επιμέρους WHMCS invoices, όχι μόνο στο container. Consumed children γίνονται
  tombstone (δεν ξανα-φέρνονται από το επόμενο fetch) και linked για ομαδοποίηση στο UI. Το ενοποιημένο
  παραστατικό φέρει **αναλυτικό σχόλιο** (PDF + admin): «Από συγκεντρωτικό προτιμολόγιο #X — εξοφλεί τα
  προτιμολόγια #a (…), #b (…), #c (…)», ώστε να φαίνεται ποια προτιμολόγια καλύπτει (business term
  «προτιμολόγιο», όχι «WHMCS» — είναι customer-facing).
- **Η ειδοποίηση «Άμεσο παραστατικό προς έκδοση» καθαρίζεται μόνη της όταν το παραστατικό εκδοθεί.** Το durable
  «καμπανάκι» που χτυπά όταν ένας «άμεσης τιμολόγησης» πελάτης ζητά παραστατικό έμενε **αδιάβαστο** μέχρι να το
  πατήσεις με το χέρι — ακόμη κι αν είχες ήδη εκδώσει το παραστατικό (άσκοπο άγχος στο desk). Πλέον μόλις η γραμμή
  WHMCS **χειριστεί** (εκδοθεί / προσχέδιο / αρχειοθετηθεί / ενοποιηθεί / διαχωριστεί) το καμπανάκι γίνεται
  αυτόματα read για όλους τους χρήστες του tenant (μέσω `PendingWhmcsInvoiceObserver`). Η ειδοποίηση φέρει
  structured tag (kind + company + WHMCS id στο viewData) για ακριβές match — όχι parsing κειμένου. Για τις
  **υπάρχουσες** αδιάβαστες ειδοποιήσεις: `php artisan whmcs:resolve-immediate-bells` (καθαρίζει και τις παλιές
  χωρίς tag, εντοπίζοντας το WHMCS id από το κείμενο· ασφαλές να ξανα-τρέξει).

### Changed
- **WHMCS «Εισερχόμενα» — default cadence 15′ → 5′.** Η προεπιλογή του `whmcs:fetch-pending` (άντληση
  πληρωμένων/αδήλωτων WHMCS τιμολογίων στο inbox) κατέβηκε σε **κάθε 5 λεπτά** (`*/5 * * * *`) ώστε τα νέα
  τιμολόγια να εμφανίζονται πιο γρήγορα. Αλλαγή και στα δύο σημεία που ορίζουν το default (`config/ekdosi.php`
  + το literal fallback στο `routes/console.php`, που είναι αυτό που πραγματικά διαβάζει ο scheduler), ώστε
  UI «Χρονισμός» και scheduler να μη διαφωνούν. Χειροκίνητο override από τη σελίδα «Χρονοπρογραμματιστής»
  εξακολουθεί να υπερισχύει.
...
- **Dependency maintenance (minor/patch, εντός constraints).** `composer update` σε: Filament 5.8.1→5.8.2,
  ...
  `composer.json`. Πλήρες suite πράσινο. (Το major `laravel/mcp` 1.0 ΔΕΝ μπαίνει — μπλοκαρισμένο από
  `laravel/boost` (dev-dep) που απαιτεί mcp `^0.7.1|^0.8.0|^0.9.0`.)
- **`spatie/laravel-permission` 7.4 → 8.x (major).** Constraint `^7.4` → `^8.0`, lock 7.4.2 → 8.3.0. Το v8
  απαιτεί PHP `^8.4` + Laravel `^12|^13` (καλυπτόμαστε: 8.4 / 13) και μοντερνοποιεί το codebase (renamed event/
  command classes με suffix, αφαίρεση `clearClassPermissions()`, προσθήκη types). Το ekdosi **δεν αγγίζει**
  κανένα από αυτά· το μόνο subclass (`App\Models\Role`) απλώς προσθέτει `company()` relation, δεν override-άρει
  spatie method· το `filament-shield` ήδη δέχεται `^8`· το `config/permission.php` (teams=true,
  team_foreign_key=company_id) μένει ως έχει. Καμία αλλαγή κώδικα/migration. Πλήρες suite πράσινο (3348),
  με έμφαση στα tenant-roles/shield (`TenantStandardRolesTest`, `RolesReprovisionTest`, `TenantScreenAccessTest`).
- **WHMCS «Εισερχόμενα»: το «Απόρριψη» έγινε «Αρχειοθέτηση» (αλλαγή κειμένων μόνο).** Το κουμπί, το tab
  ...
  διπλά), με ουδέτερο γκρι badge + εικονίδιο αρχείου αντί για κόκκινο «σφάλμα». **Το εσωτερικό status μένει
  `rejected` (STATUS_REJECTED), η στήλη `rejected_reason` και το action key `reject` αμετάβλητα** — καμία
  αλλαγή δεδομένων/λογικής (ίδιο μοτίβο με το `griniaris`: rename μόνο operator-facing strings). Η «Διαγραφή»
  (admin-only, reset της γέφυρας) μένει ως έχει, κόκκινη και διακριτή.
- **Προβολή παραστατικού: αναδιάταξη για να «αναπνέει» το «myDATA / Πάροχος».** Η ενότητα «myDATA / Πάροχος»
  (και τα «Σχετικά παραστατικά») απλώνονται πλέον σε **πλήρες πλάτος** — τα μακρά πεδία (MARK, QR/URL, Αριθμός
  Αδειοδότησης, UID, Υπογραφή) δεν στριμώχνονται σε μισή στήλη. Οι κοντές ενότητες «Παρατηρήσεις» + «Logistics»
...
### Added
- **WHMCS «Εισερχόμενα»: tabbed φίλτρα κατάστασης (CFM-style) με live counts — και το «Σε αναμονή» φαίνεται
  πλέον by default.** Στην κορυφή της λίστας μπήκαν tabs ανά κατάσταση (Ανοιχτά · Προς έλεγχο · Σε αναμονή ·
  Προσχέδια · Καταχωρημένα · Απορρίφθηκαν · Διαχωρισμένα · Όλα), το καθένα με badge αριθμού. **Το default tab
  «Ανοιχτά» δείχνει `pending_review` + `held` ΜΑΖΙ** — πριν, το status φίλτρο default-αρε μόνο σε «Προς έλεγχο»,
  οπότε ένα «Σε αναμονή» παραστατικό (π.χ. αναμονή για ΑΦΜ) **κρυβόταν και «χανόταν»**. Το nav badge
  «Εισερχόμενα N» μετράει επίσης πλέον pending_review + held. Το παλιό (default) status `SelectFilter`
  αφαιρέθηκε — τα tabs οδηγούν το φίλτρο κατάστασης.
...
- **Ψηφιακή διακίνηση — σκέλος επιστροφής ορατό (myDATA v2.0.2, Slice 2).** Το `IN_TRANSIT_RETURN` (9)
  ...
  βλέπει ότι ο μεταφορέας επιστρέφει τα αγαθά. Η «Δήλωση επιστροφής» επιτρέπεται πλέον **και** από αυτή
  την κατάσταση. Τα νέα event types στο ιστορικό διακίνησης (`RegisterTransferReturn` με στοιχεία
  μεταφοράς, `ConfirmReturn`) αποκτούν σωστό label/summary. Στο infolist προστέθηκε το «MARK επιστροφής»,
  και η στήλη «Διακίνηση» στη λίστα δείχνει πλέον ελληνικό label. **Carrier-reported**: το firebed δεν
  εκθέτει submit action για το σκέλος επιστροφής — μόνο το παρατηρούμε μέσω status query.
...
- **Χρονοπρογραμματιστής: 3 flags που ζούσαν μόνο στο `.env` απέκτησαν διακόπτη στο UI** — `whmcs_fetch_unpaid`
  ...
  καθενός). Επίσης: link από την **«Υγεία συστήματος»** προς τις **«Ρυθμίσεις χρονοπρογραμματιστή»** (ο πίνακας
  υγείας δείχνει ΤΙ έτρεξε· ο διακόπτης είναι ένα κλικ μακριά).
- **Αυτόματη καταγραφή είσπραξης όταν το WHMCS τιμολόγιο ήταν ήδη πληρωμένο στην έκδοση**
  (`WhmcsReceiptRecorder`). Για την περίπτωση Eurobank vPOS / έμβασμα (που ο inbound `whmcs:sync-payments`
  σκόπιμα ΔΕΝ πιάνει — είναι only-if-open, ενώ αυτά τα τιμολόγια είναι εξοφλημένα-στην-έκδοση), το money-trail
  (gateway + πραγματικό transaction id + ημερομηνία) περνά αυτόματα ως **είσπραξη** πάνω στο νέο παραστατικό,
  ώστε στην Καρτέλα να φαίνεται ΠΩΣ πληρώθηκε (όχι σκέτο «settled at issue»). Τρέχει και στα δύο μονοπάτια
  έκδοσης (ανεπιτήρητο `file()`/auto-issue + draft-first lifecycle), best-effort (ποτέ δεν μπλοκάρει το AADE
...
- **Guard στην «Είσπραξη (έμβασμα)» της Καρτέλας: τέλος ο κατά-λάθος πιστωτικός πελάτης.** Η ενέργεια
  ...
  πλεόνασμα έπεφτε σιωπηλά ως πίστωση και ο πελάτης εμφανιζόταν πιστωτικός. Πλέον, **όταν μέρος του ποσού
  θα γίνει πίστωση, η modal το δηλώνει και απαιτεί checkbox επιβεβαίωσης** (με το ακριβές ποσό που θα
  μείνει πίστωση)· αλλιώς μένει ένα κλικ. Το κατώφλι μετριέται με το ΙΔΙΟ κριτήριο που χρησιμοποιεί ο
  `PaymentAllocator` (live υπόλοιπο, ίδιο σύνολο τιμολογίων), ώστε η προειδοποίηση να μη διαφωνεί ποτέ με
  το τι πραγματικά κάνει η καταχώριση. Η «Πληρωμή έναντι λογαριασμού» απέκτησε αντίστοιχη επεξήγηση. Και
  οι δύο παραπέμπουν στο σωστό μονοπάτι για είσπραξη **συγκεκριμένου** τιμολογίου: άνοιξε το τιμολόγιο →
  «Πληρωμές» → «Καταχώριση πληρωμής» (δένεται στο τιμολόγιο, δεν βγάζει πιστωτικό).
- **Το κλειδί παρόχου κανονικοποιείται πλέον στο ΓΡΑΨΙΜΟ (`companies.einvoice_provider_key`).** Ο
  `ProviderTransportRegistry` έκανε `trim()` πριν στήσει το transport, οπότε ένα κλειδί με κενά υπέβαλλε
  κανονικά ενώ κάθε σύγκριση της ωμής στήλης αστοχούσε. Αποτέλεσμα: **η φόρμα εταιρείας δεν αποθηκευόταν**
  (το συντιθέμενο «Τρόπος αποστολής» έπεφτε εκτός των options → validation error σε κάθε save, ακόμη και για
  άσχετη αλλαγή), το preview payload και το `einvoice:provider-test-submit` έδειχναν **μη-augmented** AADE
  XML ενώ η πραγματική υποβολή ήταν augmented, και η κάρτα «Υπόλοιπο εκδόσεων» έλεγε «καμία υποβολή ακόμη»
  για tenant που εκδίδει καθημερινά. Mutator στο `Company` (κενό → NULL) + migration που καθαρίζει τις
  υπάρχουσες γραμμές (μόνο trim, με log ανά γραμμή· irreversible by design).
- **`ekdosi:go-live-check`: τέλος το false-green στον πάροχο.** Το gate έλεγχε μόνο `empty($key)`, οπότε
  έγραφε **pass** και για κλειδί που δεν αντιστοιχεί σε transport (→ `NullProviderTransport`) και για tenant
  με **εντελώς κενά διαπιστευτήρια** — δηλαδή για κάποιον που θα σκάσει στο πρώτο πραγματικό τιμολόγιο.
  Πλέον καλεί το `ProviderPreflight::transportReadiness()` (νέα μέθοδος: το transport-scoped μισό του
  preflight — κλειδί/transport/περιβάλλον/διαπιστευτήρια ενεργού περιβάλλοντος/ασφάλεια endpoint), οπότε
  Console και cutover gate δεν μπορούν να διαφωνήσουν για τον ίδιο tenant. Επίσης `warn` για sandbox, **με
  αναφορά της τιμής** του mode (η στήλη είναι σκέτο string· ένα `'prod'` υποβιβάζεται σιωπηλά σε sandbox).
- **Η φόρμα εταιρείας δεν «κλειδώνει» πια από άγνωστο πάροχο.** Το `trim()` έλυνε μόνο την εκδοχή με κενά·
  ένα καθαρό κλειδί εκτός `provider_labels` (γραμμή από ETL, ή πάροχος που αποφασίσαμε να μη shipάρουμε)
  συνέθετε εξίσου channel εκτός των options → validation error σε **κάθε** save. Πλέον το τρέχον channel
  επανεισάγεται σημασμένο ως «άγνωστος πάροχος», όπως ήδη κάνει το `whmcsDefaultTypeOptions`.
- **Αλλαγή παρόχου δεν κουβαλάει πια τα στοιχεία του προηγούμενου.** Το blob credentials είναι ΕΠΙΠΕΔΟ, και
  η φόρμα προ-συμπλήρωνε τα πεδία ΚΑΘΕ παρόχου από αυτό — οπότε σε κοινό όνομα πεδίου (`base_url` υπάρχει σε
  invosign ΚΑΙ sbz) ο νέος πάροχος κληρονομούσε το endpoint του παλιού και **αποθηκευόταν** έτσι, ακυρώνοντας
  τη «secret hygiene on rotation» που υλοποιεί το `dehydrate()`. Πλέον προ-συμπληρώνονται μόνο τα πεδία του
  ΤΡΕΧΟΝΤΟΣ παρόχου. Εξαίρεση με λόγο: όταν ο tenant είναι παρκαρισμένος σε κανάλι myDATA (χωρίς provider key)
  το blob κρατιέται σκόπιμα αλλά **δεν καταγράφεται σε ποιον ανήκε**, οπότε εκεί η προ-συμπλήρωση μένει —
  αλλιώς η διαδρομή «πάροχος → Καθόλου → πάροχος» έσβηνε οριστικά κρυπτογραφημένο token.
- **Το `SendChannelFormBridge` συγκρίνει normalised κλειδιά** πριν αποφασίσει «ίδιος πάροχος» — αυτή η
  σύγκριση είναι ό,τι κρατά τα αποθηκευμένα credentials σε ένα save όπου τα πεδία δεν επέστρεψαν τιμή.

### Added
- **Η καρτέλα «Πάροχος (ΥΠΑΗΕΣ)» λέει πλέον αν το κάθε στοιχείο ΕΙΝΑΙ αποθηκευμένο.** Ένα μασκαρισμένο
  πεδίο με προ-συμπληρωμένη τιμή είναι οπτικά ίδιο με ένα κενό, οπότε τα «κουκκάκια» δεν έλεγαν αν το token
  σώθηκε, αν είναι ακόμη εκεί, ή αν το save θα το χαλάσει. Κάθε πεδίο δείχνει «✓ Αποθηκευμένο (••••1234)»
  με τα 4 τελευταία ψηφία, ή «⚠ Δεν έχει αποθηκευτεί». Το fingerprint είναι **βοήθημα αναγνώρισης, όχι
  μέτρο εμπιστευτικότητας**: η φόρμα προ-συμπληρώνει τα πεδία με τις πραγματικές τιμές (αυτό κάνει το
  `->revealable()` να δουλεύει), άρα το μυστικό είναι έτσι κι αλλιώς στη σελίδα — η προστασία είναι ότι η
  οθόνη είναι super_admin-only. Σε αλλαγή παρόχου προειδοποιεί ρητά ότι τα παλιά στοιχεία **διαγράφονται
  οριστικά** με την αποθήκευση (πριν έλεγε το ηπιότερο «δεν μεταφέρονται»).

### Changed
- **Το «Υπόλοιπο εκδόσεων» του παρόχου ανέβηκε ψηλά στο dashboard.** Η κάρτα δεν είναι πια
  ξεχωριστό widget κοντά στον πάτο: για tenant που διαβάζει myDATA μπαίνει ως **4η κάρτα στην ομάδα
  «Εικόνα από myDATA — ΦΠΑ»** (`MyDataPictureStats`), με ετικέτα «Πάροχος — Υπόλοιπο εκδόσεων», **στην
  ίδια γραμμή με τις 3 κάρτες ΦΠΑ**. Όσο η σειρά έχει ≥4 κάρτες το πλέγμα κλειδώνει στις 4 στήλες, ώστε
  (α) η θέση να μη χαλάει από τυχόν αναλυτικές κάρτες μη-πωλήσεων και (β) οι υπάρχουσες κάρτες ΦΠΑ να
  μην αλλάζουν πλάτος επειδή προστέθηκε μια τέταρτη (πριν τον πρώτο συγχρονισμό, που δείχνει μόνο 2
  κάρτες, ισχύει η προεπιλογή του Filament). Το
  widget «Πάροχος ΥΠΑΗΕΣ» μένει ως fallback μόνο για πάροχο-tenant χωρίς myDATA read-creds και
  κρύβεται όταν η σειρά ΦΠΑ είναι ορατή (ποτέ διπλή, ποτέ χαμένη· κοινός builder
  `Concerns\BuildsProviderQuotaStat`).
- **Σαφέστερη χρονοσήμανση στο υπόλοιπο παρόχου:** «ενημ. …» → «τελευταία υποβολή …» — η μέτρηση
  έρχεται μόνο πάνω στην απάντηση έκδοσης (δεν υπάρχει poll), οπότε μια ήσυχη εβδομάδα δεν πρέπει να
  διαβάζεται ως χαλασμένο refresh.

### Added
- **ETL: διπλό ΑΦΜ στη legacy πηγή λύνεται ΧΩΡΙΣ να πειραχτεί η παλιά βάση.** Το
  `migrate:firebird` δέχεται `--afm-keep=CUST_ID` (επαναλαμβανόμενο, ένα ανά διπλό ΑΦΜ): ο
  χειριστής δηλώνει ποιος κρατά την ταυτότητα ΑΦΜ, οι υπόλοιποι μπαίνουν **parked** — με ΑΦΜ,
  παραστατικά και ιστορικό, αλλά χωρίς `afm_key` (νέα στήλη `customers.afm_key_parked`, μόνο το ETL
  τη γράφει, δεν είναι fillable). Καλύπτει το legacy «υποκατάστημα = δεύτερη εγγραφή με ίδιο ΑΦΜ»
  hack που αντικαθιστά το `invoices.counterpart_branch`. Η legacy Firebird μένει read-only αρχείο.
- **Read-only preflight ΑΦΜ πριν το import.** `migrate:firebird --dry-run` (μόνο ο έλεγχος
  συγκρούσεων ΑΦΜ, δεν γράφει τίποτα και δεν δημιουργεί εταιρεία· exit 0/1) και το ίδιο check μέσα
  στο «Έλεγχος σύνδεσης» της φόρμας εισαγωγής — ο χειριστής το μαθαίνει πριν πληρώσει gbak restore
  + import. Κοινός κανόνας για τα τρία σημεία: `App\Services\Etl\LegacyAfmConflicts`.
- **Το `--afm-keep` και από το panel**: νέο πεδίο «CUST_ID που κρατούν το ΑΦΜ» στη φόρμα εισαγωγής
  (`firebird_import_runs.afm_keep`, περνάει στο job) — αλλιώς ένας χειριστής χωρίς CLI δεν είχε
  τρόπο να λύσει διπλό ΑΦΜ της πηγής. Αποθηκεύεται στη γραμμή εισαγωγής, ώστε μια παλιά εισαγωγή να
  δείχνει κάτω από ποια απόφαση ταυτότητας έγινε.
- **`customers:afm-duplicates`: αναφορά parked πελατών** (ΑΦΜ χωρίς ταυτότητα) με τον κάτοχο και τις
  δύο σωστές καταλήξεις (συγχώνευση, ή υποκατάστημα → «Εγκατάσταση πελάτη» στο παραστατικό).

### Fixed
- Ένας parked πελάτης ήταν μη-επεξεργάσιμος: το `Customer::saving()` ξανα-παρήγαγε το `afm_key` σε
  κάθε αποθήκευση και χτυπούσε το unique index. Πλέον μένει parked όσο το ΑΦΜ όντως συγκρούεται και
  **ξανα-αποκτά** την ταυτότητά του μόλις πάψει (merge/διόρθωση) — self-healing, χωρίς χειροκίνητο βήμα.
- `migrate:firebird --dry-run` τύπωνε ✅ ενώ αγνοούσε σιωπηλά ένα `--afm-keep` που δεν αντιστοιχούσε
  σε κανένα διπλό ΑΦΜ· και το `afm_key_parked` έμενε κολλημένο όταν το ΑΦΜ της γραμμής γινόταν κενό/
  placeholder (καμία αναφορά δεν μπορούσε πια να το εμφανίσει).
- Το πεδίο «CUST_ID που κρατούν το ΑΦΜ» προσυμπληρώνεται από την τελευταία εισαγωγή που είχε απόφαση —
  η εβδομάδα parallel-run κάνει καθημερινό re-import πάνω στα ΙΔΙΑ διπλά.
- **Parked πελάτης δεν αποθηκευόταν από το panel**: ο κανόνας μοναδικότητας ΑΦΜ της φόρμας τον
  απέρριπτε («Υπάρχει ήδη πελάτης με αυτό το ΑΦΜ») ακόμη και όταν του έδινες πίσω το ΔΙΚΟ του ΑΦΜ —
  δηλαδή ακριβώς το αδιέξοδο που το parked state υποτίθεται ότι λύνει. (Άλλο ΑΦΜ τρίτου πελάτη
  εξακολουθεί να απορρίπτεται κανονικά.)
- Το «Έλεγχος σύνδεσης» δεν έστελνε στο probe τα CUST_ID που είχε ήδη γράψει ο χειριστής, οπότε
  μετά τη συμπλήρωση του πεδίου ξανάλεγε «η εισαγωγή θα σταματήσει».
- `CompanyImporter::rowSignature()` δεν αφαιρούσε το νέο `afm_key_parked`, οπότε ένα **παλιό**
  bundle σε αναβαθμισμένο target δεν ταίριαζε πια με τον τοπικό δίδυμο (πελάτες χωρίς `legacy_id`
  και χωρίς πραγματικό ΑΦΜ) → διπλή εγγραφή.
- `customers:merge` με **parked επιζώντα**: ο επιζών έπαιρνε τα πάντα αλλά έμενε χωρίς `afm_key` ενώ
  ο κάτοχος του ΑΦΜ είχε μόλις διαγραφεί οριστικά — κανείς δεν κρατούσε πια το ΑΦΜ και οι matchers
  (WHMCS ανά ΑΦΜ, `LeadMatcher`, myDATA sync) θα ξαναδημιουργούσαν διπλό πελάτη. Πλέον ο επιζών
  παίρνει την ταυτότητα μόλις ελευθερωθεί (ποτέ αν την κρατά άλλος).
- Bundle export/import (`CompanyImporter`) σεβάται το parked state: ο υποκατάστημα-δίδυμος ταξιδεύει
  ως ξεχωριστή γραμμή αντί να συγχωνεύεται σιωπηλά στον κάτοχο του ΑΦΜ ή να μπλοκάρει την εισαγωγή.
...
- **Σύστημα υποστήριξης — ασαφής αντιστοίχιση πελάτη σε εισερχόμενο email (privacy).** Αν η διεύθυνση αποστολέα
  ταιριάζει με **πολλαπλούς** πελάτες της ίδιας εταιρείας (το email ΔΕΝ είναι μοναδικό), ο router δεν κάνει πλέον
  αυθαίρετο auto-bind στον πρώτο — που θα εμφάνιζε στο infolist του χειριστή το υπόλοιπο/παραστατικά ΛΑΘΟΣ πελάτη.
  Ανοίγει **αδέσμευτο** ticket (χωρίς `customer_id`) με **εσωτερική σημείωση** flag, και ο χειριστής συνδέει τον
  σωστό με τη νέα ενέργεια **«Σύνδεση πελάτη»** (tenant-scoped). Το «Μόνο πελάτες» τμήμα ΔΕΝ ρίχνει έναν τέτοιο
  (γνωστό αλλά ασαφή) αποστολέα — μόνο έναν πραγματικά άγνωστο.
...
- **Στοιχεία πληρωμής πάνω στο παραστατικό.** Το tab «Πληρωμές» του τιμολογίου δείχνει στήλη «Κανάλι» («Πύλη ·
  Eurobank» vs «Χειροκίνητα») δίπλα στον κωδ. συναλλαγής — «εξοφλήθηκε μέσω πύλης …» με μια ματιά.
- **Ειδοποίηση χειριστή σε αυτόματη είσπραξη πύλης.** Όταν το vPOS return settle-άρει *χωρίς* επίβλεψη, χτυπάει το
  «καμπανάκι» των χειριστών του tenant («ο X πλήρωσε €Y μέσω eurobank»). Best-effort (δεν σπάει το settlement).
...
- **MON-13: υπόλοιπο πελάτη — πιστωτικό πάνω σε τιμολόγιο ΜΕΤΡΗΤΟΙΣ έδειχνε ΔΙΑΦΟΡΕΤΙΚΟ υπόλοιπο σε κάθε
  ...
  (το `credited_total` αθροιζόταν μόνο πάνω στη βάση απαιτήσεων, όπου τα μετρητοίς δεν μπαίνουν). Πλέον το
  `credited_total` αθροίζεται πάνω σε ΟΛΑ τα live εκδοθέντα αρχικά (κάθε όρος), ώστε τα τρία surfaces να συμφωνούν.
  Guarded από νέο case στο `MoneyStatusConsistencyTest` (η περίπτωση ήταν πριν ρητά εξαιρεμένη ως «FIFO fuzziness»).
- **Καρτέλα «Υπόλοιπο» — ανάλυση «από πού ήρθε».** Το tile υπολοίπου δείχνει τώρα το breakdown «Χρεώσεις … ·
  Πιστωτικά −… · Πληρωμές −…», ώστε ένα πιστωτικό (ή πληρωμή) που γύρισε το υπόλοιπο σε πιστωτικό να είναι ορατό
  αντί για μυστήριο.
- **MON-8 (σκλήρυνση) — μη-θετικό ποσό πληρωμής/επιστροφής απορρίπτεται στο ΜΟΝΤΕΛΟ.** Το `amount` είναι πάντα
  θετικό μέγεθος· η κατεύθυνση δίνεται από το `kind` (payment/refund). Μια «επιστροφή −15» θα μετρούσε σιωπηλά σαν
  +15 (πληρωμή). Πέρα από το `minValue(0.01)` των φορμών, το Payment model πλέον πετά exception σε amount ≤ 0 —
  κανένα σημείο (φόρμα, action, μελλοντικό script) δεν μπορεί να αντιστρέψει το πρόσημο.
- **Λίστα «Πληρωμές» — οι επιστροφές ξεχωρίζουν.** Μια επιστροφή (`kind=refund`) εμφανιζόταν σαν κανονική
  εισερχόμενη πληρωμή με θετικό ποσό. Πλέον έχει badge «Επιστροφή» και το ποσό δείχνεται **αρνητικό** (χρήμα OUT),
  ώστε το πρόσημο να ταιριάζει με την Καρτέλα.
- **DSO (ημέρες είσπραξης) — μηδέν, όχι αρνητικό, σε πιστωτικό υπόλοιπο.** Το DSO στο dashboard πλέον
  «κλειδώνει» στο 0 όταν οι απαιτήσεις είναι μηδέν/πιστωτικές (ένα πιστωτικό υπόλοιπο δεν έχει «ημέρες
  ανεξόφλητου»· MON-13 μπορεί να το κάνει αρνητικό).
- **Καρτέλα κινήσεων — σειρά ίδιας μέρας, ώρα, links.** (1) Δύο κινήσεις την ίδια μέρα (π.χ. πληρωμή + επιστροφή)
  ταξινομούνται πλέον με σειρά καταχώρισης (`created_at`) αντί αυθαίρετα — δεν εμφανίζονται «ανάποδα» και το running
  υπόλοιπο διαβάζεται σωστά. (2) Εμφανίζεται η **ώρα** κάτω από την ημερομηνία. (3) Οι γραμμές **πληρωμών/επιστροφών**
  γίνονται **links** στην (επεξεργάσιμη) εγγραφή Payment — βλέπεις/διορθώνεις π.χ. μια λάθος επιστροφή· και προστέθηκε
  κουμπί **«Καρτέλα πελάτη»** στην προβολή παραστατικού + στην επεξεργασία πληρωμής (επιστροφή στην καρτέλα με ένα κλικ).
...
- **Μενού Βήμα 1.5 — μία καθαρή Settings zone + «Διαχείριση» + κοντύτερα λεκτικά.** Τα config-ish
  ...
  Ιστορικό email/Χρήση AI/Υγεία). Νέα ομάδα **«Διαχείριση»** (super-admin: Χρήστες/Εταιρείες/Ρόλοι),
  χωριστά από το «Σύστημα». Συντομεύσεις labels: «Ψηφιακή Διακίνηση» → **«Διακίνηση»**, «myDATA &
  Διασυνδέσεις» → **«Διασυνδέσεις»** (για να χωρέσουν σε μελλοντικό one-line top-nav). `docs/menu-ia.md`.
...
- **Πύλη πελατών — «Τα παραστατικά μου» ξεχωριστό στο μενού.** Το nav item που έδειχνε τα εκδοθέντα
  ...
  κορυφή της «Καρτέλας μου» για άμεση μετάβαση.
...
- **Έξω το εσωτερικό app-name «ekdosi» από τα emails πελατών.** Το προεπιλεγμένο markdown-mail chrome του
  ...
  πραγματικοί πελάτες έβλεπαν «ekdosi» πάνω από την επωνυμία της εταιρείας. Vendor overrides στο
  `resources/views/vendor/mail` αφαιρούν και τα τρία· κάθε email καθοδηγείται πλέον από την **επωνυμία της
  εκδότριας εταιρείας** (H1 + στοιχεία επικοινωνίας στο subcopy). Αφορά ΟΛΑ τα emails (τιμολόγιο/προσφορά/
  καρτέλα), HTML + plain-text· το styling μένει ίδιο (inlined από το ξεχωριστό theme CSS). Το `APP_NAME` στο
  `.env` μένει ο σωστός μοχλός ΜΟΝΟ αν θες να μετονομάσεις και panel/τίτλους σελίδων — αυτό αγγίζει μόνο τα mail.
  Παράλληλα, το footer επικοινωνίας του email τιμολογίου δείχνει πλέον και **ΓΕΜΗ** (δίπλα στο ΑΦΜ) όταν η
  εταιρεία έχει (`companies.gemi`) — HTML + plain-text.
- **Πιο συμπαγές PDF παραστατικού (πυκνότητα «ΑΛΦΑΝΕΤ»).** Ο τίτλος τύπου («ΤΙΜΟΛΟΓΙΟ ΠΑΡΟΧΗΣ
  ΥΠΗΡΕΣΙΩΝ») χωράει πλέον σε **μία γραμμή** (13pt→11pt) αντί να σπάει σε δύο· πυκνότερες γραμμές
  ειδών (padding 1.4mm→0.9mm, 8.5pt→8pt, `table-layout: fixed` + πλατύτερη στήλη περιγραφής 46% ώστε
  μια τυπική γραμμή hosting να μένει σε μία σειρά) και σφιχτότερα μπλοκ header/πελάτη/παρόχου —
  ~8→12 γραμμές ανά σελίδα, ώστε τιμολόγια με πολλά είδη να μη «σπάνε» σε 2η σελίδα.
- **Χρωματιστή κατάσταση πληρωμής στα «Εισερχόμενα».** Στη στήλη «Ποσό», το «Πληρωμένο» βάφεται
  πράσινο και το «Απλήρωτο» πορτοκαλί (όχι μόνο το ποσό) — HtmlString description + οι υπάρχουσες
  κλάσεις `text-{success,warning}-600` του `panel.css` (theme-aware, με dark variants).
- **«Από προτιμολόγιο #N»** αντί «Από WHMCS #N» στις ΠΑΡΑΤΗΡΗΣΕΙΣ του παραστατικού που δημιουργείται
  ...
  πελατειακό PDF.
### Added
- **Web installer: «Νέα εταιρία» ή «Εισαγωγή από .zip».** Ο οδηγός `/install` ρωτά πλέον αν θες κενή πρώτη
  εταιρία ή να ανεβάσεις ένα `company:export` .zip — για όποιον δεν τα πάει καλά με κονσόλα (δώσε στον
  συνεργάτη τον κώδικα + το .zip, φτιάχνει κενή MariaDB, ανεβάζει το .zip και τελείωσε). Το .zip δέχεται
  **κρυπτογραφημένο ή μη** (πεδίο συνθηματικού — υποχρεωτικό μόνο αν το export έγινε με passphrase· ο server
  το επιβάλλει από το `secrets.mode` του ίδιου του αρχείου). Το bundle διαβάζεται + επαληθεύεται το
  συνθηματικό **πριν** το migrate (κακό αρχείο/λάθος συνθηματικό δεν αφήνει μισο-στημένη βάση)· η εγκατάσταση
  περνά μέσα από το ήδη-δοκιμασμένο `ekdosi:install --bundle`.
...
- **Διακόπτης «Αποστολή email πελάτη στον πάροχο» ανά εταιρεία (καρτέλα ΥΠΑΗΕΣ, `einvoice_include_customer_email`).**
  ...
  έφευγαν σε πραγματικούς πελάτες**. Νέος per-tenant διακόπτης (super_admin, καρτέλα «Πάροχος (ΥΠΑΗΕΣ)»),
  **default OFF**: όταν είναι σβηστός, το πεδίο φεύγει **κενό** (ίδια μορφή με πελάτη χωρίς email — ο πάροχος
  δεν βλέπει αλλαγή σχήματος) και ο πάροχος δεν στέλνει email· άναψέ το ρητά στο go-live με πάροχο. Αφορά **μόνο**
  τον δίαυλο παρόχου (το AADE/myDATA payload δεν φέρει email καθόλου) και **δεν** επηρεάζει τη δική του ροή email
  τιμολογίων του ekdosi (`SendInvoiceEmail`). Ισχύει και για τιμολόγια και για δελτία αποστολής.
...
### Changed
- **WHMCS Inbox («Εισερχόμενα»): συμπαγής πίνακας — 11 ορατές στήλες → 6 με «στοίβαγμα».** Ο πίνακας δεν
  χωρούσε ούτε σε 30″ οθόνη. Με το ίδιο pattern που φτιάξαμε στο cfm-web (`TextColumn` + `->description()`
  δεύτερη γραμμή + `->tooltip()` για το πλήρες): **Πηγή + WHMCS# + Ημ/νία** → μία στήλη «WHMCS»· **Σύνολο +
  Πληρωμή WHMCS** → μία στήλη «Ποσό» (το ποσό βάφεται warning όταν είναι απλήρωτο, ώστε να μη χαθεί το σήμα)·
  ο **«Τρίτος»** μπήκε μέσα στον **«Παραλήπτη»** (μαζί με το κλικ→δρομολόγηση)· το **«Άμεσο»** και — για
  καταχωρημένα — ο **κωδικός του ekdosi παραστατικού ως link** μπήκαν κάτω από την **«Κατάσταση»** (κωδικός =
  πλοήγηση· το ΜΑΡΚ μένει η copyable στήλη του = ταυτότητα ΑΑΔΕ). MARK/Επιστροφή ΜΑΡΚ/Legacy μένουν
  toggleable-κρυφά. Καμία αλλαγή σε φίλτρα/ενέργειες/δεδομένα. Regression lock: `InboxColumnsTest`.
- **Αναδιοργάνωση μενού: καθημερινά πάνω-πάνω, λιγότερες/καθαρές ομάδες, ελληνικά labels.** Τα 17 σκόρπια
  top-level items μαζεύτηκαν σε **8 ομάδες με ρητή σειρά** (`navigationGroups()` στον `AdminPanelProvider`).
  Νέα ομάδα **«Καθημερινά»** (Παραστατικά · Εισερχόμενα · Προσφορές · Πληρωμές · Πελάτες), τα 3 leads σε μία
  **«Leads»**, οι Ρόλοι (Shield) μπήκαν στο **«Σύστημα»** (η μονομελής «Filament Shield» καταργήθηκε), οι
  Πληρωμές ανέβηκαν στα «Καθημερινά» (η μονομελής «Είσπραξη/Πληρωμές» καταργήθηκε), «Data»→«myDATA &
  Διασυνδέσεις», «Setup»→«Ρυθμίσεις». **Ελληνικά labels** στα resources που έδειχναν αγγλικά (Προϊόντα,
  Πελάτες, Χρήστες, Εταιρείες, Τύποι παραστατικών, Τρόποι πληρωμής…), σε σωστή πεζή γραφή (όχι title-case).
  Μόνο navigation group/sort/label — μηδέν αλλαγή σε συμπεριφορά/permissions. Regression lock:
  `MenuStructureTest`.
- **PDF δελτίου αποστολής (ΔΑ): ίδια θεραπεία με το τιμολόγιο — συμπαγές layout + διόρθωση «Σελίδα X από 0».**
  Το template των δελτίων αποστολής μοιραζόταν το ίδιο roomy layout (σπαταλούσε ~το ⅓ της σελίδας) και **το
  ίδιο** σπασμένο pager («Σελίδα 1 από 0»). Εφαρμόστηκαν τα ίδια σφιξίματα (περιθώρια/logo/γραμματοσειρές/
  spacing/padding) ώστε ένα τυπικό διαβιβασμένο ΔΑ να χωρά σε **μία σελίδα Α4** (μεγάλο ΔΑ σελιδοποιεί
  κανονικά), και ο pager «Σελίδα X από Y» σχεδιάζεται στον DomPDF canvas από τον `DeliveryNotePdf` (**χωρίς**
  `isPhpEnabled`). Regression lock: `DeliveryNotePdfCompactPaginationTest`.
- **PDF παραστατικού: συμπαγές layout μιας σελίδας + διόρθωση «Σελίδα X από 0».** Το PDF άπλωνε ακόμη κι ένα
  **μονόγραμμο** τιμολόγιο σε **2 σελίδες** (πλούσια περιθώρια/γραμματοσειρές/κενά της PR-27 έσπρωχναν τα
...
- **Πρότυπα email τιμολογίου: προσυμπλήρωση του default ως πραγματικό κείμενο + «Επαναφορά προεπιλογής».**
  ...
  **χανόταν με το πρώτο πλήκτρο** — μια μικρή αλλαγή απαιτούσε ξαναγράψιμο όλου του κειμένου. Πλέον
  προσυμπληρώνονται με το πραγματικό default (επεξεργάσιμο), με hint-action **«Επαναφορά προεπιλογής»** (με
  επιβεβαίωση) για αν χαθεί/σβηστεί. Το συμβόλαιο μένει ίδιο (`MailTemplateRendererTest`): πρότυπο ίσο με το
  default —ή κενό— **αποθηκεύεται ως NULL** (δεν «παγώνει» στη στήλη), ώστε το fallback αποστολής να παραμένει
  ζωντανό και μελλοντική αλλαγή του default να φτάνει στους μη-προσαρμοσμένους tenants. Κοινός helper
  `App\Filament\Support\MailTemplateFields` (μία συμπεριφορά, δύο οθόνες).
- **Αρίθμηση χωρίς κενά — ο αύξων αριθμός δεσμεύεται στην ΑΠΟΣΤΟΛΗ, όχι στη δημιουργία (αναστρέφει το MON-4).**
  Ζητούμενο αυστηρού λογιστή: η σειρά που βλέπει η ΑΑΔΕ να είναι **πάντα συνεχής**. Πλέον ένα πρόχειρο (ή
  οριστικοποιημένο-αλλά-αστάλτο) παραστατικό κρατά **προσωρινή ταυτότητα** «ΠΡΟΣ-ΤΠΥ-{id}» (`code`=NULL) και
  **δεν καταναλώνει αριθμό**· ο πραγματικός ΑΑ/invcode/σειρά δίνονται τη στιγμή που το παραστατικό
  **υποβάλλεται** στο myDATA/πάροχο (`InvoiceNumberer::assign`, μέσα στους submitters, πριν χτιστεί το XML).
  Οριστική απόρριψη ή τοπικό σφάλμα build **επιστρέφει** τον αριθμό στη διαθεσιμότητα (`::release`,
  decrement-if-top)· ambiguous timeout **κρατά** τον αριθμό (το in-doubt recovery υιοθετεί το ΜΑΡΚ). Ο
  `series` παγώνει στην ανάθεση. Non-transmitting tenants ('off'/'none') παίρνουν τον αριθμό στην
  **οριστικοποίηση** ή μέσω `NullSubmitter`. Ισχύει σε **όλα τα κανάλια ΑΑΔΕ**· η **διαγραφή πρόχειρου δεν
  αφήνει πλέον κενό**. Migration: `invoices.code` nullable + `invcode` nullable/(30). Ξεκάθαρα gapless στην
  πράξη· περιπτώσεις-όρια (ταυτόχρονη υποβολή, XML preview προχείρου) → `docs/BACKLOG.md`.
- **Αρίθμηση χωρίς κενά — Phase 2: το ίδιο και στα ΔΕΛΤΙΑ ΑΠΟΣΤΟΛΗΣ.** Ένα πρόχειρο ΔΑ κρατά πλέον
  προσωρινή ταυτότητα «ΠΡΟΣ-ΔΑΠ-{id}» (`code`=NULL, χωρίς πάγωμα σειράς) και ο πραγματικός ΑΑ 9.x
  δεσμεύεται στην **αποστολή** (`DeliveryNoteSubmitter` → `InvoiceNumberer::assignDelivery`, πριν χτιστεί
  το payload). Οριστική απόρριψη AADE/παρόχου ή τοπικό σφάλμα build **επιστρέφει** τον αριθμό
  (`::releaseDelivery`, decrement-if-top)· ambiguous timeout **κρατά** τον αριθμό (το in-doubt recovery
  υιοθετεί το ΜΑΡΚ). Έτσι ένα εγκαταλελειμμένο/ακυρωμένο πρόχειρο ΔΑ δεν αφήνει κενό στη σειρά που βλέπει
  η ΑΑΔΕ. Migration: `delivery_notes.code` nullable + `invcode` nullable/(30). Το εργαλείο
  `delivery:sandbox-validate` κρατά αρίθμηση-στη-δημιουργία (δοκιμαστικό δελτίο, χρειάζεται `code` για το
  dry-run preview). **Διόρθωση (και για τιμολόγια):** το `release`/`releaseDelivery` εκτελείται **μόνο** για τον
  αριθμό που δέσμευσε ΑΥΤΗ η αποστολή (το `assign` επιστρέφει αν όντως δέσμευσε) — έτσι ένα ήδη-αριθμημένο
  παραστατικό (οριστικοποιημένο σε tenant 'off' πριν πάει live, ή εισαγωγή legacy) **δεν επαναριθμείται** από μια
  απόρριψη που απλώς πυροδότησε· οι δύο μέθοδοι ανά τύπο ενοποιήθηκαν σε κοινό πυρήνα. Οι προεπισκοπήσεις XML
  δείχνουν πλέον καθαρό μήνυμα «δεν έχει ΑΑ ακόμη» σε πρόχειρο αντί για σφάλμα builder. **Ολιστικός γύρος
  review (Phase 1+2 μαζί):** το `reserve()` ξαναδιαβάζει το `code` της γραμμής **κάτω από row lock** — δύο
  ταυτόχρονες αναθέσεις του ΙΔΙΟΥ πρόχειρου (finalize/NullSubmitter, που δεν κρατούν per-doc lock) δεν καίνε
  πλέον αριθμό (ο χαμένος υιοθετεί τον αριθμό του νικητή)· και η **Οριστικοποίηση** δεσμεύει τον ΑΑ **πριν**
...
- **«Τιμολόγιο πριν την πληρωμή» — απλήρωτα WHMCS invoices στο «Εισερχόμενα» για χειροκίνητη έκδοση επί
  ...
  τιμολόγηση» (`needs_immediate_invoice` = αυτόματη έκδοση μόλις ΠΛΗΡΩΘΕΙ). Νέα εντολή **`whmcs:fetch-unpaid`**
  (+ `WhmcsUnpaidFetcher`) τραβά τα **ΑΠΛΗΡΩΤΑ** WHMCS invoices αυτών των πελατών (μέσω `getInvoicesForClient`
  ανά `whmcs_client_id`) και τα σταδιοποιεί στο inbox. **ΠΟΤΕ αυτόματα:** μόνο STAGE — το `chooseType()` κρατά
  κάθε unpaid και η αυτόματη έκδοση κλειδώνει στο *άλλο* flag· ο χειριστής εκδίδει χειροκίνητα με τον τύπο
  «επί πιστώσει» (`whmcs_default_unpaid_type_id`, ήδη προεπιλέγεται). Το inbox τα δείχνει με badge «Απλήρωτο» +
  νέο φίλτρο «Απλήρωτα». Προγραμματισμένο **OFF by default** (`EKDOSI_SCHEDULE_WHMCS_FETCH_UNPAID`, ωριαία).
...
- **Στήλη «myDATA (§8.12)» στους «Τρόπους Πληρωμής» + πλουσιότερο helper στη φόρμα.** Όπως οι «Τύποι
  παραστατικών» δείχνουν τον §8.1 κωδικό (ή «λείπει») στη λίστα, τώρα και οι τρόποι πληρωμής δείχνουν τον
  §8.12 κωδικό+ετικέτα (π.χ. «7 · POS/e-POS»)· ένας **αμαπάριστος** τρόπος βγάζει **«λείπει → 3»**
  (πορτοκαλί, με tooltip) — αλλά μόνο warn, όχι μπλόκο (δηλώνεται ως «Μετρητά» 3, δεν απορρίπτεται) και
  **μόνο σε tenant που φιλάρει στην ΑΑΔΕ** (αλλιώς ουδέτερο «—»). Η φόρμα απέκτησε ελληνικό label, καθαρό
  helper και hint-icon με τη λεζάντα των 8 κωδικών §8.12 (παράγεται από τον `Codes::PAYMENT_METHODS`, δεν
  ξεφεύγει). Τα άλλα lookups (μονάδες μέτρησης / τρόποι αποστολής / σκοποί διακίνησης) **δεν έχουν κωδικό
  myDATA** να συνδεθεί, οπότε δεν χρειάζονται τέτοια στήλη· ΦΠΑ (§8.2) & κατηγορίες προϊόντων (§8.6) ήδη τη
  δείχνουν. Νέο `Company::filesToAadeByProvider()` = η μία πηγή του «ποιος είναι AADE tenant» (το διαβάζουν
  πλέον κοινά ο audit MYD-4, το Preflight και η στήλη — καμία απόκλιση).
- **«Έλεγχος ετοιμότητας» — checklist παραγωγικής ετοιμότητας ανά εταιρεία (`Preflight` page).** Το config-δίδυμο
  ...
  — επαναχρησιμοποιεί τον ΙΔΙΟ `MyDataConfigAudit` με το «Έλεγχος ρυθμίσεων» + `mydata:preflight`, οπότε ένα 0%
  χωρίς §8.3 = «Μπλόκο»), **πίνακες lookups** (ΦΠΑ/τύποι/πληρωμές/μονάδες/αποστολή/διακίνηση/κατηγορίες —
  auto-seeded, warn αν λείπουν), **κατηγορίες εσόδων προϊόντων §8.6** και τις **προαιρετικές αντιστοιχίσεις WHMCS**
  (έσοδα §8.6 + πληρωμές §8.12). super_admin-only, «Σύστημα», read-only· link στον «Οδηγό κωδικών» για τις §8.3/§8.12
  αναφορές. Cross-tenant σωστά scoped (κάθε εταιρεία χτίζεται μέσα σε `CompanyContext::actAs` — αλλιώς το ambient
  tenant του panel θα «άδειαζε» κάθε άλλη εταιρεία). _Μετά deploy: `shield:generate` (νέο page permission)._
  - **Οδηγός κωδικών myDATA: νέα ενότητα §8.12 «Τρόποι πληρωμής»** — οι 8 κωδικοί (Μετρητά, κατάθεση ημεδαπής/αλλοδαπής,
    επιταγή, επί πιστώσει, web banking, POS/e-POS, IRIS) με πλαίσιο «πού χρησιμοποιείται», ώστε ο χειριστής να ξέρει
    τι να διαλέξει στους «Τρόπους Πληρωμής» / στην «Αντιστοίχιση WHMCS (πληρωμές)».
...
### Changed
- **Ημερομηνία έκδοσης → αυτόματα η σημερινή τη στιγμή του «Αποστολή» (πάροχος + διακίνηση, PROV-020).**
  Ο πάροχος (InvoSign) απορρίπτει ημερομηνία ≠ σημερινή (σφάλμα 238). Πριν, ένα πρόχειρο φτιαγμένο χθες
  **μπλοκαριζόταν** στην αποστολή και ο operator έπρεπε να διορθώσει την ημερομηνία με το χέρι· τώρα ο
  `GrProviderSubmitter` και ο `DeliveryNoteSubmitter` **σφραγίζουν το `issued_at` στη σημερινή** τη στιγμή
  της υποβολής (η νόμιμη ημερομηνία έκδοσης ΕΙΝΑΙ η στιγμή έκδοσης — δηλαδή το send — όχι πότε φτιάχτηκε
  το πρόχειρο). Παραλείπεται όταν είναι ήδη σημερινή (καμία περιττή εγγραφή)· ο `ProviderIssueDateGuard`
  μένει ως σιωπηλό δίχτυ ασφαλείας. **Μόνο ο πάροχος** — το direct-myDATA μένει ως έχει (η ΑΑΔΕ δέχεται
  παλαιότερη ημερομηνία εντός προθεσμίας, και το «σπρώξιμο» σε σήμερα θα ρίσκαρε λάθος περίοδο ΦΠΑ). Στην
  προβολή Παραστατικού εμφανίζονται πλέον **δύο ημερομηνίες**: «Ημερομηνία έκδοσης» (`issued_at`, δημόσια)
  και «Ημερομηνία δημιουργίας» (`created_at`, εσωτερική).
...
- **Συγχρονισμός εξωτερικής ακύρωσης δελτίου κατά τον «Έλεγχο κατάστασης» (MYD-019).** Αν ένα δελτίο
  ...
  διαφορετικές οθόνες αποφάσιζαν αντίθετα και τα side-effects της ακύρωσης παραλείπονταν. Πλέον ένα
  τερματικό `CANCELLED` της ΑΑΔΕ συγχρονίζει **και τα τρία** πεδία ατομικά, γράφει διακριτή γραμμή
  **`STATE_SYNC`** στο «Ιστορικό myDATA» («Συγχρονισμός κατάστασης (ΑΑΔΕ)» — ώστε να ξεχωρίζει από
  ακύρωση που κάναμε εμείς) και τρέχει την ίδια αντιστροφή αποθέματος (STOCK-001). Idempotent· μια
  μη-τερματική κατάσταση δεν «ανασταίνει» ήδη ακυρωμένο δελτίο· το action δείχνει warning notification.
...
- **Ακύρωση μέσω πιστωτικού μετά από μερικό πιστωτικό (PROV-018).** Η «Ακύρωση μέσω πιστωτικού» και η
  «Ακύρωση & επανέκδοση» ζητούσαν την **πλήρη** αρχική ποσότητα κάθε γραμμής, οπότε μόλις μια γραμμή
  είχε πιστωθεί μερικώς έσκαγαν με «Επιστροφή X > διαθέσιμη ποσότητα Y» — και επειδή στον πάροχο
  (InvoSign) **δεν υπάρχει ακύρωση παρά μόνο πιστωτικό**, το παραστατικό έμενε αδύνατο να ακυρωθεί.
  Νέα `IssueCreditNote::reverseRemaining()` υπολογίζει το **υπόλοιπο** κάθε γραμμής (αρχική − ό,τι έχει
  ήδη πιστωθεί) **κάτω από το row-lock του αρχικού**, παραλείπει τις μηδενικές γραμμές και αστοχεί
  καθαρά μόνο όταν δεν μένει τίποτα· και τα δύο actions περνούν πλέον από αυτή τη μία διαδρομή.
- **Επαναφορά ακυρωμένου = επανεφαρμογή αποθέματος (STOCK-001 revive edge).** Η «Επαναφορά» ενός
  ακυρωμένου παραστατικού δεν ξανα-εφάρμοζε την κίνηση αποθέματος (η `record*` έκανε skip γιατί υπήρχε
  ήδη η βασική κίνηση, ενώ η αντιστάθμιση της ακύρωσης έμενε) → πιστωτικό υπο-εκτιμούσε, τιμολόγιο
  υπερ-εκτιμούσε το απόθεμα κατά qty. Νέο signed `REASON_REVIVE` που μηδενίζει το εκκρεμές cancel-
  compensation στην επαναφορά, και οι reverse* «already reversed» guards έγιναν **net-based**
  (`compensationBalance`) ώστε ένας κύκλος ακύρωση→επαναφορά→ξανα-ακύρωση να μένει idempotent.
...
- **«Οδηγός κωδικών myDATA» — γλωσσάρι κωδικών ΑΑΔΕ (§8).** «Δεν είναι όλοι λογιστές»: μια read-only
  ...
  υπηρεσίες), κατηγορίες ΦΠΑ §8.2, αιτίες απαλλαγής §8.3, είδη δραστηριότητας. Ένα source
  (`App\Support\MyData\CodeReference`, χτισμένο πάνω στα `Codes`/guidance ώστε αλλαγή spec να μη
  γεννά κενή γραμμή). Οι φόρμες (τύπος παραστατικού, κατηγορία εσόδων, είδος δραστηριότητας)
  παραπέμπουν με link «📖 Οδηγός κωδικών». Ανοιχτό σε κάθε χειριστή (help, όχι privileged).
- **Απόδειξη παρόχου (ΥΠΑΗΕΣ): πάγωμα ανά έγγραφο + στη σελίδα παραστατικού (PROV-003).** Η ταυτότητα
  παρόχου εν ισχύ **τη στιγμή έκδοσης** (επωνυμία/site/κωδικός ΑΑΔΕ/**αριθμός αδειοδότησης**) παγώνει
  πλέον ανά παραστατικό στο `mydata_marks.provider_identity` (JSON), ώστε μια μελλοντική αλλαγή/rotation
  αδείας να **μη ξαναγράφει** την απόδειξη σε ήδη εκδοθέντα έγγραφα· γράφεται στο persist από τον
  `GrProviderSubmitter` και διαβάζεται μέσω `ProviderIdentity::forMark` (snapshot, αλλιώς τρέχον config
  για παλιές γραμμές). Η ίδια απόδειξη — Πάροχος / Αριθμός Αδειοδότησης / Αναγνωριστικό(UID) / Υπογραφή
  + η URL επαλήθευσης ως **δείκτης προς το επίσημο έγγραφο του παρόχου** — εμφανίζεται τώρα και στη
  **σελίδα** του παραστατικού («myDATA / Πάροχος»), από το ίδιο snapshot (screen == PDF), με μία
  memoised `Invoice::latestProviderMark()`. Οι ετικέτες ακολουθούν τη γλώσσα του επίσημου εγγράφου.
  Το αρχειοθετικό μισό (ανάκτηση επίσημου PDF) μένει BLOCKED στον πάροχο (κανένα download API).
- **MYD-006 — πολιτική κατηγοριοποίησης εσόδων ανά επιχείρηση (§8.6).** Το seed ταξινομούσε ΟΛΑ τα
  αγαθά ως **category1_1 (Εμπορεύματα)** — σωστό για μεταπωλητή, **λάθος για παραγωγό** (τα δικά του
  προϊόντα είναι **category1_2**). Δεν υπάρχει ένα σωστό default για κάθε επιχείρηση, οπότε πλέον
  επιλέγεται ρητά:
  - **Πεδίο `companies.business_activity_type`** (μεταπωλητής/παραγωγός/υπηρεσίες/μικτή) + helper
    **`App\Support\MyData\ClassificationGuidance`** (σαν το `VatExemptionGuidance`): κωδικοποιεί τη
    σχέση είδους δραστηριότητας → §8.6 κατηγορία αγαθών, με επεξηγήσεις σε απλά ελληνικά.
  - **Ο `AadeInvoiceDocument` εφαρμόζει την πολιτική** στη μηδενικά-καθορισμένη κατηγορία αγαθών: ο
    παραγωγός φιλοδοτεί τα δικά του προϊόντα ως category1_2, ο μεταπωλητής μένει category1_1· υπηρεσίες/
    μικτή/μη-ορισμένο = καμία αλλαγή. Ρητό override ανά κατηγορία προϊόντος υπερισχύει.
  - **Πιστωτικά κληρονομούν την ταξινόμηση του αρχικού** παραστατικού (αντιστρέφουν την ίδια γραμμή
    εσόδων), αντί για τη γενική προεπιλογή του τύπου πιστωτικού (E3_561_001/category1_3).
  - **Go-live gate**: η έκδοση σε ΑΑΔΕ **δεν γίνεται πράσινη** μέχρι να επιλεγεί το είδος δραστηριότητας
    (ισχύει και για υπάρχοντες tenants — επιλογή «Υπηρεσίες» για επιχείρηση υπηρεσιών, ~10").
  - **UI**: picker «Είδος δραστηριότητας» στη «Ρυθμίσεις εταιρείας» (self-service) και στην καρτέλα
    εταιρείας (super_admin). **Ορατότητα ταξινόμησης**: το προϊόν/υπηρεσία δείχνει τώρα (create/edit)
    πώς δηλώνεται στην ΑΑΔΕ (§8.6, κληρονομείται από την κατηγορία/τύπο/πολιτική), και οι λίστες
    Προϊόντων + Κατηγοριών προϊόντων έχουν στήλη «Κατηγ. εσόδων». Ένα source (`ClassificationGuidance`)
    για τα τρία item-nature buckets (εμπορεύματα/προϊόντα/υπηρεσίες).
- **MYD-007 — αιτία απαλλαγής ΦΠΑ §8.3 ανά γραμμή (per-line), σωστά μοντελοποιημένη.** Πριν, μία
  tenant-wide αιτία 0% εφαρμοζόταν σε ΚΑΘΕ μηδενική γραμμή και το seed hint είχε καρφωμένο **16
  (άρθρο 45, εγχώριο reverse-charge)** σαν γενικό «ενδοκοινοτικό» — λάθος: η ενδοκοινοτική **υπηρεσία**
  είναι **4** (άρθρο 18), τα ενδοκοινοτικά **αγαθά** **14** (άρθρο 33), η **εξαγωγή** αγαθών **8**
  (άρθρο 29). Τώρα:
  - **Helper `App\Support\MyData\VatExemptionGuidance`** (σαν το `DeliveryGuidance`): ο χειριστής
    διαλέγει σενάριο σε απλά ελληνικά → σωστός κωδικός §8.3 + τύπος + επεξήγηση· καλύπτει και OSS/IOSS
    (B2C ΕΕ), εξαγωγή, μικρή επιχείρηση, Tax Free. `recommendForType()` προτείνει από τον τύπο
    (2.2→4, 1.2→14, 1.3→8) και επιστρέφει null στα διφορούμενα (refuse-to-guess).
  - **Snapshot ανά γραμμή** (`invoice_lines.vat_exemption_category`, `php artisan migrate`): κάθε
    μηδενική γραμμή κρατά τη ΔΙΚΗ της αιτία· ο `AadeInvoiceDocument` τη διαβάζει ανά γραμμή (fallback
    στην παλιά tenant-wide μόνο για legacy/imported γραμμές). Πολλαπλές 0% κατηγορίες πλέον ΟΚ.
  - **Φόρμα παραστατικού**: πεδίο «Αιτία απαλλαγής ΦΠΑ (§8.3)» που εμφανίζεται μόνο σε γραμμή 0%,
    υποχρεωτικό τότε, με auto-πρόταση από τον τύπο· καθαρίζει αν αλλάξει ο συντελεστής.
  - **Πιστωτικά** κληρονομούν την αιτία της αρχικής γραμμής.
  - Διόρθωση του λάθος helper text στη φόρμα κατηγορίας ΦΠΑ (16/15 → σωστή αντιστοίχιση ανά είδος).
- **Επεξεργασία & εύρεση πρόχειρων παραστατικών.** Κουμπί «Επεξεργασία» στα πρόχειρα (στη σελίδα του
  παραστατικού **και** στη λίστα) που ανοίγει το πλήρες περιβάλλον επεξεργασίας (ημερομηνία/τύπος/
  γραμμές/είδος/τιμή) — μέχρι τώρα η σελίδα edit υπήρχε αλλά **κανένα κουμπί δεν οδηγούσε σε αυτήν**.
  Action «Ημερομηνία έκδοσης → σήμερα» σε πρόχειρα παρόχου (η κανονική online έκδοση απαιτεί ημερομηνία
  = σήμερα, InvoSign 238) — ένα κλικ αντί για «γύρνα σε πρόχειρο, βρες το πεδίο, διόρθωσε». Και section
  «Πρόχειρα» στην **καρτέλα πελάτη**: τα ανέκδοτα πρόχειρα του πελάτη είναι πλέον ορατά και ανοίγουν
  απευθείας — **χωρίς** να μπαίνουν στο υπόλοιπο/κινήσεις (ένα πρόχειρο δεν είναι κίνηση).
...
- **`php artisan roles:reprovision [--tenant=] [--dry-run] [--prune] [--force]`** — διαγνωστικό +
  επισκευαστικό για τα δικαιώματα των ρόλων `company_admin`/`operator` ανά tenant. Το
  `shield:sync-super-admin` (που τρέχει ήδη στο deploy) κάνει **πλήρη** re-sync αυτών των ρόλων· αυτή η
  εντολή προσθέτει τα τρία που εκείνο δεν έχει: **`--dry-run`** (δες τι λείπει/περισσεύει χωρίς να
  γράψεις), **προσθετική λειτουργία** (μια χειροκίνητη προσαρμογή tenant ΕΠΙΒΙΩΝΕΙ — το πλήρες sync τη
  σβήνει) και **επισκευή ενός tenant**. `--prune` δίνει το ίδιο αποτέλεσμα με το πλήρες sync, αφού
  δείξει τι θα αφαιρέσει.
...
- **Leads / mini-CRM (L1) — μετατροπή σε πελάτη** — header action «Μετατροπή σε πελάτη» στο lead:
  ...
  **σύνδεση με υπάρχοντα** (προεπιλέγεται το ΑΦΜ/email/τηλέφωνο match — ποτέ διπλός πελάτης)·
  `ConvertLeadToCustomer` (transaction, row-lock, idempotent) γράφει `converted_customer_id`/
  `converted_at`/«Πελάτης» (ο ΜΟΝΟΣ writer του Won) + γραμμή «Μετατροπή» στο χρονολόγιο. Ο πελάτης
  αποκτά tab **«Προέλευση»** (από ποιο lead, πηγή, χειριστής, πρώτη επαφή, ημέρες ως τη μετατροπή,
  πλήθος επαφών, link στο χρονολόγιο) + φίλτρο «Από lead» στη λίστα πελατών. **Προσφορά από lead**:
  `quotes.lead_id`, «Νέα προσφορά» στο lead ανοίγει τη φόρμα προσυμπληρωμένη, η έκδοση γράφει γραμμή
  «Προσφορά» στο χρονολόγιο και πάει το lead σε «Στάλθηκε προσφορά» **όταν όντως σταλεί** (επιτυχής
  αποστολή email — που πάει στο email του lead αν δεν υπάρχει πελάτης — ή η νέα ενέργεια «Σήμανση ως
  απεσταλμένη»· το draft δεν μετρά ως επαφή)· tab «Προσφορές» στο lead· στη μετατροπή οι προσφορές
  του lead περνούν στον πελάτη. Portability rewire για `quotes.lead_id`.
...
- **MYD-004 (0%-χωρίς-αιτία) — το preflight ΜΠΛΟΚΑΡΕΙ πλέον (ήταν warning → false-green).** Μια
  ...
  οριστεί η αιτία — κλείνει το false-green που άφηνε λάθος υποβολή να περάσει.
- **Το ΜΑΡΚ ακύρωσης δεν χάνεται πια — ούτε παριστάνει το ΜΑΡΚ έκδοσης (MYD-023)** — μια ακύρωση
  παράγει **δύο** ΜΑΡΚ: του παραστατικού που ακυρώνεται και της ίδιας της **πράξης** ακύρωσης. Μόνο
  η απευθείας διαδρομή τιμολογίου τα κρατούσε χωριστά. Η διαδρομή **δελτίων** πετούσε το ΜΑΡΚ
  ακύρωσης (το `delivery_marks` δεν είχε καν στήλη), και οι **δύο διαδρομές παρόχου** έγραφαν
  `$result->cancellationMark ?? $markToCancel` — δηλαδή όταν ο πάροχος δεν επέστρεφε ΜΑΡΚ, η γραμμή
  CANCEL **δήλωνε** το ΜΑΡΚ έκδοσης ως απόδειξη ακύρωσης· χειρότερο από το να μην κρατάς τίποτα,
  γιατί διαβάζεται ως απόδειξη — και είναι το κανάλι που γίνεται υποχρεωτικό. Νέα στήλη
  `delivery_marks.cancellation_mark` (ίδιο σχήμα με `mydata_marks`) και τα δύο ΜΑΡΚ πλέον σε δικές
  τους στήλες παντού. Ο **Συγχρονισμός κατάστασης από ΑΑΔΕ** υιοθετούσε ακύρωση **χωρίς καμία
  απόδειξη**, ενώ ο `TransmittedDocReader` κρατούσε το ΜΑΡΚ της ΑΑΔΕ και το πετούσε — τώρα
  καταγράφεται στη γραμμή `STATE_SYNC` (χωρίς άρνηση: είναι η διαδρομή που **επιδιορθώνει**
  παραστατικό του οποίου την ακύρωση μάθαμε αργά). Το ΜΑΡΚ ακύρωσης εμφανίζεται πλέον και στις
  τρεις οθόνες (PDF δελτίου + οι δύο καρτέλες ΜΑΡΚ) — στα τιμολόγια η στήλη υπήρχε από τον Ιούνιο
  και **δεν** είχε εμφανιστεί ποτέ. Το ΜΑΡΚ που φτάνει από το panel ελέγχεται πλέον ως προς τη μορφή
  του πριν γραφτεί στο νομικό ιστορικό, ένα κενό ΜΑΡΚ αποθηκεύεται ως NULL (όχι ως `''`), και ένα
  συνοδευτικό migration **ξε-αντιστρέφει** τυχόν παλιές γραμμές ακύρωσης παρόχου (τις ξεχωρίζει από τη
  γραμμή INSERT του ίδιου παραστατικού· όπου δεν μπορεί να αποδείξει, δεν αγγίζει τίποτα, και το
  `down()` **αρνείται** να ρίξει τη στήλη όσο κρατά ΜΑΡΚ που δεν υπάρχει πουθενά αλλού). Ο κανόνας
  «κενό ≠ απόδειξη» ζει πλέον σε **ένα** σημείο (`App\Support\MyData\CancellationMark`) αντί για
  τρία αντίγραφα. Καμία αλλαγή σε δρομολόγηση, ελέγχους ή καταστάσεις.
- **Η διαγραφή εταιρείας/δεδομένων δεν γίνεται πια στα τυφλά (MYD-025)** — το `--force` gate του
  ...
  αλλά χωρίς γραμμένο state, **κάθε ακυρωμένο** τιμολόγιο και **όλα** τα δελτία αποστολής ήταν
  αόρατα — tenant φτιαγμένος από αυτά σβηνόταν χωρίς καμία προειδοποίηση. Νέος `LegalEvidence` =
  **μία** μέτρηση του τι έχει όντως υποβληθεί, κοινή για το gate και για την επιβεβαίωση διαγραφής.
  Η διαγραφή εταιρείας δείχνει πλέον **τι ακριβώς χάνεται**, ότι η **ΑΑΔΕ τα κρατά ούτως ή άλλως**
...
- **Υποβολή ακριβώς μία φορά — και για τα δελτία αποστολής (MYD-021)** — η σήμανση «σε εξέλιξη»
  ...
  lock έληγε, και η επόμενη προσπάθεια ξαναϋπέβαλλε τυφλά. Πλέον γράφεται **πριν** το POST και
  καθαρίζεται μόνο σε εκβάσεις που **αποδεικνύουν** ότι δεν δημιουργήθηκε ΜΑΡΚ (401, 429, protocol
  error πριν την αποστολή, ρητή απόρριψη ΑΑΔΕ). Τα **δελτία αποστολής** δεν είχαν **τίποτα** —
  ούτε lock, ούτε σήμανση, ούτε ανάκτηση, ούτε έλεγχο για τοπικά ακυρωμένο δελτίο· δύο ταυτόχρονα
  αιτήματα μπορούσαν να δημιουργήσουν **δύο** έγγραφα στην ΑΑΔΕ. Τώρα έχουν ακριβώς το ίδιο σχήμα
  με τα τιμολόγια, με ανάκτηση μέσω `RequestTransmittedDocs` (βρίσκει και 9.x). Αν δεν μπορούμε να
  επαληθεύσουμε (ΑΑΔΕ μη προσβάσιμη ή tenant χωρίς read credentials) → **άρνηση** μέσα στο κρίσιμο
  παράθυρο, όχι τυφλή επανάληψη — αλλά **ούτε** μόνιμο μπλοκάρισμα: ένα παραστατικό που δεν μπορεί
  ποτέ να εκδοθεί είναι χειρότερο από τον κίνδυνο. Πήρε lock και η υποβολή **τιμολογίου μέσω
  παρόχου** (ήταν το τελευταίο σημείο χωρίς). Το provider-side exactly-once (hard kill σε πάροχο)
  μένει στο PROV-001.
- **Καμία υποβολή με στοιχεία άλλου tenant (MYD-022)** — οι υπηρεσίες υποβολής δέχονταν την
  `Company` **ανεξάρτητα** από το παραστατικό: ΑΦΜ εκδότη, branch, διαπιστευτήρια myDATA και
  σύμβαση παρόχου από τον tenant· αντισυμβαλλόμενος, τύπος, γραμμές και τρόπος πληρωμής από το
  έγγραφο — χωρίς κανέναν έλεγχο ότι συμφωνούν. Ένα λάθος στον κώδικα ή μια κλήση από
  service/API/CLI μπορούσε να στείλει τα εμπορικά δεδομένα του tenant Β με το ΑΦΜ του Α: και
  ψευδής υποβολή και διαρροή μεταξύ εταιρειών, μη αναστρέψιμα μόλις η ΑΑΔΕ δώσει ΜΑΡΚ. Νέος
  `TenantCoherence` — **μία** fail-closed δικλείδα σε **12** σημεία εξόδου (άμεσο myDATA,
  πάροχος, δελτία αποστολής, και οι 4 ενέργειες κύκλου ζωής), **πριν** από κατασκευή payload,
  εγγραφές audit ή οποιοδήποτε αίτημα. Ελέγχει τα **δεδομένα**, όχι το ambient context: το
  `CompanyScope` είναι τεκμηριωμένα no-op εκτός request (CLI/queue/webhooks), δηλαδή ακριβώς
  εκεί που τρέχει η αυτοματοποίηση.
- **Η ενημέρωση εφαρμογής γίνεται από τον server, όχι από το panel (UPD-001…015)** — το in-app
  **apply** είναι πλέον **OFF by default** (`EKDOSI_UPDATE_IN_APP_APPLY`). Ο **έλεγχος** μένει
  ενεργός: η «Υγεία συστήματος» δείχνει τη νέα έκδοση **και την ακριβή εντολή** (`deploy/update.sh
  vX.Y.Z`) αντί για κουμπί που δεν υπάρχει. Το ίδιο flag κρύβει «Εγκατάσταση ενημέρωσης» και
  «Επαναφορά», και — το ουσιαστικό — το `ekdosi:self-update` **αρνείται κάθε queued run** (update ή
  rollback) και το μαρκάρει failed με την εντολή που πρέπει να τρέξει, αντί να το προσπερνά σιωπηλά
  (ο scheduler τρέχει κάθε λεπτό όσο υπάρχει queued row). Ο μηχανισμός δεν διαγράφηκε — ξανα-ενεργοποιείται
  αφού κλείσουν τα UPD-001…004.
- **Προειδοποίηση σε αλλαγή ΑΦΜ/ΓΕΜΗ εταιρείας (MYD-024)** — η καρτέλα εταιρείας δείχνει πλέον,
  στα πεδία ΑΦΜ και ΓΕΜΗ, πόσα παραστατικά έχουν **ήδη υποβληθεί** με τα τρέχοντα στοιχεία και ότι
  μια τέτοια αλλαγή σημαίνει κανονικά **νέα εταιρεία** (νέα διαπιστευτήρια myDATA/παρόχου) — τα ήδη
  υποβληθέντα MARK μένουν στην ΑΑΔΕ με τα παλιά στοιχεία. Συνειδητά **προειδοποίηση, όχι φραγή**:
  η διόρθωση λάθους καταχώρισης πριν την πρώτη υποβολή είναι θεμιτή.
- **Η σειρά παραστατικού «παγώνει» πάνω στο έγγραφο (MYD-018 / MYD-024)** — η σειρά διαβαζόταν
  **ζωντανά** από το `invoice_types.code`, που είναι μεταβαλλόμενο lookup. Μια μετονομασία σειράς
  ξανάγραφε λοιπόν το τι δηλώνει ένα ήδη αριθμημένο έγγραφο — και, κυρίως, έκανε το **in-doubt
  recovery** να ψάχνει στην ΑΑΔΕ ένα (σειρά, ΑΑ) που η ΑΑΔΕ δεν είχε δει ποτέ: μην βρίσκοντας το
  MARK που ήδη υπήρχε, υπέβαλλε το παραστατικό **δεύτερη φορά** (η ΑΑΔΕ δεν κάνει dedup →
...
- **Ταυτότητα αντισυμβαλλόμενου = το παγωμένο snapshot (MYD-009)** — ο `AadeInvoiceDocument::buildCounterpart`
  ...
  αλλαγή στοιχείων πελάτη σήμερα άλλαζε το XML περσινού παραστατικού, και ένα πιστωτικό που
  αντέγραφε πιστά το snapshot του πρωτοτύπου το ξαναέγραφε με τα σημερινά στοιχεία. Πλέον η νομική
  ταυτότητα χτίζεται **εξ ολοκλήρου** από το snapshot μέσω κοινών helpers (`counterpartAfm()` /
  `counterpartName()` / `counterpartCountryIso()`), κοινών με το payload του παρόχου ώστε ένα
  έγγραφο να μην μπορεί να ονομάζει δύο πρόσωπα. Το fallback στον ζωντανό πελάτη **σταματά στην
  υποβολή** και ισχύει **μόνο αν ο πελάτης είναι όντως ο αντισυμβαλλόμενος** (τα πεδία είναι
  επεξεργάσιμα ενώ το `customer_id` μένει — ίδια τρύπα με το MYD-011· η σύγκριση ΑΦΜ κρατά
  **γράμματα**, ώστε το «DE811234567» να μην ταιριάζει με ελληνικό «811234567»). Ό,τι λύθηκε από
  τον πελάτη **παγώνει στην ίδια εγγραφή με το MARK** (`frozenPartyColumns()`, direct + provider),
  αλλιώς το δηλωμένο πρόσωπο μιας legacy γραμμής θα γινόταν αδιάβαστο τη στιγμή της υποβολής. Ο
  «άδεια χώρα → GR» κανόνας μένει, αλλά **χώρα παρούσα που δεν κανονικοποιείται εξακολουθεί να
  ρίχνει** — το να γίνει το «Neverland» σιωπηλά GR είναι το ίδιο misreport. Ο guard «θέλει πελάτη
  με ΑΦΜ» έγινε «θέλει ΑΦΜ αντισυμβαλλόμενου», οπότε τιμολόγιο με παγωμένο ΑΦΜ και διαγραμμένο
  πελάτη παραμένει εκδόσιμο.
...
### Fixed
- **Χώρα παραλήπτη στο Δελτίο Αποστολής — ποτέ σιωπηλά GR (MYD-011)** — ο παραλήπτης ενός ΔΑ
  μπορεί να είναι πελάτης, **προμηθευτής** ή χειροκίνητη καταχώριση, αλλά μόνο ο πελάτης
  κουβαλούσε χώρα (μέσω FK)· ο submitter διάβαζε `customer?->country ?: 'GR'`, οπότε **κάθε
  ξένος προμηθευτής/χειροκίνητος παραλήπτης δηλωνόταν ως Ελλάδα**. Νέα στήλη
  `delivery_notes.recipient_country` (ISO-2, frozen στην έκδοση) που συμπληρώνεται από όποιο
  party επιλεγεί στον picker (πελάτης **ή** προμηθευτής) και είναι **υποχρεωτική μόλις υπάρχει
  εξωτερικός παραλήπτης** (ΑΦΜ ή πελάτης ή επωνυμία), με **ολόκληρο τον πίνακα ISO** (όχι
  shortlist — αλλιώς αποστολή σε χώρα εκτός λίστας θα ήταν αδύνατη). Ο submitter **αρνείται** να υποβάλει εξωτερικό
  παραλήπτη χωρίς αναγνωρίσιμη χώρα (καθαρό ελληνικό μήνυμα) αντί να τον περάσει ως GR· το
  μοναδικό επιτρεπτό GR default είναι η **ενδοδιακίνηση** — και μόνο όταν δεν υπάρχει **καμία**
  ταυτότητα παραλήπτη (ούτε ΑΦΜ, ούτε επωνυμία, ούτε πελάτης)· η ρητή σεντινέλα `000000000`
  μετράει ως ενδοδιακίνηση, ενώ ένας **επώνυμος ξένος χωρίς ΑΦΜ** δεν περνά πια ως GR. Ο
  `IsoCountry` επικυρώνει το alpha-2 πάνω στον **πλήρη πίνακα ISO** της firebed (ένα `ZZ` δεν
  περνά πια) και αναγνωρίζει **ονόματα χωρών** — τις ελληνικές ετικέτες και των 247 (με folding
  τόνων, ώστε το legacy «ΙΤΑΛΙΑ» να ταιριάζει με το «Ιταλία») συν λατινικά ονόματα/alpha-3· χωρίς
  αυτό η άρνηση θα καθιστούσε **ανέκδοτα** τα υπαρκτά δελτία με ελεύθερο κείμενο χώρας. **Καμία
  τεκμαρτή GR διαδρομή**: ούτε «είναι πελάτης μας», ούτε ΑΦΜ που περνά το ελληνικό checksum —
  ένας ξένος ιδιώτης ή ξένος πελάτης με ελλιπή legacy στοιχεία έχει ακριβώς αυτή τη μορφή (ένας
  9ψήφιος ξένος VAT περνά το mod-11 περίπου 1 στις 10). Στο **filing boundary δεν μαντεύουμε**:
  μόνο η πραγματική ενδοδιακίνηση παίρνει GR, όλα τα άλλα **αρνούνται** με το επίμαχο value στο
  μήνυμα. Η migration κάνει **backfill** από τη χώρα του συνδεδεμένου πελάτη — **μόνο σε μη
  υποβληθέντα** δελτία: η στήλη είναι snapshot του τι υποβλήθηκε, ενώ το `customers.country` είναι
  ζωντανό, οπότε ένας πελάτης που μετακόμισε (ή που απλώς συμπληρώθηκε αργότερα) θα εμφάνιζε χώρα
  που η εγγραφή ΑΑΔΕ ποτέ δεν είχε. Το ίδιο ισχύει και στην **ανάγνωση**: το `recipientCountryIso()`
  κόβει το fallback στον πελάτη μόλις το δελτίο υποβληθεί (`mydata_sent || mydata_mark`) — αλλιώς
  PDF/infolist/CMR θα έδειχναν τη ζωντανή χώρα σαν να ήταν η υποβληθείσα. Και οι δύο διαδρομές
  υποβολής **παγώνουν** πλέον τη χώρα που όντως δηλώθηκε στο `recipient_country`, αλλιώς η ίδια
  αποκοπή θα την εξαφάνιζε από κάθε αναγνώστη τη στιγμή της υποβολής. Το fallback ισχύει επίσης
  μόνο όταν ο παραλήπτης **είναι** ο συνδεδεμένος πελάτης: το `customer_id` είναι Hidden που η
  φόρμα δεν καθαρίζει, οπότε «Müller GmbH / DE811234567» πάνω σε Έλληνα πελάτη δηλωνόταν ως GR. Το **CMR** διαβάζει πλέον τα ίδια shared helpers με το payload
  (`isInternalMovement()` / `externalRecipientAfm()`), ώστε χαρτί και υποβολή να μη λένε άλλα. Την ίδια
  παγωμένη χώρα διαβάζει πλέον και το **CMR** (τύπωνε GR για ξένο παραλήπτη), και εμφανίζεται
  σε infolist/PDF/activity log. Ο διπλός normaliser χώρας ενοποιήθηκε σε
  **`Support\IsoCountry`** (κοινός με το monetary invoice), οπότε το ΔΑ αποκτά και τα aliases
  **EL→GR / UK→GB** που του έλειπαν (ένα 2-γράμματο `EL`/`UK` περνούσε αυτούσιο ως μη-ISO κωδικός).
- **Εφαρμογή ακύρωσης προμηθευτή σε υπάρχον έξοδο (MYD-014)** — μια ακύρωση που έκανε ο
  προμηθευτής (την επιστρέφει το `RequestDocs`) εντοπιζόταν ως «ασυμφωνία κατάστασης» αλλά
  δεν μπορούσε να εφαρμοστεί: ο `ExpenseImporter` παραλείπει ό,τι ΜΑΡΚ υπάρχει ήδη, οπότε το
  τοπικό έξοδο έμενε VALID επ' αόριστον. Νέα υπηρεσία `SyncExpenseStateFromAade` (δίδυμο του
  `SyncInvoiceStateFromAade`) + operator action **«Συγχρονισμός κατάστασης από ΑΑΔΕ»** στην
  κονσόλα Εξόδων: φέρνει την κατάσταση της ΑΑΔΕ στο υπάρχον έξοδο (VALID↔CANCELLED +
  `cancelled_by_mark`), χωρίς επανεισαγωγή/διπλότυπο, με forensic `ExpenseMark` `STATE_SYNC`.
  Τα βιβλία (`LedgerBook`/`VatPeriodReport`) ήδη εξαιρούν τα CANCELLED, οπότε το έξοδο φεύγει
  αμέσως από ΦΠΑ/έσοδα-έξοδα. Επανεκτέλεση αντιπαραβολής μετακινεί τη γραμμή από «ασυμφωνία»
  σε «συμφωνεί». **Ακεραιότητα (review):** ο reconciler κρατά πλέον το πραγματικό **ΜΑΡΚ
  ακύρωσης** (`invoiceMark ⇒ cancellationMark` και από το inline `<cancelledByMark>` και από
  το αυτόνομο `<cancelledInvoicesDoc>`)· η υπηρεσία **αρνείται** να καταχωρίσει CANCELLED χωρίς
...
- **Έλεγχος περιεχομένου στη ζωντανή αντιπαραβολή myDATA (MYD-017)** — η αντιπαραβολή
  ...
  ένα τοπικό edit μετά την υποβολή, μια ελλιπής εισαγωγή ή λάθος αντιστοίχιση ΜΑΡΚ έδινε
  ψευδές «πράσινο». Νέος κάδος **«Διαφορά περιεχομένου»** (`contentMismatch`), ξεχωριστός από
  την «Ασυμφωνία κατάστασης»: όταν ΜΑΡΚ+κατάσταση συμφωνούν, συγκρίνονται πλέον **μικτό**
  (±0,01 ανοχή), **τύπος §8.1**, **σειρά/ΑΑ**, **ημ/νία** και **ΑΦΜ αντισυμβαλλόμενου** (όπου
  το επιστρέφει η ΑΑΔΕ). Κοινός καθαρός `ReconciliationContentComparator` (πωλήσεις & έξοδα,
  χωρίς drift)· το `problem` λέει ακριβώς ποια πεδία διαφέρουν, καμία σιωπηλή επανεγγραφή. Και
  οι δύο κονσόλες εμφανίζουν τον νέο κάδο. **Ελλιπή vs σύγκρουση (review):** ένα πεδίο που η
  ΑΑΔΕ επιστρέφει αλλά **λείπει τοπικά** δεν είναι σύγκρουση αλλά ούτε «συμφωνεί» — μπαίνει σε
  ...
  ψευδής σύγκρουση). Σύγκριση net/VAT-split → BACKLOG. **Κλείσιμο του AADE-side fail-open:**
  ο παραπάνω date guard απέτρεπε ψευδή *σύγκρουση* αλλά άφηνε ψευδές *πράσινο* — ό,τι πεδίο δεν
  επέστρεφε η ΑΑΔΕ απλώς παραλειπόταν. Πλέον τα **υποχρεωτικά** πεδία κεφαλίδας (**μικτό, τύπος
  §8.1, σειρά, ΑΑ, ημ/νία**) όταν λείπουν ή δεν αναγνωρίζονται πάνε σε `contentIncomplete`
  («λείπει από την ΑΑΔΕ — ανεπαλήθευτο»): δεν επαληθεύτηκε το παραστατικό, άρα ποτέ «συμφωνεί».
  Το **ΑΦΜ αντισυμβαλλόμενου παραμένει προαιρετικό** (η λιανική 11.x δεν το έχει).
- **Σύγκριση καθαρής αξίας + σωστή βάση ποσών στην αντιπαραβολή (MYD-017)** — το μικτό από
  μόνο του δεν πιάνει λάθος κατηγορία ΦΠΑ όπου net/ΦΠΑ αλληλοαναιρούνται στο ίδιο μικτό
  (100+24 τοπικά έναντι 110+14 ΑΑΔΕ)· συγκρίνεται πλέον και η **καθαρή αξία**
  (`<totalNetValue>`, υποχρεωτικό πεδίο). Κυρίως όμως διορθώθηκε η **βάση των ποσών**: τα
  `net_total`/`gross_total` είναι **άλλο roll-up** από αυτό που υποβάλλεται (ο
  `InvoiceVatBreakdown` στρογγυλοποιεί **ανά συντελεστή ΦΠΑ**, τα columns μία φορά στο σύνολο —
  ο `RecomputeInvoiceTotals` το προειδοποιεί ρητά), και το `<totalGrossValue>` της ΑΑΔΕ κουβαλά
  επιπλέον το **[208] adjustment**. Νέο `FiledInvoiceTotals` ανασυνθέτει ό,τι **πράγματι
  υποβλήθηκε** (ίδιος κανόνας με τον submitter) και είναι πλέον η μία βάση για την αντιπαραβολή,
  τη στήλη «μικτό» της κονσόλας ΚΑΙ τη «Σύγκριση με ΑΑΔΕ» του κάθε παραστατικού (οι δύο οθόνες
  αλληλοαναιρούνταν). Όπου δεν ανασυντίθεται πιστά — τιμολόγιο χωρίς γραμμές, ή **παρακράτηση
...
- **Πρόσημο επιστροφής POS 8.5 στην «Εικόνα από myDATA» (MYD-015)** — ο τύπος 8.5
  ...
  (8.5) δίνει καθαρά 60€, όχι 140€. Επιπλέον fixture: ένα zero-value 8.6 (Δελτίο
  Παραγγελίας Εστίασης) μετριέται αλλά προσθέτει 0 στα έσοδα (δεν φουσκώνει την εικόνα).
